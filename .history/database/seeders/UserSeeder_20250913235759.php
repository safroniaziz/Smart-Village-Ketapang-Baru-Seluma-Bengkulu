            ...
            if (File::exists($filePath)) {
                try {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE && is_array($data)) {
                        $dusunNumber = $index + 1; // kadun1 = dusun 1, kadun2 = dusun 2, etc.
                        
                        foreach ($data as $item) {
                            $this->processUserData($item, $dusunNumber);
                        }
                        
                        echo "Successfully loaded " . count($data) . " records from {$fileName}\n";
                    } else {
                        echo "Invalid JSON format in {$fileName}\n";
                    }
                } catch (\Exception $e) {
                    echo "Error processing {$fileName}: " . $e->getMessage() . "\n";
                }
            } else {
                echo "File {$fileName} not found in public/assets/files/\n";
            }
        }
    }

    private function processUserData(array $item, int $dusunNumber): void
    {
        ...
        $userData = array_merge([
            'name' => $item['nama_lengkap'] ?? 'Unknown',
