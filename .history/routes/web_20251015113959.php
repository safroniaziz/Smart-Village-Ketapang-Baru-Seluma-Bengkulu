...
use App\Http\Controllers\PotensiWisataPublicController;
...
use App\Http\Controllers\Admin\PaguEarmarkController;
use App\Http\Controllers\Admin\BltDdController;
...
// Public Potensi Wisata Routes
Route::prefix('potensi-wisata')->name('potensi-wisata.')->group(function () {
    Route::get('/test', function() {
        $wisata = App\Models\PotensiWisata::first();
        return view('potensi-wisata.test', compact('wisata'));
    })->name('test');
    Route::get('/', [App\Http\Controllers\PotensiWisataPublicController::class, 'index'])->name('index');
    Route::post('/increment-views/{id}', [App\Http\Controllers\PotensiWisataPublicController::class, 'incrementViews'])->name('increment-views');
    Route::get('/popular', [App\Http\Controllers\PotensiWisataPublicController::class, 'popular'])->name('popular');
    Route::get('/search', [App\Http\Controllers\PotensiWisataPublicController::class, 'search'])->name('search');
    Route::get('/category/{kategori}', [App\Http\Controllers\PotensiWisataPublicController::class, 'getByCategory'])->name('category');
    Route::get('/map-data', [App\Http\Controllers\PotensiWisataPublicController::class, 'mapData'])->name('map-data');
    Route::get('/{slug}', [App\Http\Controllers\PotensiWisataPublicController::class, 'show'])->name('show');
});
...
Route::get('/anggaran/api/data', [AnggaranController::class, 'getApiData'])->name('anggaran.api.data');
...
Route::middleware(['auth'])->group(function () {
    ...
    // Admin Peta Desa Routes
    Route::prefix('admin/peta-desa')->name('admin.peta-desa.')->group(function () {
        // Main index
        Route::get('/', [PetaDesaController::class, 'index'])->name('index');

        // Lahan Point Routes
        Route::prefix('lahan')->name('lahan.')->group(function () {
            Route::get('/', [PetaDesaController::class, 'lahanIndex'])->name('index');
            Route::get('/data', [PetaDesaController::class, 'lahanData'])->name('data');
            Route::post('/', [PetaDesaController::class, 'lahanStore'])->name('store');
            Route::get('/{id}', [PetaDesaController::class, 'lahanShow'])->name('show');
            Route::put('/{id}', [PetaDesaController::class, 'lahanUpdate'])->name('update');
            Route::delete('/{id}', [PetaDesaController::class, 'lahanDestroy'])->name('destroy');
        });

        // Batas Wilayah Routes
        Route::prefix('batas-wilayah')->name('batas.')->group(function () {
            Route::get('/', [PetaDesaController::class, 'batasWilayahIndex'])->name('index');
            Route::get('/data', [PetaDesaController::class, 'batasWilayahData'])->name('data');
            Route::post('/', [PetaDesaController::class, 'batasWilayahStore'])->name('store');
            Route::get('/{id}', [PetaDesaController::class, 'batasWilayahShow'])->name('show');
            Route::put('/{id}', [PetaDesaController::class, 'batasWilayahUpdate'])->name('update');
            Route::delete('/{id}', [PetaDesaController::class, 'batasWilayahDestroy'])->name('destroy');
        });

        // Warga Coordinates Routes
        Route::prefix('warga')->name('warga.')->group(function () {
            Route::get('/', [PetaDesaController::class, 'wargaIndex'])->name('index');
            Route::get('/data', [PetaDesaController::class, 'wargaData'])->name('data');
            Route::get('/{id}', [PetaDesaController::class, 'wargaShow'])->name('show');
            Route::put('/{id}', [PetaDesaController::class, 'wargaUpdate'])->name('update');
        });
    });
...
Route::get('/api/warga-with-coordinates-count', [HomeController::class, 'wargaWithCoordinatesCount']);
Route::get('/api/desa-count', [HomeController::class, 'desaCount']);
...
Route::prefix('admin')->name('admin.')->middleware(['auth'])->group(function () {
    ...
    // Admin - APBDes Management
    Route::resource('apbdes', APBDesController::class);
    Route::post('apbdes/bulk-store', [APBDesController::class, 'bulkStore'])->name('apbdes.bulk-store');

    // Admin - Pagu Earmark Management
    Route::resource('pagu-earmark', PaguEarmarkController::class);
    Route::post('pagu-earmark/bulk-store', [PaguEarmarkController::class, 'bulkStore'])->name('pagu-earmark.bulk-store');

    // Admin - BLT-DD Management
    Route::resource('blt-dd', BltDdController::class);
    Route::post('blt-dd/bulk-store', [BltDdController::class, 'bulkStore'])->name('blt-dd.bulk-store');
    ...
    // Admin - Potensi Wisata Management
    Route::prefix('potensi-wisata')->name('potensi-wisata.')->group(function () {
        Route::get('/', [App\Http\Controllers\Admin\PotensiWisataController::class, 'index'])->name('index');
        Route::get('/create', [App\Http\Controllers\Admin\PotensiWisataController::class, 'create'])->name('create');
        Route::post('/', [App\Http\Controllers\Admin\PotensiWisataController::class, 'store'])->name('store');

        // Additional routes for Potensi Wisata (move before parameterized routes)
        Route::post('/bulk-action', [App\Http\Controllers\Admin\PotensiWisataController::class, 'bulkAction'])->name('bulk-action');

        Route::get('/{potensiWisata}', [App\Http\Controllers\Admin\PotensiWisataController::class, 'show'])->name('show');
        Route::get('/{potensiWisata}/edit', [App\Http\Controllers\Admin\PotensiWisataController::class, 'edit'])->name('edit');
        Route::put('/{potensiWisata}', [App\Http\Controllers\Admin\PotensiWisataController::class, 'update'])->name('update');
        Route::delete('/{potensiWisata}', [App\Http\Controllers\Admin\PotensiWisataController::class, 'destroy'])->name('destroy');
    });

    // Admin - Kategori Wisata Management
    Route::resource('kategori-wisata', App\Http\Controllers\Admin\KategoriWisataController::class)
        ->parameters(['kategori-wisata' => 'kategoriWisata']);

    // Admin - Gallery Foto Management
    Route::prefix('gallery-foto')->name('gallery-foto.')->group(function () {
        Route::get('/', [App\Http\Controllers\Admin\GalleryFotoController::class, 'index'])->name('index');
        Route::get('/create', [App\Http\Controllers\Admin\GalleryFotoController::class, 'create'])->name('create');
        Route::post('/', [App\Http\Controllers\Admin\GalleryFotoController::class, 'store'])->name('store');
        Route::get('/{galleryFoto}', [App\Http\Controllers\Admin\GalleryFotoController::class, 'show'])->name('show');
        Route::get('/{galleryFoto}/edit', [App\Http\Controllers\Admin\GalleryFotoController::class, 'edit'])->name('edit');
        Route::put('/{galleryFoto}', [App\Http\Controllers\Admin\GalleryFotoController::class, 'update'])->name('update');
        Route::delete('/{galleryFoto}', [App\Http\Controllers\Admin\GalleryFotoController::class, 'destroy'])->name('destroy');

        // Additional routes for Gallery Foto
        Route::post('/bulk-action', [App\Http\Controllers\Admin\GalleryFotoController::class, 'bulkAction'])->name('bulk-action');
        Route::post('/{galleryFoto}/set-hero', [App\Http\Controllers\Admin\GalleryFotoController::class, 'setHero'])->name('set-hero');
        Route::patch('/{galleryFoto}/toggle-status', [App\Http\Controllers\Admin\GalleryFotoController::class, 'toggleStatus'])->name('toggle-status');
    });

    // Admin - Lahan Management
    Route::resource('lahan', \App\Http\Controllers\Admin\LahanController::class)->parameters(['lahan' => 'lahan_point:id']);

    // Pengajuan Surat Management Routes
    Route::prefix('pengajuan-surat')->name('pengajuan-surat.')->group(function () {
        Route::get('/', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'index'])->name('index');
        Route::get('/create', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'create'])->name('create');
        Route::post('/store', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'store'])->name('store');
        Route::get('/{pengajuanSurat}', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'show'])->name('show');
        Route::get('/{pengajuanSurat}/edit', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'edit'])->name('edit');
        Route::put('/{pengajuanSurat}', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'update'])->name('update');
        Route::patch('/{pengajuanSurat}/approve', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'approve'])->name('approve');
        Route::patch('/{pengajuanSurat}/reject', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'reject'])->name('reject');
        Route::patch('/{pengajuanSurat}/jenis-ttd', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'updateJenisTtd'])->name('update-jenis-ttd');
        Route::patch('/{pengajuanSurat}/jenis-ttd-camat', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'updateJenisTtdCamat'])->name('update-jenis-ttd-camat');
        Route::get('/{pengajuanSurat}/generate-pdf', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'generatePdf'])->name('generate-pdf');
        Route::delete('/{pengajuanSurat}', [App\Http\Controllers\Admin\AdminPengajuanSuratController::class, 'destroy'])->name('destroy');
    });
});
...
// API untuk auto-complete orang tua
Route::get('/api/search-orang-tua', [App\Http\Controllers\SuratController::class, 'searchOrangTua'])->name('api.search-orang-tua');

// Surat Verification Route (for QR Code verification)
Route::get('/surat/verify/{trackingNumber}', [App\Http\Controllers\SuratController::class, 'verifySurat'])->name('surat.verify');
