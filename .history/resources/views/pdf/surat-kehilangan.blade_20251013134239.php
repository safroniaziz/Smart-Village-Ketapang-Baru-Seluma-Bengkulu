...
<head>
    ...
    <style>
        ...
        @page {
            size: A4 portrait;
            margin: 10mm 15mm;  /* top/bottom left/right */
        }

        body {
            font-family: 'DejaVu Sans', Arial, sans-serif;
            font-size: 9pt;  /* Kecilkan font dari 9.5pt */
            line-height: 1.3;  /* Perapat line height */
            color: #000;
            margin: 0;
            padding: 0;
        }
        ...
        .page {
            max-width: 100%;
            margin: 0;
            padding: 0;  /* Hilangkan padding, pakai @page margin */
            background: #fff;
        }
        ...
        .section {
            ...
            page-break-inside: avoid;  /* Cegah section terpotong */
        }
        .section-title {
            font-size: 11pt; font-weight: 700; margin-bottom: 6px;  /* Kurangi margin */
        ...
        .data-label {
            width: 200px;
            font-weight: 600;
            font-size: 9pt;  /* Sesuaikan dengan body font */
            color: #374151;
        }
        ...
        .footer {
            ...
            page-break-inside: avoid;  /* Jangan pisah footer */
        ...
        .signature-title {
            margin-bottom: 50px;  /* Kurangi space tanda tangan */
        }
...
<body>
    <div class="page">
        ...
        <div class="section">
            ...
            <table class="data-table">
                <tr>
                    ...
                    <td class="data-value">{{ $nama_pemohon ?? 'HELESMI' }}</td>
                </tr>
                <tr>
                    <td class="data-label">NIK</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $nik ?? '1701115607870003' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Tempat, Tanggal Lahir</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $tempat_lahir ?? 'Muara Dua' }}, {{ $tanggal_lahir ?? '16 Juli 1987' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Jenis Kelamin</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $jenis_kelamin ?? 'Perempuan' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Agama</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $agama ?? 'Islam' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Pekerjaan</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $pekerjaan ?? 'Mengurus Rumah Tangga' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Alamat</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $alamat ?? 'Desa Ketapang Baru, Kecamatan Semidang Alas Maras, Kabupaten Seluma' }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <table class="data-table">
                <tr>
                    <td class="data-label">Dokumen/Barang</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $jenis_dokumen ?? 'KARTU KELUARGA' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Nomor Dokumen</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $nomor_dokumen ?? '1705052511190005' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Tempat Kehilangan</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $tempat_kehilangan ?? 'Desa Ketapang Baru' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Waktu Kehilangan</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $waktu_kehilangan ?? '6 Bulan yang lalu' }}</td>
                </tr>
                <tr>
                    <td class="data-label">Keperluan</td>
                    <td class="data-separator">:</td>
                    <td class="data-value">{{ $keperluan ?? 'Pengurusan administrasi kependudukan' }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="footer" style="position: relative;">
            <!-- Tembusan (Sebelah Kiri) - Dinamis -->
            @if(isset($tembusan) && !empty($tembusan))
            <div style="position: absolute; left: 0; bottom: 0; width: 200px; text-align: left;">
                <p style="font-size: 12pt; margin: 2px 0; font-weight: bold;">Tembusan :</p>
                @foreach(explode("\n", $tembusan) as $item)
                    @if(trim($item))
                        <p style="font-size: 12pt; margin: 2px 0;">{{ trim($item) }}</p>
                    @endif
                @endforeach
            </div>
            @endif
            <div class="footer-date">
                {{ $tempat_surat ?? 'Ketapang Baru' }}, {{ $tanggal_surat ?? '24 August 2025' }}
            </div>
