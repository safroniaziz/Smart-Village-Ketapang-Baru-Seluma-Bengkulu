                    ...
                    <div class="chart-container-premium">
                        <div id="genderChartLoading" class="absolute inset-0 flex items-center justify-center bg-gray-50 rounded-lg">
                            <div class="text-center">
                                ...
                                <p class="text-sm text-gray-600">Grafik Gender</p>
                                <p class="text-xs text-gray-500">Laki-laki: 1,280 • Perempuan: 1,220</p>
                            </div>
                        </div>
                        <canvas id="genderChart" width="300" height="300" style="display: none;"></canvas>
                    </div>
                </div>
                    ...
                    <div class="chart-container-premium">
                        <div id="ageChartLoading" class="absolute inset-0 flex items-center justify-center bg-gray-50 rounded-lg">
                            <div class="text-center">
                                ...
                                <p class="text-sm text-gray-600">Grafik Usia</p>
                                <p class="text-xs text-gray-500">0-17: 520 • 18-30: 680 • 31-45: 750 • 46-60: 420 • 60+: 130</p>
                            </div>
                        </div>
                        <canvas id="ageChart" width="600" height="300" style="display: none;"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
console.log('🚀 Loading EPIC Charts with Advanced Visual Effects!');

$(document).ready(function() {
    console.log('🎯 Ready to create STUNNING charts!');
    ...
    function initializeCharts() {
        ...
        if (genderCtx) {
            console.log('🎨 Creating EPIC Gender Chart with 3D Effects...');
            const genderLoading = document.getElementById('genderChartLoading');

            // Hide loading and show canvas
            if (genderLoading) {
                genderLoading.style.display = 'none';
            }
            genderCtx.style.display = 'block';
                    ...
                    options: {
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    ...
                                    afterLabel: function(context) {
                                        const total = context.dataset.data.reduce((a, b) => a + b, 0);
                                        return `🏘️ Total: ${total.toLocaleString()} warga`;
                                    }
                                }
                            }
                        },
                        ...
                        onHover: (event, activeElements, chart) => {
                            ...
                            if (activeElements.length > 0) {
                                ...
                                chart.canvas.style.filter = 'drop-shadow(0 0 20px rgba(59, 130, 246, 0.7)) brightness(1.1)';
                                chart.canvas.style.transform = 'scale(1.02)';
                                chart.canvas.style.transition = 'all 0.3s ease';
                            } else {
    ...
    // 🚀 EPIC Age Distribution Chart with 3D Bars and Animations
    const ageCtx = document.getElementById('ageChart');
    console.log('Age chart canvas found:', !!ageCtx);

    let ageChart;
    if (ageCtx) {
        console.log('🎨 Creating STUNNING Age Chart with 3D Effects...');
            ...
            ageChart = new Chart(ageCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: [
                            // ✨ Stunning gradient bars with rainbow vibes
                            'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', // Baby blue to purple
                            'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)', // Pink to coral  
                            'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)', // Sky blue to cyan
                        ...
                        borderRadius: 15,
                        borderSkipped: false,
                        // 🌟 Epic 3D shadow and hover effects
                        shadowOffsetX: 8,
                        shadowOffsetY: 8,
                        shadowBlur: 25,
                        shadowColor: 'rgba(0, 0, 0, 0.3)',
                        hoverBorderWidth: 5,
                        hoverBorderColor: '#FFD700', // Gold hover border
                        hoverShadowOffsetX: 12,
                        hoverShadowOffsetY: 12,
                        hoverShadowBlur: 30,
                        hoverShadowColor: 'rgba(255, 215, 0, 0.6)' // Golden glow on hover
                            'rgb(234, 88, 12)',
                            'rgb(220, 38, 38)'
                        ]
                    }]
                ...
                options: {
                    ...
                    animation: {
                        ...
                        animateScale: true,
                        onComplete: function() {
                            console.log('🎉 Age chart animation complete!');
                            // Add sparkle effects on completion
                            this.canvas.style.filter = 'drop-shadow(0 0 20px rgba(255, 215, 0, 0.3))';
                        }
                    },
                    interaction: {
                        intersect: false,
                    ...
                    onHover: function(event, elements) {
                        ...
                        if (elements && elements.length > 0) {
                            ...
                            canvas.style.filter = 'drop-shadow(0 0 30px rgba(255, 215, 0, 0.5)) brightness(1.1)';
                        } else {
                            canvas.style.cursor = 'default';
                            canvas.style.transform = 'scale(1)';
                            canvas.style.filter = 'drop-shadow(0 0 20px rgba(255, 215, 0, 0.3))';
                        }
                    },
                    plugins: {
                        legend: {
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                ...
                                label: function(context) {
                                    ...
                                    return `👥 ${context.parsed.y.toLocaleString()} orang (${percentage}%)`;
                                },
                                afterLabel: function(context) {
                                    // Add fun facts based on age group
                                    const ageGroup = context.label;
                                    if (ageGroup.includes('0-17')) return '🎓 Generasi masa depan!';
                                    if (ageGroup.includes('18-30')) return '💪 Generasi produktif muda!';
                                    if (ageGroup.includes('31-45')) return '🏆 Tulang punggung ekonomi!';
                                    if (ageGroup.includes('46-60')) return '🎯 Generasi berpengalaman!';
                                    if (ageGroup.includes('60+')) return '👑 Generasi emas!';
                                    return '';
                                }
                            }
                        },
    ...
    // Fallback function to show static content if charts fail to load
    function showChartFallback() {
        console.log('Showing chart fallback content');
        ...
        if (genderLoading) {
            genderLoading.innerHTML = `
                <div class="text-center p-8">
                    ...
                    <h4 class="text-lg font-bold text-gray-800 mb-2">Distribusi Gender</h4>
                    <div class="space-y-2">
                        <div class="flex items-center justify-between bg-blue-50 rounded-lg p-3">
                            ...
                            <span class="text-sm font-bold text-blue-700">1,280 (51.2%)</span>
                        </div>
                        <div class="flex items-center justify-between bg-pink-50 rounded-lg p-3">
                            <div class="flex items-center">
                                <div class="w-4 h-4 bg-pink-500 rounded-full mr-3"></div>
                                <span class="text-sm font-medium text-gray-700">Perempuan</span>
                            </div>
                            <span class="text-sm font-bold text-pink-700">1,220 (48.8%)</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        if (ageLoading) {
            ageLoading.innerHTML = `
                <div class="text-center p-8">
                    ...
                    <h4 class="text-lg font-bold text-gray-800 mb-4">Distribusi Usia</h4>
                    <div class="space-y-3">
                        <div class="flex items-center justify-between bg-green-50 rounded-lg p-3">
                            ...
                            <span class="text-sm font-bold text-green-700">520 (20.8%)</span>
                        </div>
                        <div class="flex items-center justify-between bg-blue-50 rounded-lg p-3">
                            <span class="text-sm font-medium text-gray-700">18-30 Tahun</span>
                            <span class="text-sm font-bold text-blue-700">680 (27.2%)</span>
                        </div>
                        <div class="flex items-center justify-between bg-purple-50 rounded-lg p-3">
                            <span class="text-sm font-medium text-gray-700">31-45 Tahun</span>
                            <span class="text-sm font-bold text-purple-700">750 (30.0%)</span>
                        </div>
                        <div class="flex items-center justify-between bg-orange-50 rounded-lg p-3">
                            <span class="text-sm font-medium text-gray-700">46-60 Tahun</span>
                            <span class="text-sm font-bold text-orange-700">420 (16.8%)</span>
                        </div>
                        <div class="flex items-center justify-between bg-red-50 rounded-lg p-3">
                            <span class="text-sm font-medium text-gray-700">60+ Tahun</span>
                            <span class="text-sm font-bold text-red-700">130 (5.2%)</span>
                        </div>
                    </div>
                </div>
            `;
    ...
    // Set timeout for fallback if charts don't load within 10 seconds
    const chartTimeout = setTimeout(() => {
        console.log('Chart loading timeout reached, showing fallback');
        showChartFallback();
    }, 10000);

    // Call initializeCharts with retry mechanism
    if (typeof Chart !== 'undefined') {
        ...
        function tryInitialize() {
            attempts++;
            console.log(`Attempt ${attempts} to initialize charts`);
            ...
            if (typeof Chart !== 'undefined') {
                ...
                console.log(`Chart.js still not found, retrying in ${attempts * 500}ms`);
                setTimeout(tryInitialize, attempts * 500);
            } else {
