@forelse($wargas as $index => $warga)
    @php
        $umur = $warga->tanggal_lahir ? \Carbon\Carbon::parse($warga->tanggal_lahir)->age : '-';
        $nomorUrut = ($wargas->currentPage() - 1) * $wargas->perPage() + $index + 1;
    @endphp
    <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200 {{ $warga->is_kepala_keluarga ? 'bg-blue-50 border-l-4 border-blue-500' : '' }}">
