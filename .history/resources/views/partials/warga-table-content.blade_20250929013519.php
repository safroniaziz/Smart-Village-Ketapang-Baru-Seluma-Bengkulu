                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex items-center space-x-2 text-sm text-gray-600">
                            <span class="px-2 py-1 bg-blue-50 text-blue-700 rounded">
                                L: {{ $familyMembers->where('jenis_kelamin', 'Laki-laki')->count() }}
                            </span>
                            <span class="px-2 py-1 bg-pink-50 text-pink-700 rounded">
                                P: {{ $familyMembers->where('jenis_kelamin', 'Perempuan')->count() }}
                            </span>
                        </div>
        ...
        <tr class="family-member group" data-family="{{ $noKK }}" data-warga-id="{{ $warga->id }}">
            ...
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <!-- Avatar -->
                    <div class="w-10 h-10 {{ $warga->jenis_kelamin === 'Laki-laki' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600' }} rounded-full flex items-center justify-center text-sm font-medium mr-4">
            ...
            <td class="px-6 py-4 text-center">
                @if($warga->tanggal_lahir)
                    <span class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($warga->tanggal_lahir)->age }}</span>
                @else
