@forelse($wargas as $index => $warga)
    ...
    <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200">
        ...
        <td class="px-6 py-4 text-sm">
            @php
                $statusColors = [
                    'Belum Kawin' => ['bg' => 'from-blue-100 to-indigo-100', 'text' => 'text-blue-800', 'border' => 'border-blue-200', 'icon' => 'text-blue-600', 'iconName' => 'fa-user'],
                    'Kawin' => ['bg' => 'from-emerald-100 to-teal-100', 'text' => 'text-emerald-800', 'border' => 'border-emerald-200', 'icon' => 'text-emerald-600', 'iconName' => 'fa-heart'],
                    'Cerai Hidup' => ['bg' => 'from-orange-100 to-amber-100', 'text' => 'text-orange-800', 'border' => 'border-orange-200', 'icon' => 'text-orange-600', 'iconName' => 'fa-user-minus'],
                    'Cerai Mati' => ['bg' => 'from-gray-100 to-slate-100', 'text' => 'text-gray-800', 'border' => 'border-gray-200', 'icon' => 'text-gray-600', 'iconName' => 'fa-user-times'],
                ];
                $statusStyle = $statusColors[$warga->status_perkawinan] ?? $statusColors['Belum Kawin'];
            @endphp
            <span class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-semibold bg-gradient-to-r {{ $statusStyle['bg'] }} {{ $statusStyle['text'] }} border {{ $statusStyle['border'] }}">
                <i class="fas {{ $statusStyle['iconName'] }} mr-1.5 {{ $statusStyle['icon'] }}"></i>
