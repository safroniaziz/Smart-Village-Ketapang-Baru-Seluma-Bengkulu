...
@section('title', 'Tambah Pagu Earmark')
@section('page-header', 'Tambah Pagu Earmark')

@section('content')
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Tambah Data Pagu Earmark</h1>
    ...
    <form action="{{ route('admin.pagu-earmark.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="tahun" id="tahun" 
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('tahun') border-red-500 @enderror"
                        required>
            ...
            <div>
                ...
                <input type="text" name="kode_rekening" id="kode_rekening" 
                       ...
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('kode_rekening') border-red-500 @enderror"
                       required>
                @error('kode_rekening')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div>
            ...
            <textarea name="nama_kegiatan" id="nama_kegiatan" rows="3"
                      ...
                      class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nama_kegiatan') border-red-500 @enderror"
        ...
        <div>
            ...
            <input type="number" name="jumlah_pagu" id="jumlah_pagu" 
                   value="{{ old('jumlah_pagu') }}"
                   placeholder="Masukkan jumlah pagu..."
                   class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('jumlah_pagu') border-red-500 @enderror"
                   required>
            @error('jumlah_pagu')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <textarea name="target_output" id="target_output" rows="4"
                      ...
                      class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('target_output') border-red-500 @enderror"
                      required>{{ old('target_output') }}</textarea>
            @error('target_output')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="keterangan" id="keterangan" rows="3"
                      ...
                      class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('keterangan') border-red-500 @enderror">{{ old('keterangan') }}</textarea>
            @error('keterangan')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
