@if(isset($familyGroups) && $familyGroups->count() > 0)
    @foreach($familyGroups as $noKK => $familyMembers)
        ...
        <tr class="border-bottom border-gray-300 family-member" data-family="{{ $noKK }}" data-warga-id="{{ $warga->id }}">
            ...
            <td>
                <div class="d-flex align-items-center">
                    <div class="symbol symbol-40px me-3 foto-square" style="border-radius: 8px !important;">
                        <div class="symbol-label overflow-hidden foto-square-inner" style="border-radius: 8px !important;">
                            @if($warga->foto)
                                <img src="{{ asset('storage/' . $warga->foto) }}" alt="{{ $warga->nama_lengkap }}" class="w-100 h-100 object-fit-cover foto-square-img" style="border-radius: 8px !important;" />
                            @else
                                <div class="symbol-label fs-6 fw-bold foto-square-placeholder {{ in_array($warga->jenis_kelamin, ['Laki-laki', 'L']) ? 'bg-light-primary text-primary' : 'bg-light-danger text-danger' }}" style="border-radius: 8px !important;">
