                ...
                <div class="card-header border-0 pt-6">
                    @if(request('q'))
                        <div class="alert alert-info d-flex align-items-center p-5 mb-5">
                            <i class="ki-duotone ki-magnifier fs-2hx text-info me-4">
                                <span class="path1"></span>
                                <span class="path2"></span>
                            </i>
                            <div class="d-flex flex-column">
                                <h4 class="mb-1 text-info">Hasil Pencarian</h4>
                                <span>Menampilkan hasil untuk: "<strong>{{ request('q') }}</strong>"</span>
                                <span class="text-muted fs-7">Ditemukan {{ $wargas->total() }} data dari {{ $wargas->count() }} yang ditampilkan</span>
                            </div>
                        </div>
                    @endif
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            <form method="GET" action="{{ route('admin.data-warga.index') }}" class="d-flex align-items-center position-relative" id="search-form">
                                <!-- Preserve other filters -->
                                @foreach(request()->except(['q', 'page']) as $key => $value)
                                    <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                @endforeach
                                
                                <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-5 z-index-2">
                                    <span class="path1"></span>
                                    <span class="path2"></span>
                                </i>
                                <input type="text" name="q" id="kt_table_search"
                                ...
                                @if(request('q'))
                                    <a href="{{ route('admin.data-warga.index', request()->except(['q', 'page'])) }}" class="btn btn-sm btn-light ms-2" title="Clear search">
...
$(document).ready(function() {
    ...
    $('#kt_table_search').on('keypress', function(e) {
        if (e.which === 13) { // Enter key
            $('#search-form').submit();
        }
    });
    
    // Optional: Auto-submit search after typing (debounced)
    let searchTimeout;
