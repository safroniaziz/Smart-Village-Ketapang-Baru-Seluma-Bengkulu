            ...
            @foreach($familyGroups as $noKk => $familyMembers)
                ...
                @foreach($familyMembers->sortByDesc('is_kepala_keluarga') as $warga)
                    <tr class="{{ $warga->is_kepala_keluarga ? 'kepala-keluarga' : '' }}">
                        ...
                        <td>{{ $warga->dusun ?? '-' }}</td>
                        <td>{{ $warga->agama ?? '-' }}</td>
                        <td>{{ $warga->mata_pencaharian ? ($warga->mata_pencaharian == 'DLL' ? 'Lain-Lain' : $warga->mata_pencaharian) : '-' }}</td>
        ...
        <table style="width: 100%; border: none; font-size: 9px;">
            <tr style="border: none;">
                <td style="border: none; width: 60%;">
                    ...
                    • Laki-laki: {{ $wargas->whereIn('jenis_kelamin', ['Laki-laki', 'L'])->count() }} orang<br>
                    • Perempuan: {{ $wargas->whereIn('jenis_kelamin', ['Perempuan', 'P'])->count() }} orang
                </td>
