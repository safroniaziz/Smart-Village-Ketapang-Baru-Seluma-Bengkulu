...
/* Photo Upload Styling - Same as Edit Page */
.current-photo {
    width: 120px;
    height: 160px;
    border-radius: 0.75rem;
                        ...
                        <div class="row g-5">
                            <div class="col-md-6">
                                <div class="photo-upload-wrapper mb-4 text-center">
                                    ...
                                    <button type="button" class="btn btn-primary change-photo-btn d-flex align-items-center justify-content-center">
...
// Hold processed photo for browsers without DataTransfer (e.g., Safari)
let croppedPhotoBlob = null;
let croppedPhotoName = null;
...
$(document).ready(function() {
    ...
    // Photo upload functionality - add debugging for button click
    ...
    // Use document delegation to ensure event works even if DOM changes
    $(document).off('click', '.change-photo-btn').on('click', '.change-photo-btn', function(e) {
        console.log('Change photo button clicked via delegation!');
        e.preventDefault();
        e.stopPropagation();
        $('#foto').trigger('click');
        return false;
    ...
    $(document).off('change', '#foto').on('change', '#foto', function(e) {
        ...
        if (file) {
            ...
            // Show loading in current photo area
            $('#current-photo').html(`
                <div class="d-flex align-items-center justify-content-center h-100">
                    <div class="text-center">
                        ...
                        <p class="mt-2 text-muted small">Memproses foto...</p>
                    </div>
                </div>
            `);
            ...
            reader.onload = function(e) {
                ...
                img.onload = function() {
                    ...
                    // Check if needs cropping to 300x400 (portrait 3:4 ratio) first
                    if (originalWidth !== 300 || originalHeight !== 400) {
                        // Create canvas for cropping
                        const canvas = document.createElement('canvas');
                        const ctx = canvas.getContext('2d');

                        // Set target dimensions (portrait 3:4)
                        const targetWidth = 300;
                        const targetHeight = 400;
                        canvas.width = targetWidth;
                        canvas.height = targetHeight;

                        // Calculate crop area (center crop to maintain aspect ratio)
                        ...
                        // Draw cropped image (center crop for portrait)
                        ...
                        canvas.toBlob(function(blob) {
                            ...
                            // Create new file from blob with JPEG type to match validation
                            const safeFileName = file.name.replace(/\.[^/.]+$/, '') + '.jpg';
                            let croppedFile = null;
                            try {
                                croppedFile = new File([blob], safeFileName, {
                                    type: 'image/jpeg',
                                    lastModified: Date.now()
                                });
                            } catch (err) {
                                // Safari may not support File constructor; fallback to Blob
                                console.warn('File constructor unsupported, falling back to Blob:', err);
                                croppedFile = blob;
                            }

                            // Try to update the native file input if supported; otherwise keep blob for submit
                            let inputUpdated = false;
                            try {
                                if (window.DataTransfer) {
                                    const dt = new DataTransfer();
                                    // Only File objects can be added; if we only have Blob, skip
                                    if (croppedFile instanceof File) {
                                        dt.items.add(croppedFile);
                                        $('#foto')[0].files = dt.files;
                                        inputUpdated = true;
                                        console.log('Files in input after update:', $('#foto')[0].files);
                                    }
                                }
                            } catch (dtErr) {
                                console.warn('Unable to assign cropped file to input via DataTransfer:', dtErr);
                            }

                            // Store for submit fallback when DataTransfer not available
                            if (!inputUpdated) {
                                croppedPhotoBlob = blob;
                                croppedPhotoName = safeFileName;
                                console.log('Stored cropped photo for submit fallback:', { name: croppedPhotoName, size: croppedPhotoBlob.size });
                            } else {
                                croppedPhotoBlob = null;
                                croppedPhotoName = null;
                            }
                            ...
                            // Clear validation errors since we have a valid file now
                            $('#foto').removeClass('is-invalid');
                            $('.text-danger').hide();

                            // Show preview after cropping - same as edit page
                            showPreview();
                        }, 'image/jpeg', 0.9);
                    } else {
                        // No cropping needed, keep original file and show preview immediately
                        console.log('No cropping needed - perfect dimensions');
                        finalImageSrc = e.target.result;
                        processMessage = `Foto sudah berukuran ${originalWidth}x${originalHeight}px (tidak perlu dipotong)`;

                        // Clear any previous validation errors
                        $('#foto').removeClass('is-invalid');
                        $('.text-danger').hide();

                        console.log('Showing preview for perfect dimensions photo');
                        showPreview();
                    }

                    function showPreview() {
                        console.log('showPreview() called with finalImageSrc length:', finalImageSrc ? finalImageSrc.length : 0);
                        console.log('Current photo element found:', $('#current-photo').length);

                        if (finalImageSrc) {
                            // Update current photo - same as edit page
                            $('#current-photo').html(`<img src="${finalImageSrc}" alt="Preview Foto" style="width: 100%; height: 100%; object-fit: cover; border-radius: 10px;">`);
                            $('#photo-btn-text').text('Ganti Foto');
                            $('#photo-status').show();

                            console.log('Preview image set successfully');
                        } else {
                            console.error('No finalImageSrc available for preview');
                        }
                    }
...
function submitFormData() {
    ...
    // If we have a cropped blob fallback (e.g., Safari), ensure it's appended
    if (croppedPhotoBlob) {
        try {
        ...
        const nameForUpload = croppedPhotoName || 'foto.jpg';
        formData.append('foto', croppedPhotoBlob, nameForUpload);
        console.log('Appended cropped photo blob to FormData:', { name: nameForUpload, size: croppedPhotoBlob.size });
    }
