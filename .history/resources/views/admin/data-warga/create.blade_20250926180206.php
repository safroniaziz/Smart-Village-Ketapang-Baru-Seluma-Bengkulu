                ...
                <div class="wizard-content">
                        ...
                        <div class="row g-5">
                            ...
                            <div class="col-md-4">
                                <div class="form-floating">
                                    ...
                                    <label for="rt_rw" style="color: #7e8299; font-weight: 400;">RT/RW <span style="color: #dc3545;">*</span></label>
                                    @error('rt_rw')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                    ...
                    <div class="wizard-actions">
                        ...
                        <div class="d-flex gap-3">
                            ...
                            <button type="button" class="btn btn-primary btn-wizard d-none" id="submit-btn">
...
function updateWizardDisplay() {
    console.log('updateWizardDisplay() called with currentStep:', currentStep);
            ...
            img.onload = function() {
                ...
                $currentPhoto.html(`
                    <img src="${finalImageSrc}" alt="Preview Foto" class="current-photo"
                         style="width: 120px; height: 160px; object-fit: cover; border-radius: 8px; border: 3px solid #E4E6EA;">
                `);
...
$('#kt_warga_form').off('submit').on('submit', function(e) {
    ...
    // Final validation - check all steps without changing currentStep
    console.log('=== FORM SUBMISSION VALIDATION START ===');
    ...
    for (let step = 1; step <= totalSteps; step++) {
        ...
        let stepValid = true;
        const stepInvalidFields = [];

        // Clear previous validation states for this step
        stepElement.find('.form-control, .form-select').removeClass('is-invalid');
        ...
        stepElement.find('input[required], select[required], textarea[required]').each(function() {
            ...
            if (!value) {
                ...
                stepValid = false;
                stepInvalidFields.push(fieldName);
            }
            ...
            if ($field.attr('name') === 'nik' && value) {
                if (value.length !== 16 || !/^\d+$/.test(value)) {
                    ...
                    $feedback.text('NIK harus 16 digit angka').show();
                    stepValid = false;
                    stepInvalidFields.push('NIK (format salah)');
                }
            }
            ...
            if ($field.attr('name') === 'no_kk' && value) {
                if (value.length !== 16 || !/^\d+$/.test(value)) {
                    ...
                    $feedback.text('No. Kartu Keluarga harus 16 digit angka').show();
                    stepValid = false;
                    stepInvalidFields.push('No. KK (format salah)');
                }
            }
            ...
            if ($field.attr('type') === 'email' && value) {
                ...
                if (!emailRegex.test(value)) {
                    ...
                    $feedback.text('Format email tidak valid').show();
                    stepValid = false;
                    stepInvalidFields.push('Email (format salah)');
                }
            }
        });

        if (!stepValid) {
            console.log(`Step ${step} is INVALID:`, stepInvalidFields);
            isFormValid = false;
            if (firstInvalidStep === 0) {
                firstInvalidStep = step;
            }
            invalidFields = invalidFields.concat(stepInvalidFields);
        } else {
            console.log(`Step ${step} is VALID`);
        }
    }

    console.log('Validation result:', { isFormValid, firstInvalidStep, invalidFields });

    if (!isFormValid) {
        // Go to first invalid step
        console.log('Form is invalid, going to step:', firstInvalidStep);
        ...
        Swal.fire({
            title: `⚠️ Data ${stepNames[firstInvalidStep]} Belum Lengkap!`,
            html: `
                <div class="text-start">
                    <p class="mb-3">Mohon lengkapi field berikut sebelum menyimpan:</p>
                    <div class="alert alert-warning text-start">
                        ${fieldsList}
                    </div>
                    <p class="text-muted small">
            ...
            // Focus on first invalid field
            const firstInvalidField = $(`#step-${firstInvalidStep}`).find('.form-control.is-invalid, .form-select.is-invalid').first();
            if (firstInvalidField.length) {
                firstInvalidField.focus();
                $('html, body').animate({
                    scrollTop: firstInvalidField.offset().top - 150
