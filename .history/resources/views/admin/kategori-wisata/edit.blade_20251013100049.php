...
@section('page-header', 'Edit Kategori Wisata')
...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Kategori Wisata: {{ $kategoriWisata->nama }}</h1>

    <form action="{{ route('admin.kategori-wisata.update', $kategoriWisata) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text"
                       id="nama"
                       name="nama"
                       value="{{ old('nama', $kategoriWisata->nama) }}"
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="deskripsi"
                          ...
                          placeholder="Deskripsi singkat tentang kategori wisata">{{ old('deskripsi', $kategoriWisata->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                        <i id="icon-preview" class="{{ old('icon', $kategoriWisata->icon) }} text-gray-400"></i>
                    </div>
                </div>
                @error('icon')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div>
                ...
                <select id="warna"
                    ...
                    <option value="blue" {{ old('warna', $kategoriWisata->warna) == 'blue' ? 'selected' : '' }}>Biru</option>
                    <option value="green" {{ old('warna', $kategoriWisata->warna) == 'green' ? 'selected' : '' }}>Hijau</option>
                    <option value="red" {{ old('warna', $kategoriWisata->warna) == 'red' ? 'selected' : '' }}>Merah</option>
                    <option value="yellow" {{ old('warna', $kategoriWisata->warna) == 'yellow' ? 'selected' : '' }}>Kuning</option>
                    <option value="purple" {{ old('warna', $kategoriWisata->warna) == 'purple' ? 'selected' : '' }}>Ungu</option>
                    <option value="pink" {{ old('warna', $kategoriWisata->warna) == 'pink' ? 'selected' : '' }}>Pink</option>
                    <option value="indigo" {{ old('warna', $kategoriWisata->warna) == 'indigo' ? 'selected' : '' }}>Indigo</option>
                    <option value="gray" {{ old('warna', $kategoriWisata->warna) == 'gray' ? 'selected' : '' }}>Abu-abu</option>
                </select>
                @error('warna')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number"
                       id="urutan"
                       name="urutan"
                       value="{{ old('urutan', $kategoriWisata->urutan) }}"
                       min="0"
            ...
            <div>
                <label class="flex items-center">
                    <input type="checkbox"
                           name="aktif"
                           value="1"
                           {{ old('aktif', $kategoriWisata->aktif) ? 'checked' : '' }}
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="flex items-center space-x-3">
                <i id="preview-icon" class="{{ old('icon', $kategoriWisata->icon) }} text-lg text-gray-600"></i>
                <span id="preview-nama" class="text-lg font-medium text-gray-900">{{ old('nama', $kategoriWisata->nama) }}</span>
                <span id="preview-badge" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ old('warna', $kategoriWisata->warna) }}-100 text-{{ old('warna', $kategoriWisata->warna) }}-800">
                    {{ ucfirst(old('warna', $kategoriWisata->warna)) }}
                </span>
            </div>
        </div>
