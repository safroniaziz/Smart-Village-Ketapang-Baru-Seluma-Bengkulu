...
<div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                ...
                <div class="space-y-4">
                    <div class="text-center">
                        <div class="text-3xl font-bold text-blue-600">{{ $kategoriWisata->jumlah_wisata }}</div>
                        <div class="text-sm text-gray-600">Total Wisata</div>
                    </div>

                    <div class="text-center">
                        <div class="text-2xl font-bold text-green-600">{{ $kategoriWisata->potensiWisata->where('aktif', true)->count() }}</div>
                        <div class="text-sm text-gray-600">Wisata Aktif</div>
                    </div>

                    <div class="text-center">
                        <div class="text-2xl font-bold text-gray-600">{{ $kategoriWisata->potensiWisata->where('aktif', false)->count() }}</div>
                        <div class="text-sm text-gray-600">Wisata Tidak Aktif</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Daftar Wisata dalam Kategori -->
    @if($kategoriWisata->potensiWisata->count() > 0)
    <div class="mt-8">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Daftar Wisata dalam Kategori Ini</h3>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white border border-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($kategoriWisata->potensiWisata as $index => $wisata)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $index + 1 }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $wisata->nama }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $wisata->lokasi }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($wisata->aktif ?? true)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('admin.potensi-wisata.show', $wisata) }}"
    ...
    <div class="mt-8 text-center py-8">
        ...
        <h3 class="text-lg font-medium text-gray-600 mb-2">Belum Ada Wisata</h3>
        <p class="text-gray-500">Kategori ini belum memiliki wisata yang terkait.</p>
    </div>
    @endif
</div>
@endsection
