                ...
                @if($pengajuan->status === 'Diajukan')
                    ...
                    <div class="mb-7">
                        ...
                        <div class="alert alert-info mb-3">
                            <small><i class="fas fa-info-circle"></i> <strong>Catatan:</strong> Untuk sementara, TTD Camat menggunakan file/gambar yang sama dengan TTD Kepala Desa.</small>
                        </div>
                        <select class="form-select jenis-ttd-camat-select" data-pengajuan-id="{{ $pengajuan->id }}">
                            ...
                            <option value="gambar" {{ ($pengajuan->jenis_ttd_camat ?? 'manual') == 'gambar' ? 'selected' : '' }}>TTD Gambar (Menggunakan file TTD Kepala Desa)</option>
                            <option value="qrcode" {{ ($pengajuan->jenis_ttd_camat ?? 'manual') == 'qrcode' ? 'selected' : '' }}>TTD QR Code (Menggunakan file TTD Kepala Desa)</option>
                        </select>
                    </div>
                @endif
                                ...
                                <div class="timeline">
                                    ...
                                    @if($pengajuan->approved_at)
                                        <div class="timeline-item">
                                            ...
                                            <div class="timeline-content mb-10 mt-n1">
                                                <div class="pe-3 mb-5">
                                                    <div class="fs-5 fw-semibold mb-2">Pengajuan Valid</div>
                                                    <div class="d-flex align-items-center mt-1 fs-6">
...
$(document).ready(function() {
    ...
    // Store initial values for TTD Camat
    $('.jenis-ttd-camat-select').each(function() {
        $(this).data('previous-value', $(this).val());
    });

    // Update jenis TTD Camat (for surat miskin) - menggunakan file yang sama dengan TTD kepala desa
    $('.jenis-ttd-camat-select').on('change', function() {
        const $select = $(this);
        const pengajuanId = $select.data('pengajuan-id');
        const jenisTtdCamat = $select.val();
        const previousValue = $select.data('previous-value');

        // Validate inputs
        if (!pengajuanId || !jenisTtdCamat || !['manual', 'gambar', 'qrcode'].includes(jenisTtdCamat)) {
            console.error('Invalid data for TTD Camat update');
            $select.val(previousValue);
            return;
        }
        ...
        console.log('Updating TTD Camat for pengajuan ID:', pengajuanId, 'to:', jenisTtdCamat);

        $.ajax({
            url: `/admin/pengajuan-surat/${pengajuanId}/jenis-ttd-camat`,
            method: 'POST',
            data: {
                jenis_ttd_camat: jenisTtdCamat,
                _method: 'PATCH',
                _token: $('meta[name="csrf-token"]').attr('content')
            },
            success: function(response) {
                // Update previous value for next change
                $select.data('previous-value', jenisTtdCamat);

                Swal.fire({
                    text: response.message || "Jenis TTD Camat berhasil diupdate! (Menggunakan file yang sama dengan TTD Kepala Desa)",
                    icon: "success",
                    buttonsStyling: false,
                    confirmButtonText: "Ok",
                    customClass: {
            ...
            error: function(xhr) {
                console.error('TTD Camat Update Error:', xhr.status, xhr.statusText);
                console.error('Response:', xhr.responseText);

                let errorMessage = "Terjadi kesalahan saat mengupdate jenis TTD Camat";
