...
function openEditBatasModal() {
    // Load existing data
    $.ajax({
        url: '/profil-desa/batas',
        type: 'GET',
        success: function(response) {
            if (response.length > 0) {
                const data = response[0]; // Get first record
                $('#batas_utara').val(data.utara || '');
                $('#batas_selatan').val(data.selatan || '');
                $('#batas_timur').val(data.timur || '');
                $('#batas_barat').val(data.barat || '');
                $('#batas_deskripsi').val(data.deskripsi || '');
                $('#batas_id').val(data.id);
            }
...
function saveBatas() {
    ...
    $.ajax({
        ...
        success: function(response) {
            Swal.fire({
                title: 'Berhasil!',
                text: 'Data batas wilayah berhasil disimpan',
                icon: 'success',
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.iklim && response.iklim.length > 0) {
                // Populate monthly climate data if available
                response.iklim.forEach(function(data) {
                    // Populate form fields based on month or data structure
                    if (data.januari_curah_hujan !== undefined) {
                        $('input[name="januari_curah_hujan"]').val(data.januari_curah_hujan || '');
                        $('input[name="januari_suhu"]').val(data.januari_suhu || '');
                        $('input[name="januari_kelembaban"]').val(data.januari_kelembaban || '');
                    }
                    if (data.februari_curah_hujan !== undefined) {
                        $('input[name="februari_curah_hujan"]').val(data.februari_curah_hujan || '');
                        $('input[name="februari_suhu"]').val(data.februari_suhu || '');
                        $('input[name="februari_kelembaban"]').val(data.februari_kelembaban || '');
                    }
                    // Continue for all 12 months...
                    const months = ['maret', 'april', 'mei', 'juni', 'juli', 'agustus', 'september', 'oktober', 'november', 'desember'];
                    months.forEach(function(month) {
                        if (data[month + '_curah_hujan'] !== undefined) {
                            $('input[name="' + month + '_curah_hujan"]').val(data[month + '_curah_hujan'] || '');
                            $('input[name="' + month + '_suhu"]').val(data[month + '_suhu'] || '');
                            $('input[name="' + month + '_kelembaban"]').val(data[month + '_kelembaban'] || '');
                        }
                    });
                });
            }
