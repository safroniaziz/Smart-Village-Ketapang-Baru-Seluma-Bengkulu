...
function editPotensiDesa(id) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#potensi_jumlah_unit').val(response.jumlah_unit);
            $('#potensi_satuan').val(response.satuan);
            $('#potensi_pengelola').val(response.pengelola);
            $('#potensi_status').val(response.status);
            console.log('is_unggulan from response:', response.is_unggulan, typeof response.is_unggulan);
            $('#potensi_is_unggulan').val(response.is_unggulan ? '1' : '0');
            $('#potensi_deskripsi').val(response.deskripsi);
            $('#potensi_peluang_pengembangan').val(response.peluang_pengembangan);
            $('#potensi_kendala').val(response.kendala);
            $('#potensiModal').modal('show');
        },
        error: function() {
            Swal.fire({
...
function savePotensi() {
    ...
    console.log('nama_potensi value:', $('#potensi_nama_potensi').val());
    console.log('kategori value:', $('#potensi_kategori').val());
    ...
    $.ajax({
        ...
        error: function(xhr) {
            ...
            // Handle success: false response from controller (status 500)
            if (xhr.responseJSON && xhr.responseJSON.success === false) {
                errorTitle = 'Error!';
                errorMessage = xhr.responseJSON.message;
            ...
            $('#errorModal .modal-body').html('<p class="text-danger">' + errorMessage + '</p>');
...
// Potensi Photo Preview Function
function previewPotensiPhoto(input) {
    if (input.files && input.files[0]) {
        const reader = new FileReader();
        reader.onload = function(e) {
            const preview = document.getElementById('potensi_photo_preview');
            const img = document.getElementById('potensi_preview_img');
