    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.iklim && response.iklim.length > 0) {
                ...
                // Note: The iklim_desa table has general climate fields, not monthly data
                // The modal structure might need to be updated to match the database schema
                // For now, we'll load any existing monthly data if the modal expects it
                
                // If the modal has monthly fields, try to populate them
                // But the database schema doesn't have these fields currently
                console.log('Iklim data loaded:', data);
                
                // Populate any existing general climate data
                if (data.jenis_iklim !== undefined) {
                    $('select[name="jenis_iklim"]').val(data.jenis_iklim || '');
                }
                if (data.suhu_min !== undefined) {
                    $('input[name="suhu_min"]').val(data.suhu_min || '');
                }
                if (data.suhu_max !== undefined) {
                    $('input[name="suhu_max"]').val(data.suhu_max || '');
                }
                if (data.kelembaban_rata !== undefined) {
                    $('input[name="kelembaban_rata"]').val(data.kelembaban_rata || '');
                }
                if (data.curah_hujan_tahunan !== undefined) {
                    $('input[name="curah_hujan_tahunan"]').val(data.curah_hujan_tahunan || '');
                }
            }
