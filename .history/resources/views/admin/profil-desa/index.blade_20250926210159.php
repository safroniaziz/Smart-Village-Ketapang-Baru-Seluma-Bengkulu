                                    ...
                                    <div class="row g-4">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select form-select-lg" name="status_desa" required>
                                                ...
                                                <option value="Desa" {{ ($monografi->status_desa ?? '') == 'Desa' ? 'selected' : '' }}>Desa</option>
...
function openEditMonografiModal() {
    // Load latest data via AJAX
    $.ajax({
        url: '{{ route("profil-desa.index") }}',
        type: 'GET',
        success: function(response) {
            // Update form values with latest data
            const parser = new DOMParser();
            const doc = parser.parseFromString(response, 'text/html');
            const monografiData = doc.querySelector('[data-monografi]');
            
            if (monografiData) {
                const data = JSON.parse(monografiData.getAttribute('data-monografi'));
                // Update form fields
                $('input[name="nama_desa"]').val(data.nama_desa || '');
                ...
                $('input[name="kode_area"]').val(data.kode_area || '');
                $('select[name="status_desa"]').val(data.status_desa || '');
                $('input[name="tahun_berdiri"]').val(data.tahun_berdiri || '');
                $('input[name="luas_wilayah"]').val(data.luas_wilayah || '');
                $('input[name="ketinggian_mdpl"]').val(data.ketinggian_mdpl || '');
                $('select[name="topografi"]').val(data.topografi || '');
                $('select[name="iklim"]').val(data.iklim || '');
                $('input[name="suhu_rata_rata"]').val(data.suhu_rata_rata || '');
                $('input[name="jarak_ke_kecamatan"]').val(data.jarak_ke_kecamatan || '');
                $('input[name="waktu_ke_kecamatan"]').val(data.waktu_ke_kecamatan || '');
                $('input[name="jarak_ke_kabupaten"]').val(data.jarak_ke_kabupaten || '');
                $('input[name="waktu_ke_kabupaten"]').val(data.waktu_ke_kabupaten || '');
                $('input[name="latitude"]').val(data.latitude || '');
                $('input[name="longitude"]').val(data.longitude || '');
                $('input[name="google_street_view_url"]').val(data.google_street_view_url || '');
