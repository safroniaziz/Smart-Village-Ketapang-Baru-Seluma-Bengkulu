...
function changeImage(imageUrl, imageNumber) {
    ...
    // Update counter
    if (counter) counter.textContent = imageNumber;
...
const images = @json($wisata->gambar);
const totalImages = images.length;

function nextImage() {
    currentImageIndex = (currentImageIndex + 1) % totalImages;
    changeImage(images[currentImageIndex], currentImageIndex + 1);
}

// Auto advance every 5 seconds
const galleryInterval = setInterval(nextImage, 5000);

// Pause auto-advance when user hovers over gallery
const galleryMain = document.getElementById('mainGallery');
if (galleryMain) {
...
function playVideo() {
    ...
    if (overlay && iframe) {
        ...
        setTimeout(() => {
            ...
            iframe.src = 'https://www.youtube.com/embed/{{ $youtubeId }}?autoplay=1&rel=0&showinfo=0&modestbranding=1&controls=1';
        }, 300);
    }
}

// Auto-play video when scrolled into view (after 2 seconds)
const videoContainer = document.getElementById('videoContainer');
if (videoContainer) {
    const observer = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting && entry.intersectionRatio > 0.5) {
                // Auto play after 2 seconds of being in view
                setTimeout(() => {
                    const overlay = document.getElementById('videoOverlay');
