...
<style>
    ...
    .spectacular-btn {
        background: linear-gradient(45deg, #2563eb, #4f46e5); /* blue-600 to indigo-600 */
        border: none;
        transition: all 0.3s ease;
    }
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            ...
            <div class="flex justify-center" data-aos="fade-up" data-aos-delay="200">
                <form method="GET" action="{{ route('pengumuman') }}" class="flex flex-col sm:flex-row gap-3">
                    ...
                    <select name="status" class="px-4 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" onchange="this.form.submit()">
                ...
                @foreach($pengumumans as $index => $pengumuman)
                    <article class="announcement-card group bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 priority-{{ strtolower($pengumuman->prioritas) }}" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                        <div class="relative overflow-hidden">
                            <div class="w-full h-64 bg-gradient-{{ strtolower($pengumuman->prioritas) }} relative">
                                @if($pengumuman->prioritas == 'Tinggi')
                                    <div class="absolute inset-0 bg-gradient-to-br from-red-400/20 to-orange-500/20"></div>
                                @endif
                                <div class="absolute inset-0 flex items-center justify-center opacity-60">
                                    <i class="fas fa-{{ $pengumuman->prioritas == 'Tinggi' ? 'exclamation-triangle' : ($pengumuman->prioritas == 'Sedang' ? 'info-circle' : 'check-circle') }} text-7xl text-white"></i>
                                </div>
                                <!-- Overlay title and meta -->
                                <div class="absolute inset-x-0 bottom-0 z-10">
                                    <div class="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/65 via-black/35 to-transparent"></div>
                                    <div class="relative px-5 py-4 backdrop-blur-[2px]">
                                        <h3 class="text-white text-lg lg:text-xl font-bold leading-snug line-clamp-2">
                                            {{ $pengumuman->judul }}
                                        </h3>
                                        <p class="mt-1 text-white/90 text-xs lg:text-sm leading-snug line-clamp-1">
                                            {{ Str::limit($pengumuman->excerpt, 90) }}
                                        </p>
                                        <div class="mt-2 flex items-center justify-between text-[11px] lg:text-xs text-white/90">
                                            <div class="flex items-center space-x-3">
                                                <span class="inline-flex items-center"><i class="fas fa-calendar-alt mr-1"></i>{{ $pengumuman->formatted_date }}</span>
                                                @if($pengumuman->tanggal_berakhir)
                                                    <span class="inline-flex items-center"><i class="fas fa-hourglass-end mr-1"></i>{{ $pengumuman->formatted_end_date }}</span>
                                                @endif
                                            </div>
                                            <span class="inline-flex items-center"><i class="fas fa-eye mr-1"></i>{{ number_format($pengumuman->views) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Simple effects only - no cursor, no floating cards, no ripples, no priority animations
