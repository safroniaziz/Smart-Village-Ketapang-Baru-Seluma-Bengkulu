...
<section id="ajukan-surat" class="section-nav py-16 bg-gray-50 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 relative" data-aos="fade-up" data-aos-delay="300">
                ...
                <div class="p-6 text-center h-full flex flex-col">
                    <div class="bg-gray-200 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4">
                        <i class="fas fa-cross text-2xl text-gray-800"></i>
                    </div>
        ...
        <div id="moreCards" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-16 mt-6 hidden">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 relative" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="p-6 text-center h-full flex flex-col">
                    <div class="bg-pink-100 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4">
                        <i class="fas fa-rings-wedding text-2xl text-pink-600"></i>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // List surat resmi Desa Ketapang Baru
    const allSuratTypes = [
        { id: 'surat_kehilangan', name: 'Surat Kehilangan', desc: 'Keterangan kehilangan dokumen/barang', icon: 'fas fa-search', color: 'red', hasTemplate: true },
        { id: 'surat_bersih_diri', name: 'Surat Keterangan Bersih Diri', desc: 'Keterangan kelakuan baik', icon: 'fas fa-shield-alt', color: 'green', hasTemplate: true },
        { id: 'sppd', name: 'SPPD (Surat Perintah Perjalanan Dinas)', desc: 'Perintah perjalanan dinas', icon: 'fas fa-route', color: 'blue', hasTemplate: false },
        { id: 'izin_keramaian', name: 'Surat Izin Keramaian', desc: 'Izin untuk acara/keramaian', icon: 'fas fa-calendar-alt', color: 'purple', hasTemplate: false },
        { id: 'ket_belum_menikah', name: 'Surat Keterangan Belum Menikah', desc: 'Keterangan status belum menikah', icon: 'fas fa-user', color: 'gray', hasTemplate: false },
        { id: 'ket_berkelakuan_baik', name: 'Surat Keterangan Berkelakuan Baik', desc: 'Keterangan kelakuan baik', icon: 'fas fa-user-check', color: 'green', hasTemplate: false },
        { id: 'ket_domisili', name: 'Surat Keterangan Domisili', desc: 'Keterangan tempat tinggal', icon: 'fas fa-home', color: 'blue', hasTemplate: false },
        { id: 'ket_kematian', name: 'Surat Keterangan Kematian', desc: 'Keterangan kematian penduduk', icon: 'fas fa-cross', color: 'black', hasTemplate: false },
        { id: 'ket_menikah', name: 'Surat Keterangan Menikah', desc: 'Keterangan status menikah', icon: 'fas fa-heart', color: 'pink', hasTemplate: false },
        { id: 'ket_miskin_dtks', name: 'Surat Keterangan Miskin DTKS', desc: 'Keterangan keluarga miskin DTKS', icon: 'fas fa-hand-holding-heart', color: 'red', hasTemplate: false },
        { id: 'ket_penduduk_desa', name: 'Surat Keterangan Penduduk Desa', desc: 'Keterangan sebagai penduduk desa', icon: 'fas fa-users', color: 'blue', hasTemplate: false },
        { id: 'ket_penghasilan_ortu', name: 'Surat Keterangan Penghasilan Orang Tua', desc: 'Keterangan penghasilan orang tua', icon: 'fas fa-money-bill', color: 'yellow', hasTemplate: false },
        { id: 'ket_usaha', name: 'Surat Keterangan Usaha', desc: 'Keterangan memiliki usaha', icon: 'fas fa-store', color: 'orange', hasTemplate: false },
        { id: 'pengantar_nikah', name: 'Surat Pengantar Nikah (N1-N4)', desc: 'Pengantar untuk pernikahan', icon: 'fas fa-rings-wedding', color: 'pink', hasTemplate: false },
        { id: 'perjanjian_perdamaian', name: 'Surat Perjanjian Perdamaian', desc: 'Perjanjian penyelesaian masalah', icon: 'fas fa-handshake', color: 'green', hasTemplate: false },
        { id: 'surat_pindah', name: 'Surat Pindah', desc: 'Keterangan pindah domisili', icon: 'fas fa-truck-moving', color: 'blue', hasTemplate: false },
        { id: 'surat_rekomendasi', name: 'Surat Rekomendasi', desc: 'Rekomendasi untuk berbagai keperluan', icon: 'fas fa-thumbs-up', color: 'green', hasTemplate: false },
        { id: 'surat_undangan', name: 'Surat Undangan', desc: 'Undangan kegiatan desa', icon: 'fas fa-envelope', color: 'purple', hasTemplate: false }
    ];

    // Render kartu surat
    function renderSuratCards() {
        const suratGrid = document.getElementById('suratGrid');
        const showMoreContainer = document.getElementById('showMoreContainer');
        const isLoggedIn = {{ auth()->check() ? 'true' : 'false' }};

        let html = '';
        const initialShow = 8; // Tampilkan 8 surat pertama
        const toShow = allSuratTypes.slice(0, initialShow);
        const remaining = allSuratTypes.length - initialShow;

        toShow.forEach((surat, index) => {
            const colorClasses = getColorClasses(surat.color);
            const templateBadge = surat.hasTemplate ?
                '<div class="absolute top-3 right-3 bg-green-500 text-white text-xs px-2 py-1 rounded-full font-semibold">✓ Ready</div>' :
                '<div class="absolute top-3 right-3 bg-orange-500 text-white text-xs px-2 py-1 rounded-full font-semibold">Soon</div>';

            html += `
                <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 relative" data-aos="fade-up" data-aos-delay="${(index % 4) * 100}">
                    ${templateBadge}
                    <div class="p-6 text-center h-full flex flex-col">
                        <div class="${colorClasses.bg} rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4">
                            <i class="${surat.icon} text-2xl ${colorClasses.text}"></i>
                        </div>
                        <h3 class="text-lg font-bold text-gray-900 mb-3">${surat.name}</h3>
                        <p class="text-gray-600 mb-4 flex-grow text-sm">${surat.desc}</p>
                        ${isLoggedIn ? `
                            <button onclick="openSuratModal('${surat.id}', '${surat.name}')" class="${colorClasses.btn} text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-300 mt-auto text-sm ${surat.hasTemplate ? '' : 'opacity-75 cursor-not-allowed'}" ${!surat.hasTemplate ? 'disabled' : ''}>
                                ${surat.hasTemplate ? 'Ajukan Surat' : 'Segera Hadir'}
                            </button>
                        ` : `
                            <button onclick="showLoginModal()" class="bg-gray-400 text-gray-200 font-semibold py-3 px-4 rounded-lg cursor-not-allowed mt-auto text-sm">Login untuk Ajukan</button>
                        `}
    ...
    function getColorClasses(color) {
        const colorMap = {
            ...
            'yellow': { bg: 'bg-yellow-100', text: 'text-yellow-600', btn: 'bg-yellow-600 hover:bg-yellow-700' },
            ...
            'gray': { bg: 'bg-gray-100', text: 'text-gray-600', btn: 'bg-gray-600 hover:bg-gray-700' },
            'black': { bg: 'bg-gray-200', text: 'text-gray-800', btn: 'bg-gray-800 hover:bg-black' }
        };
    ...
    document.getElementById('showMoreBtn')?.addEventListener('click', function() {
        ...
        let html = '';
        allSuratTypes.forEach((surat, index) => {
            const colorClasses = getColorClasses(surat.color);
            const templateBadge = surat.hasTemplate ?
                '<div class="absolute top-3 right-3 bg-green-500 text-white text-xs px-2 py-1 rounded-full font-semibold">✓ Ready</div>' :
                '<div class="absolute top-3 right-3 bg-orange-500 text-white text-xs px-2 py-1 rounded-full font-semibold">Soon</div>';
            ...
            html += `
                <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 relative" data-aos="fade-up" data-aos-delay="${(index % 4) * 50}">
                    ${templateBadge}
                    <div class="p-6 text-center h-full flex flex-col">
                        <div class="${colorClasses.bg} rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4">
                            <i class="${surat.icon} text-2xl ${colorClasses.text}"></i>
