...
<style>
    ...
    /* Match news-card behavior */
    .announcement-card .relative {
        position: relative;
        z-index: 2;
    }
        ...
        @if($pengumumans->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($pengumumans as $index => $pengumuman)
                    <article class="announcement-card group bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-2" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                        <div class="relative overflow-hidden">
                            @if($pengumuman->gambar)
                                <img src="{{ asset('storage/' . $pengumuman->gambar) }}" alt="{{ $pengumuman->judul }}" class="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-500">
                            @else
                                <div class="w-full h-64 bg-gradient-to-br from-blue-500 to-indigo-600"></div>
                            @endif

                            @if($pengumuman->prioritas === 'Tinggi')
                                <div class="absolute top-4 left-4 z-10">
                                    <span class="priority-badge-tinggi text-white px-3 py-1 rounded-full text-xs font-semibold">
                            ...
                            <!-- Title Overlay on media (match berita) -->
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <a href="{{ route('pengumuman.show', $pengumuman->slug) }}" class="spectacular-btn text-white px-4 py-2 rounded-lg text-sm font-semibold">
