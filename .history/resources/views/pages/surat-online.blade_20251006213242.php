...
// Form section validation dan handling
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (suratForm) {
        suratForm.addEventListener('submit', function(e) {
            ...
            // Validasi form
            const requiredFields = ['nama_lengkap', 'no_hp', 'alamat', 'jenis_surat', 'keperluan'];
            let isValid = true;
            ...
            // Validasi NIK (16 digit, optional)
            const nikInput = document.getElementById('nik');
            if (nikInput && nikInput.value && (nikInput.value.length !== 16 || !/^\d+$/.test(nikInput.value))) {
                isValid = false;
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showSuccessNotification('Pengajuan surat berhasil dikirim!', 
                        'Anda akan mendapat notifikasi WhatsApp ketika surat siap atau jika ada yang perlu diperbaiki.');
                    
                    // Reset form
                    this.reset();
                } else {
                    alert(data.message || 'Terjadi kesalahan saat mengirim pengajuan');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert('Terjadi kesalahan saat mengirim pengajuan. Pastikan koneksi internet Anda stabil.');
            })
            .finally(() => {
                // Reset button state
                submitBtn.innerHTML = originalText;
...
function showSuccessNotification(title, message) {
    const notification = document.createElement('div');
    notification.className = 'fixed top-4 right-4 bg-white border border-green-200 rounded-xl shadow-xl p-6 z-50 max-w-sm';
    notification.innerHTML = `
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center">
                ...
                <h4 class="text-lg font-semibold text-green-900">${title}</h4>
            </div>
            <button onclick="this.parentElement.parentElement.remove()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times"></i>
            </button>
        ...
        <div class="bg-blue-50 rounded-lg p-3">
            <div class="flex items-start">
                <i class="fas fa-info-circle text-blue-600 mr-2 mt-0.5 text-sm"></i>
                <div class="text-xs text-blue-800">
                    <p><strong>Status:</strong> Menunggu validasi admin</p>
                    <p><strong>Estimasi:</strong> 1-2 hari kerja</p>
                </div>
            </div>
        </div>
    `;
