...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('particles-berita')) {
        particlesJS('particles-berita', {
            particles: {
                ...
                color: { value: ["#ffffff", "#ffd700", "#00ffff", "#ff69b4"] },
                shape: {
                ...
                size: {
                    value: 5,
                    random: true,
                    anim: { enable: true, speed: 8, size_min: 0.5, sync: false }
                },
                line_linked: {
                    enable: true,
                    distance: 200,
            ...
            interactivity: {
                ...
                modes: {
                    grab: { distance: 500, line_linked: { opacity: 0.8 } },
                    bubble: { distance: 300, size: 15, duration: 3, opacity: 1, speed: 4 },
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.classList.add('spectacular-entrance');
    ...
    document.querySelectorAll('.news-card, .spectacular-btn').forEach(element => {
    ...
    const colors = ['#667eea', '#764ba2', '#f093fb', '#f5576c', '#48dbfb', '#feca57'];
    let colorIndex = 0;

    setInterval(() => {
        colorIndex = (colorIndex + 1) % colors.length;
        document.documentElement.style.setProperty('--glow-primary', colors[colorIndex] + '40');
        cursor.style.background = `radial-gradient(circle, ${colors[colorIndex]}, ${colors[(colorIndex + 1) % colors.length]})`;
    }, 3000);
