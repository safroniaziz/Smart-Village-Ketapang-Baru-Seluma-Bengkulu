...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (complaintForm) {
        ...
        complaintForm.addEventListener('submit', function(e) {
            ...
            .then(async (response) => {
                ...
                // Success
                if (data.success) {
                    // Tampilkan panel sukses dan nomor tracking di halaman
                    complaintForm.reset();
    ...
    // Auto fill tracking from URL (?no=...) and fetch status
    try {
        const url = new URL(window.location.href);
        ...
        if (no) {
            ...
            if (input) {
                input.value = no;
                // If already on #cek-status, fetch immediately
                if (url.hash === '#cek-status') {
                    checkStatus();
                }
            }
        }
