...
<style>
    ...
    .pdf-viewer-wrapper.loading::before {
        opacity: 1;
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.openPdfModal = function() {
            ...
            if (modal) {
                ...
                // Handle PDF.js loading states
                const pdfWrapper = modal.querySelector('.pdf-viewer-wrapper');
                const pdfFrame = modal.querySelector('.pdf-viewer-frame');
                ...
                // Set up load handler for PDF.js iframe
                if (pdfFrame) {
                    pdfFrame.onload = function() {
                        setTimeout(() => {
                            pdfFrame.classList.add('loaded');
                            pdfWrapper?.classList.remove('loading');
                        }, 1000); // Give PDF.js time to initialize
                    };

                    pdfFrame.onerror = function() {
                        console.warn('PDF.js failed to load, fallback will be shown');
                        pdfWrapper?.classList.remove('loading');
                    };
                }

                console.log('PDF modal opened with PDF.js viewer');
            }
        };
