...
#map { height: 500px; }
.leaflet-popup-content img { max-width: 220px; height: auto; border-radius: 8px; display: block; }
</style>
@endpush
...
function loadMap() {
    ...
    const map = L.map('map').setView([-4.314632, 102.763257], 14);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '&copy; OpenStreetMap contributors'
    }).addTo(map);

    // Data from backend (expects array of { nik, nama_lengkap, lat, long })
    ...
    (realData || []).forEach(item => {
        ...
        const imgUrl = nik && nik !== '-' ? `${imgBase}/${nik}.jpg` : `${imgBase}/placeholder.jpg`;
        ...
        const popupHtml = `
            <div>
                ...
                <div style="margin-top:8px">
                    <img src="${imgUrl}" alt="${nik}" onerror="this.style.display='none'"/>
                </div>
            </div>
        `;
