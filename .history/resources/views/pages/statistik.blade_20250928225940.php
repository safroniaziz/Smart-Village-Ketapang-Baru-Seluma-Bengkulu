        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            ...
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl" data-aos="fade-left" data-aos-duration="800" data-aos-delay="200">
                ...
                <div class="space-y-4 max-h-96 overflow-y-auto custom-scrollbar">
                    @forelse($programDetails as $program)
                    <div class="bg-gradient-to-r {{ $program['colors']['bg'] }} rounded-xl p-4 border {{ $program['colors']['border'] }} hover:shadow-lg transition-all duration-300">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-gradient-to-br {{ $program['colors']['gradient'] }} rounded-lg flex items-center justify-center">
                                    <i class="{{ $program['icon'] }} text-white text-sm"></i>
                                </div>
                                <div>
                                    <h4 class="font-bold {{ $program['colors']['text'] }}">{{ $program['program'] }}</h4>
                                    <p class="text-sm {{ $program['colors']['subtext'] }}">{{ $program['description'] }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="text-lg font-black {{ $program['colors']['amount'] }}">{{ number_format($program['unique_recipients']) }} KK</div>
                                <div class="text-xs {{ $program['colors']['subamount'] }}">Rp {{ $program['amount'] }}/bulan</div>
                            </div>
                        </div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-xs {{ $program['colors']['subtext'] }}">Cakupan: {{ $program['percentage'] }}%</span>
                            <span class="text-xs {{ $program['colors']['subtext'] }}">{{ number_format($program['total_recipients']) }} total penerima</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-gradient-to-r {{ $program['colors']['bar'] }} h-2 rounded-full transition-all duration-1000" style="width: {{ $program['percentage'] }}%"></div>
                        </div>
                    </div>
                    @empty
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-500">Belum ada data program bantuan</p>
                    </div>
                    @endforelse
                </div>
            </div>
