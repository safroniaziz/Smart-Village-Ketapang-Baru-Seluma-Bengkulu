    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div id="form-pengaduan" class="section-nav bg-gradient-to-br from-white via-blue-50/30 to-purple-50/30 rounded-3xl shadow-2xl border border-white/50 overflow-hidden" data-aos="fade-up" data-aos-delay="200">
            <div class="p-8 lg:p-12" data-aos="fade-up" data-aos-delay="300">
                ...
                <!-- Inline Alert for NIK Check -->
                <div id="nikAlert" class="hidden mb-6 rounded-xl border p-4"></div>

                <form id="complaintForm" class="space-y-8" method="POST" action="{{ route('pengaduan.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            <label for="no_wa" class="block text-sm font-semibold text-gray-700">Nomor WhatsApp *</label>
                            <input type="tel"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 bg-white"
                                   id="no_wa"
                                   name="no_wa"
                            ...
                            <p class="text-sm text-gray-600 mt-1">
                                ...
                                Nomor WhatsApp diperlukan untuk menerima notifikasi tindak lanjut pengaduan
                            </p>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        Swal.fire({
            ...
            .then(async (response) => {
                ...
                if (data.success) {
                    ...
                    Swal.fire({
                        title: 'Berhasil!',
                        text: data.message || 'Pengaduan berhasil dikirim!',
                        icon: 'success',
                        ...
                        if (data.tracking_number) {
                            Swal.fire({
                                ...
                                html: `
                                    <div class="text-center">
                                        <p class="text-lg mb-4">Simpan nomor tracking untuk mengecek status pengaduan:</p>
                                        <div class="bg-gray-100 p-4 rounded-lg mb-4">
                                            <code class="text-2xl font-bold text-blue-600">${data.tracking_number}</code>
                                        </div>
                                        <div class="mb-4">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${data.asal_pelapor === 'Warga Desa' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}">${data.asal_pelapor || 'Eksternal'}</span>
                                        </div>
                                        <button onclick="navigator.clipboard.writeText('${data.tracking_number}'); Swal.fire('Copied!', 'Nomor tracking disalin', 'success');"
    ...
    function setNikAlert(type, html) {
        ...
        nikAlert.className = base + ' ' + (map[type] || map.info);
        nikAlert.innerHTML = html;
        nikAlert.classList.remove('hidden');
    }

    if (nikInput && nikAlert) {
        nikInput.addEventListener('blur', () => {
            ...
            fetch(`{{ route('pengaduan.cekNik', ['nik' => '___NIK___']) }}`.replace('___NIK___', encodeURIComponent(val)), {
                headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }
            })
                .then(r => r.json())
                .then(j => {
                    if (!j.success) {
                        setNikAlert('error', '<div class="flex items-center gap-3"><i class="fas fa-times-circle"></i><span>Gagal memeriksa NIK</span></div>');
                        return;
                    }
                    if (j.is_warga) {
                        setNikAlert('success', `<div class="flex items-center gap-3"><i class="fas fa-check-circle"></i><span>Anda terdaftar sebagai warga desa atas nama <b>${j.nama}</b>${j.dusun ? ` (Dusun ${j.dusun})` : ''}.</span></div>`);
                    } else {
                        setNikAlert('info', '<div class="flex items-center gap-3"><i class="fas fa-info-circle"></i><span>Anda tidak terdaftar sebagai warga desa (status: Eksternal).</span></div>');
                    }
                })
                .catch(() => setNikAlert('error', '<div class="flex items-center gap-3"><i class="fas fa-times-circle"></i><span>Terjadi kesalahan saat memeriksa NIK.</span></div>'));
        });
    }
    // Form validation
...
function checkStatus() {
    ...
    if (!trackingNumber) {
        ...
        Swal.fire({
            title: 'Warning!',
            text: 'Masukkan nomor tracking pengaduan!',
            icon: 'warning',
            confirmButtonColor: '#f59e0b',
            confirmButtonText: 'OK'
        });
    ...
    Swal.fire({
        title: 'Mencari...',
        text: 'Sedang mencari status pengaduan',
        icon: 'info',
        allowOutsideClick: false,
    ...
    fetch(`/pengaduan/status/${encodeURIComponent(trackingNumber)}`, {
        ...
        .then(json => {
            if (!json.success) {
                Swal.fire({
                    ...
                    text: json.message || 'Pengaduan tidak ditemukan',
                    icon: 'error',
                    confirmButtonColor: '#ef4444',
                    confirmButtonText: 'OK'
                });
            ...
            // Asal pelapor badge (optional if container exists)
            const originBadge = document.getElementById('originBadge');
            if (originBadge) {
                originBadge.textContent = data.asal_pelapor || 'Eksternal';
                originBadge.className = 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ' + (data.is_warga ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800');
            }
        ...
        .catch(err => {
            ...
            Swal.fire({
                title: 'Error!',
                text: 'Gagal mengambil status pengaduan',
                icon: 'error',
                confirmButtonColor: '#ef4444',
                confirmButtonText: 'OK'
            });
