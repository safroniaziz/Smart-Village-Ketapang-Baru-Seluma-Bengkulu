        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl" data-aos="fade-up" data-aos-duration="800" data-aos-delay="400">
                <div class="text-center mb-8">
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mb-2">Distribusi Bantuan Per Dusun</h3>
                    <p class="text-gray-600">Sebaran penerima bantuan berdasarkan wilayah dusun</p>
                </div>
                ...
                <div class="space-y-4">
                    @foreach($dusunStats as $dusun => $count)
                        @php
                            ...
                            $colors = ['orange', 'red', 'amber', 'yellow', 'pink', 'rose'];
                            $color = $colors[array_search($dusun, array_keys($dusunStats)) % count($colors)];
                        @endphp
                        <div class="bg-gradient-to-br from-{{ $color }}-50 to-{{ $color }}-100 rounded-xl p-6 border border-{{ $color }}-200 hover:shadow-lg transition-all duration-300">
                            <div class="flex items-center justify-between mb-4">
                                    <div class="flex items-center">
                                    <div class="w-10 h-10 bg-gradient-to-br from-{{ $color }}-500 to-{{ $color }}-600 rounded-lg flex items-center justify-center mr-3">
                                        <i class="fas fa-home text-white text-sm"></i>
                                        </div>
                                    <h4 class="font-bold text-{{ $color }}-900">{{ $dusun }}</h4>
                                    </div>
                                <div class="text-right">
                                    <div class="text-lg font-bold text-{{ $color }}-600">{{ $aidRecipients }} KK</div>
                                    <div class="text-sm text-{{ $color }}-700">{{ $aidPercentage }}% dari populasi</div>
                                </div>
                                        </div>
                            <div class="w-full bg-{{ $color }}-200 rounded-full h-2">
                                <div class="bg-gradient-to-r from-{{ $color }}-500 to-{{ $color }}-600 h-2 rounded-full transition-all duration-1000" style="width: {{ $aidPercentage }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const chartConfigs = {
        ...
        dusunPopulation: {
            type: 'doughnut',
            data: {
                labels: {!! json_encode(empty($dusunStats) ? ['Belum Ada Data'] : array_keys($dusunStats)) !!},
                datasets: [{
                    label: 'Populasi Per Dusun',
                    data: {!! json_encode(empty($dusunStats) ? [1] : array_values($dusunStats)) !!},
                    backgroundColor: [
                        'rgba(59, 130, 246, 0.8)',
                        'rgba(16, 185, 129, 0.8)',
                        'rgba(139, 92, 246, 0.8)',
        ...
        if (canvas) {
            ...
            setTimeout(() => {
                try {
                    ...
                    if (chartConfigs[chartName] && chartConfigs[chartName].data) {
                ...
                setTimeout(() => {
                    ...
                    if (loadingEl) loadingEl.innerHTML = '<i class="fas fa-exclamation-triangle mr-2"></i>Error loading chart';
                }
            }, 300);
        }
    });
