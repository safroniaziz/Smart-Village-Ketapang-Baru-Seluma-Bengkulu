            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.pengajuan_id) {
                        ...
                        setTimeout(() => {
                            showStatusNotification(data.pengajuan_id, data.tracking_number, data.message);
                        }, 500);
                    } else {
...
function showStatusNotification(pengajuanId, trackingNumber = null, successMessage = null) {
    console.log('showStatusNotification called:', {pengajuanId, trackingNumber, successMessage});
    const notification = document.createElement('div');
    notification.className = 'fixed top-4 right-4 bg-white border border-blue-200 rounded-xl shadow-xl p-6 z-50 max-w-sm animate-slide-in';
    notification.innerHTML = `
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center space-x-2">
                ...
                <h4 class="text-lg font-semibold text-green-900">${successMessage || 'Pengajuan Berhasil Dikirim!'}</h4>
            </div>
        ...
        <div class="bg-green-50 rounded-lg p-3 mb-4 border border-green-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="font-mono font-bold text-green-800 text-lg">${trackingNumber}</p>
                </div>
                <button onclick="copyToClipboard('${trackingNumber}')" class="px-3 py-1 bg-green-100 hover:bg-green-200 text-green-700 text-xs rounded-lg transition-colors">
            ...
            <div class="flex items-start space-x-3">
                ...
                <div class="flex-1">
                    <div class="flex items-center space-x-2 mb-2">
                        <h4 class="font-semibold text-gray-800">📱 Notifikasi WhatsApp</h4>
                    ...
                    <div class="bg-white/70 backdrop-blur-sm rounded-lg p-3 mb-3 border border-green-100">
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="font-semibold text-green-700">${userData.no_hp}</span>
                        </div>
                    </div>
