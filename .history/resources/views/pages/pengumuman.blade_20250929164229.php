...
<style>
    ...
    /* Announcement Cards - Simple like statistik */
    .announcement-card {
...
@if($urgentPengumuman)
<section class="pt-0 pb-20 bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 relative overflow-hidden">
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize particles for pengumuman page (sama dengan statistik)
    ...
    // 🌟 SPECTACULAR PRIORITY-BASED EFFECTS 🌟
    const priorityEffects = {
        tinggi: ['#dc3545', '#fd7e14', '#ffc107'],
        sedang: ['#ffc107', '#fd7e14', '#20c997'],
        rendah: ['#20c997', '#17a2b8', '#007bff']
    };

    // Apply dynamic colors based on priority
    document.querySelectorAll('.announcement-card').forEach(card => {
        const priority = card.classList.contains('priority-tinggi') ? 'tinggi' :
                        card.classList.contains('priority-sedang') ? 'sedang' : 'rendah';

        card.addEventListener('mouseenter', () => {
            const colors = priorityEffects[priority];
            const randomColor = colors[Math.floor(Math.random() * colors.length)];
            card.style.setProperty('--glow-color', randomColor + '60');
        });
    });

    // 🎭 SPECTACULAR SCROLL EFFECTS 🎭
    let ticking = false;
    function updateScrollEffects() {
        const scrolled = window.pageYOffset;
        const rate = scrolled * -0.3;

        // Parallax effect on hero - DISABLED to prevent white gap
        // const hero = document.querySelector('.hero-section');
        // if (hero) {
        //     hero.style.transform = `translateY(${rate}px)`;
        // }

        // Floating cards effect with priority-based movement
        const cards = document.querySelectorAll('.announcement-card');
        cards.forEach((card, index) => {
            let multiplier = 0.08;
            if (card.classList.contains('priority-tinggi')) multiplier = 0.12;
            else if (card.classList.contains('priority-sedang')) multiplier = 0.10;

            const cardRate = scrolled * (multiplier + index * 0.01);
    ...
    // 🎪 SPECTACULAR MOUSE CURSOR EFFECTS 🎪
    ...
    // Scale cursor based on priority
    document.querySelectorAll('.announcement-card').forEach(element => {
        element.addEventListener('mouseenter', () => {
            if (element.classList.contains('priority-tinggi')) {
                cursor.style.transform = 'scale(3)';
                cursor.style.background = 'radial-gradient(circle, #dc3545, #fd7e14)';
            } else if (element.classList.contains('priority-sedang')) {
                cursor.style.transform = 'scale(2.5)';
                cursor.style.background = 'radial-gradient(circle, #ffc107, #fd7e14)';
            } else {
                cursor.style.transform = 'scale(2)';
                cursor.style.background = 'radial-gradient(circle, #20c997, #17a2b8)';
            }
        });
        element.addEventListener('mouseleave', () => {
            cursor.style.transform = 'scale(1)';
            cursor.style.background = 'radial-gradient(circle, #dc3545, #ffc107)';
        });
    });

    // 🌈 SPECTACULAR COLOR CHANGING EFFECTS 🌈
    const priorityColors = ['#dc3545', '#ffc107', '#20c997', '#17a2b8', '#007bff'];
    let colorIndex = 0;

    setInterval(() => {
        colorIndex = (colorIndex + 1) % priorityColors.length;
        document.documentElement.style.setProperty('--glow-primary', priorityColors[colorIndex] + '40');
        document.documentElement.style.setProperty('--glow-warning', priorityColors[(colorIndex + 1) % priorityColors.length] + '60');
        document.documentElement.style.setProperty('--glow-danger', priorityColors[(colorIndex + 2) % priorityColors.length] + '60');
    }, 2000);

    // 🎯 SPECTACULAR CLICK RIPPLE EFFECT 🎯
    document.addEventListener('click', function(e) {
        const ripple = document.createElement('div');

        // Different ripple colors based on clicked element
        let rippleColor = 'rgba(102, 126, 234, 0.6)';
        if (e.target.closest('.priority-tinggi')) {
            rippleColor = 'rgba(220, 53, 69, 0.8)';
        } else if (e.target.closest('.priority-sedang')) {
            rippleColor = 'rgba(255, 193, 7, 0.8)';
        } else if (e.target.closest('.priority-rendah')) {
            rippleColor = 'rgba(32, 201, 151, 0.8)';
        }
    ...
    // 🎊 SPECTACULAR PRIORITY ALERTS 🎊
    const urgentCards = document.querySelectorAll('.priority-tinggi');
    urgentCards.forEach((card, index) => {
        setTimeout(() => {
            card.style.animation = 'spectacularPulse 3s infinite';
        }, index * 500);
    });
    ...
    style.textContent = `
        ...
        .priority-tinggi .priority-badge-tinggi {
    ...
    console.log('🚀 SPECTACULAR PENGUMUMAN PAGE LOADED! 🚀');
});
</script>
@endpush
