...
<section class="py-20 bg-white">
    <div class="w-[80%] max-w-none mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" data-aos="fade-up" data-aos-duration="600">
            @php
                $colors = [
                    'green' => ['from-green-50', 'to-emerald-100', 'border-green-200', 'hover:border-green-300', 'from-green-500', 'to-green-600', 'text-green-700', 'bg-green-100'],
                    'yellow' => ['from-yellow-50', 'to-orange-100', 'border-yellow-200', 'hover:border-yellow-300', 'from-yellow-500', 'to-orange-500', 'text-yellow-700', 'bg-yellow-100'],
                    'cyan' => ['from-cyan-50', 'to-blue-100', 'border-cyan-200', 'hover:border-cyan-300', 'from-cyan-500', 'to-blue-500', 'text-cyan-700', 'bg-cyan-100'],
                    'purple' => ['from-purple-50', 'to-indigo-100', 'border-purple-200', 'hover:border-purple-300', 'from-purple-500', 'to-indigo-500', 'text-purple-700', 'bg-purple-100'],
                    'emerald' => ['from-emerald-50', 'to-green-100', 'border-emerald-200', 'hover:border-emerald-300', 'from-emerald-500', 'to-green-500', 'text-emerald-700', 'bg-emerald-100'],
                    'red' => ['from-red-50', 'to-pink-100', 'border-red-200', 'hover:border-red-300', 'from-red-500', 'to-pink-500', 'text-red-700', 'bg-red-100'],
                ];
                $colorKeys = array_keys($colors);
            @endphp

            @foreach($peruntukanLahan->where('is_active', true)->sortBy('urutan') as $index => $lahan)
                @php
                    $colorKey = $colorKeys[$index % count($colorKeys)];
                    $color = $colors[$colorKey];
                    $totalLuas = $peruntukanLahan->where('kategori', $lahan->kategori)->sum('luas_hektar');
                @endphp
                
                <div class="group bg-gradient-to-br {{ $color[0] }} {{ $color[1] }} rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 border {{ $color[2] }} {{ $color[3] }}">
                    <div class="w-16 h-16 bg-gradient-to-br {{ $color[4] }} {{ $color[5] }} rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                        <i class="{{ $lahan->icon ?? 'fas fa-layer-group' }} text-white text-2xl"></i>
                    </div>
                    <h3 class="text-xl font-bold text-gray-900 mb-3">{{ $lahan->kategori }}</h3>
                    
                    @if($totalLuas > 0)
                        <div class="{{ $color[7] }} rounded-lg p-3 mb-3">
                            <div class="text-center">
                                <div class="text-2xl font-black {{ $color[6] }}">{{ number_format($totalLuas, 0) }}</div>
                                <div class="text-sm {{ str_replace('text-', 'text-', $color[6]) }}-600">Total Hektar</div>
                            </div>
                        </div>
                    @endif

                    <div class="space-y-2 text-sm text-gray-600">
                        @foreach($peruntukanLahan->where('kategori', $lahan->kategori) as $subLahan)
                            <div class="flex justify-between {{ $subLahan->luas_hektar > 0 ? '' : 'text-xs text-gray-500' }}">
                                <span>• {{ $subLahan->sub_kategori }}</span>
                                <span class="font-mono {{ $subLahan->luas_hektar > 0 ? 'font-bold' : '' }}">
                                    {{ $subLahan->luas_hektar > 0 ? number_format($subLahan->luas_hektar, 0) . ' Ha' : '- Ha' }}
                                </span>
                            </div>
                        @endforeach

                        @if($lahan->deskripsi)
                            <div class="mt-3 p-2 bg-white/50 rounded text-xs text-gray-600">
                                {{ $lahan->deskripsi }}
                            </div>
                        @endif
                    </div>
