...
<style>
    ...
    .data-table tbody tr:nth-child(even) td {
        background: #fcfdff; /* subtle zebra */
    }
    ...
    /* Two-up layout for Pagu Earmark & BLT sections */
    .two-up-wrapper {}
    .two-up-section { display: block; }
    ...
    @media (max-width: 768px) {
        ...
        /* PDF Embed Mobile */
        #pdfFrame { height: 400px !important; }
    }

    @media (max-width: 480px) {
        .filter-btn { padding: 6px 10px; font-size: 12px; }

        /* PDF Embed Small Mobile */
        #pdfFrame { height: 300px !important; }
        #pdfEmbedContainer .bg-gradient-to-r { padding: 12px !important; }
        #pdfEmbedContainer h3 { font-size: 14px !important; }
    }
    ...
    <div class="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-0">
        <div class="flex flex-col lg:flex-row items-center justify-between gap-10 min-h-[80vh]">
            ...
            <div class="flex-1 space-y-10 relative z-10">
                <div class="space-y-8">
                    ...
                    <div class="flex items-center space-x-3 mb-6" data-aos="fade-up" data-aos-delay="200">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-blue-100">{{ $districtName ?? 'Kecamatan Semidang Alas Maras' }}</p>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between gap-4" data-aos="fade-up" data-aos-delay="900">
                    ...
                    <div class="relative">
                        <select id="yearSelector" class="bg-white/20 backdrop-blur-sm border border-white/30 rounded-full px-4 py-2 text-white font-semibold text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent cursor-pointer hover:bg-white/30 transition-all duration-300">
                            @foreach($availableYears as $year)
                                <option value="{{ $year }}" {{ $year == $tahun ? 'selected' : '' }} class="bg-blue-600 text-white">
                                    Tahun {{ $year }}
                                </option>
                            @endforeach
                        </select>
                        <i class="fas fa-calendar-alt absolute right-3 top-1/2 transform -translate-y-1/2 text-white/70 pointer-events-none"></i>
                    </div>
                </div>
            </div>
...
<section id="stats" class="py-20 relative">
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            <!-- Total Pendapatan -->
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl text-center" data-aos="fade-up" data-aos-delay="100">
            ...
            <!-- Total Belanja -->
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl text-center" data-aos="fade-up" data-aos-delay="200">
            ...
            <!-- Surplus/Defisit -->
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl text-center" data-aos="fade-up" data-aos-delay="300">
            ...
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl text-center" data-aos="fade-up" data-aos-delay="400">
        ...
        <!-- Charts + APBDes Document Section -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <!-- Right: Embedded APBDes Document -->
            @if($pendapatan->first() && $pendapatan->first()->file_apbdes)
            <div data-aos="fade-left">
                ...
                <div class="bg-white rounded-2xl shadow-xl p-4 border border-gray-200">
                    <div class="w-full" style="height: 860px;">
                        <iframe
                            src="{{ Storage::url($pendapatan->first()->file_apbdes) }}#toolbar=0&navpanes=0&scrollbar=0"
                            title="APBDes {{ $tahun }} PDF"
                            class="w-full h-full rounded-lg"
                            style="border: 0;"
                            loading="lazy"
                            onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        </iframe>
                        <div class="w-full h-full flex items-center justify-center bg-gray-100 rounded-lg" style="display: none;">
                            <div class="text-center text-gray-500">
                                <i class="fas fa-exclamation-triangle text-4xl mb-4"></i>
                                <p>Dokumen tidak dapat dimuat</p>
                                <a href="{{ Storage::url($pendapatan->first()->file_apbdes) }}" target="_blank" class="text-blue-600 hover:underline">
                    ...
                    <div class="text-center mt-3">
                        <a href="{{ Storage::url($pendapatan->first()->file_apbdes) }}" target="_blank"
                           class="inline-flex items-center px-4 py-2 text-sm font-semibold text-blue-700 hover:text-blue-900 transition-colors">
            ...
            <div data-aos="fade-left">
                ...
                <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-200 text-center">
                    <div class="text-gray-500">
                        ...
                        <p class="text-lg font-semibold mb-2">Dokumen APBDes Tidak Tersedia</p>
                        <p>Dokumen APBDes untuk tahun {{ $tahun }} sedang dalam proses atau belum diunggah.</p>
                    </div>
                </div>
            </div>
            @endif
...
<section class="py-20 bg-gradient-to-br from-gray-50 to-white">
    <div class="container mx-auto px-4">
        <div class="text-center mb-16" data-aos="fade-up">
            <!-- Badge -->
            <div class="inline-flex items-center bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-3 rounded-full text-sm font-semibold shadow-lg mb-6">
                <i class="fas fa-file-invoice-dollar mr-2 text-yellow-300"></i>
                Detail APBDes {{ $tahun }}
            </div>

            <h2 class="section-title">
                <span class="text-green-600">Anggaran Pendapatan</span> & Belanja Desa
            </h2>
            <div class="section-title-underline"></div>
            <p class="section-subtitle">
                <span class="font-semibold text-slate-700">Rincian lengkap</span> <span class="font-semibold text-green-600">pendapatan dan belanja desa</span>
                dengan <span class="font-semibold text-green-700">transparansi penuh</span> untuk akuntabilitas keuangan
            </p>

            <!-- PDF Embed Button -->
            @if($pendapatan->first() && $pendapatan->first()->file_apbdes)
            <div class="mt-6">
                <button onclick="togglePdfEmbed()"
                        class="inline-flex items-center bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-semibold px-6 py-3 rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300"
                        data-aos="zoom-in" data-aos-delay="200">
                    <i class="fas fa-file-pdf mr-2"></i>
                    <span id="pdfToggleText">Lihat APBDes {{ $tahun }} (PDF)</span>
                </button>
            </div>
            @endif
        </div>
        ...
        <!-- PDF Embed Container -->
        @if($pendapatan->first() && $pendapatan->first()->file_apbdes)
        <div id="pdfEmbedContainer" class="hidden mb-12" data-aos="fade-up">
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
                <div class="bg-gradient-to-r from-red-600 to-red-700 p-4">
                    <div class="flex items-center justify-between">
                        <h3 class="text-white font-semibold flex items-center">
                            <i class="fas fa-file-pdf mr-2"></i>
                            APBDes {{ $tahun }} - Dokumen Resmi
                        </h3>
                        <div class="flex items-center gap-2">
                            <a href="{{ Storage::url($pendapatan->first()->file_apbdes) }}"
                               target="_blank"
        ...
        <div class="text-center mb-12" data-aos="fade-up" data-aos-delay="100">
            <div class="inline-flex flex-wrap justify-center gap-2 p-3 bg-white rounded-full shadow-xl border border-gray-100 max-w-md mx-auto">
                <button class="filter-btn active" data-filter="all">
                    <i class="fas fa-list mr-2"></i>Semua
                </button>
                <button class="filter-btn" data-filter="pendapatan">
                    <i class="fas fa-arrow-up mr-2 text-green-400"></i>Pendapatan
                </button>
                <button class="filter-btn" data-filter="belanja">
                    <i class="fas fa-arrow-down mr-2 text-red-400"></i>Belanja
                </button>
            </div>
        </div>
...
<section class="py-20 bg-white">
    <div class="container mx-auto px-4">
        <div class="text-center mb-16" data-aos="fade-up">
            <div class="inline-flex items-center bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-full text-sm font-semibold shadow-lg mb-6">
                <i class="fas fa-clipboard-list mr-2 text-yellow-300"></i>
                Pagu Earmark {{ $tahun }}
            </div>
            <h2 class="section-title"><span class="text-blue-600">Anggaran</span> Kegiatan Khusus</h2>
            <div class="section-title-underline"></div>
            <p class="section-subtitle">
                <span class="font-semibold text-slate-700">Anggaran yang ditentukan penggunaannya</span> <span class="font-semibold text-blue-600">untuk kegiatan spesifik</span>
                dengan <span class="font-semibold text-blue-700">alokasi dana yang terencana</span> dan terukur
            </p>
        </div>
        <div class="data-table max-w-6xl mx-auto" data-aos="fade-up">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 p-6">
                <h3 class="text-2xl font-bold text-white flex items-center"><i class="fas fa-tasks mr-3"></i>Daftar Kegiatan Pagu Earmark</h3>
                    ...
                    <tbody>
                        @foreach($paguEarmarks as $index => $pagu)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-right font-bold text-blue-600">{{ $pagu->formatted_jumlah_pagu }}</td>
                                <td class="px-6 py-4 text-gray-600">{{ $pagu->keterangan ?? '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
...
<section class="py-20 bg-gradient-to-br from-purple-50 to-white">
    <div class="container mx-auto px-4">
        <div class="text-center mb-16" data-aos="fade-up">
            <div class="inline-flex items-center bg-gradient-to-r from-purple-600 to-violet-600 text-white px-6 py-3 rounded-full text-sm font-semibold shadow-lg mb-6">
                <i class="fas fa-hand-holding-usd mr-2 text-yellow-300"></i>
                BLT Dana Desa {{ $tahun }}
            </div>
            <h2 class="section-title"><span class="text-purple-600">Bantuan Langsung</span> Tunai Desa</h2>
            <div class="section-title-underline"></div>
            <p class="section-subtitle">
                <span class="font-semibold text-slate-700">Daftar lengkap penerima BLT-DD</span> <span class="font-semibold text-purple-600">dan informasi demografinya</span>
                untuk <span class="font-semibold text-purple-700">transparansi penyaluran bantuan</span> kepada masyarakat
            </p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl text-center" data-aos="fade-up">
                <div class="stat-icon mx-auto" style="background: linear-gradient(135deg, #8b5cf6, #5b21b6);"><i class="fas fa-users"></i></div>
            ...
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl text-center" data-aos="fade-up" data-aos-delay="100">
                <div class="stat-icon mx-auto" style="background: linear-gradient(135deg, #ec4899, #be185d);"><i class="fas fa-venus"></i></div>
            ...
            <div class="stat-card bg-white rounded-3xl p-8 shadow-2xl text-center" data-aos="fade-up" data-aos-delay="200">
                <div class="stat-icon mx-auto" style="background: linear-gradient(135deg, #06b6d4, #0891b2);"><i class="fas fa-mars"></i></div>
        ...
        <div class="data-table" data-aos="fade-up">
            <div class="bg-gradient-to-r from-purple-600 to-purple-700 p-6">
                <h3 class="text-2xl font-bold text-white flex items-center"><i class="fas fa-hand-holding-usd mr-3"></i>Daftar Penerima BLT-DD</h3>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody id="blt-table-body">
                        @php $currentPage = 1; $perPage = 10; @endphp
                        @foreach($bltDds->take($perPage) as $index => $blt)
                            <tr>
                                <td class="px-6 py-4 text-center font-semibold text-gray-600">{{ $index + 1 }}</td>
                                <td class="px-6 py-4 font-semibold text-gray-800">{{ $blt->nama }}</td>
                                <td class="px-6 py-4 text-center"><span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $blt->jenis_kelamin === 'P' ? 'bg-pink-100 text-pink-800' : 'bg-blue-100 text-blue-800' }}">{{ $blt->jenis_kelamin }}</span></td>
                                <td class="px-6 py-4 text-gray-600">{{ Str::limit($blt->alamat, 30) }}</td>
                                <td class="px-6 py-4 text-gray-600 font-mono">{{ $blt->no_rekening ?? '-' }}</td>
                                <td class="px-6 py-4 text-right font-bold text-purple-600">{{ $blt->formatted_nominal_bantuan }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <!-- Pagination Controls (Server-side without URL params) -->
            @if($bltDds->count() > 10)
            <div class="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="text-sm text-gray-600">
                    Menampilkan <span id="showing-from">1</span>-<span id="showing-to">{{ min(10, $bltDds->count()) }}</span>
                    dari <span id="total-records">{{ $bltDds->count() }}</span> penerima
                </div>

                <div class="flex items-center gap-2" id="pagination-controls">
                    <button onclick="loadBltPage(1)"
                        class="px-3 py-1 text-sm bg-white border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50"
                        id="first-btn">
                        <i class="fas fa-angle-double-left"></i>
                    </button>

                    <button onclick="loadBltPage(currentBltPage - 1)"
                        class="px-3 py-1 text-sm bg-white border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50"
                        id="prev-btn">
                        <i class="fas fa-angle-left"></i>
                    </button>

                    <span class="px-4 py-1 text-sm bg-purple-600 text-white rounded" id="page-info">
                        1 / {{ ceil($bltDds->count() / 10) }}
                    </span>
                    ...
                    <button onclick="loadBltPage({{ ceil($bltDds->count() / 10) }})"
                        class="px-3 py-1 text-sm bg-white border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50"
                        id="last-btn">
                ...
                <div class="text-sm text-gray-600">
                    <select onchange="changeBltPerPage(this.value)" class="px-2 py-1 text-sm border border-gray-300 rounded">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(paguCtx, {
        type: 'bar',
        data: {
            labels: chartData.pagu.map(item => {
                const kegiatan = item.kegiatan || 'Kegiatan Tidak Diketahui';
                return kegiatan.substring(0, 20) + (kegiatan.length > 20 ? '...' : '');
            }),
            datasets: [{
                label: 'Pagu (Rp)',
                data: chartData.pagu.map(item => item.pagu || 0),
    ...
    // PDF Embed Toggle Function
    function togglePdfEmbed() {
        const container = document.getElementById('pdfEmbedContainer');
        ...
        if (container && container.classList.contains('hidden')) {
            ...
            @if($pendapatan->first() && $pendapatan->first()->file_apbdes)
            if (iframe) {
                const pdfUrl = '{{ Storage::url($pendapatan->first()->file_apbdes) }}';
                console.log('Loading PDF:', pdfUrl);
                iframe.src = pdfUrl;
            }
            @else
            console.warn('No PDF file available for current year');
            @endif
            toggleText.innerHTML = '<i class="fas fa-eye-slash mr-2"></i>Sembunyikan PDF';
            ...
            if (toggleText) {
                toggleText.innerHTML = '<i class="fas fa-file-pdf mr-2"></i>Lihat APBDes {{ $tahun }} (PDF)';
            }
        }
    }
});
...
<!-- BLT Pagination Script (Global Scope) -->
<script>
// BLT Pagination Variables (Server-side without URL params)
let currentBltPage = 1;
let bltPerPage = 10;
let totalBltRecords = {{ $bltDds->count() }};
let bltData = @json($bltDds->toArray());

// Function to load BLT page
function loadBltPage(page) {
    const totalPages = Math.ceil(totalBltRecords / bltPerPage);

    if (page < 1 || page > totalPages) return;

    currentBltPage = page;
    const startIndex = (page - 1) * bltPerPage;
    const endIndex = Math.min(startIndex + bltPerPage, totalBltRecords);

    // Clear current table rows
    const tbody = document.getElementById('blt-table-body');
    if (!tbody) return;
    ...
    for (let i = startIndex; i < endIndex; i++) {
        ...
        const row = `
            <tr>
                <td class="px-6 py-4 text-center font-semibold text-gray-600">${i + 1}</td>
                <td class="px-6 py-4 font-semibold text-gray-800">${blt.nama || ''}</td>
                <td class="px-6 py-4 text-center">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${blt.jenis_kelamin === 'P' ? 'bg-pink-100 text-pink-800' : 'bg-blue-100 text-blue-800'}">
                        ${blt.jenis_kelamin || ''}
                    </span>
                </td>
                <td class="px-6 py-4 text-gray-600">${blt.alamat && blt.alamat.length > 30 ? blt.alamat.substring(0, 30) + '...' : (blt.alamat || '')}</td>
                <td class="px-6 py-4 text-gray-600 font-mono">${blt.no_rekening || '-'}</td>
                <td class="px-6 py-4 text-right font-bold text-purple-600">${formatRupiah(blt.nominal_bantuan || 0)}</td>
            </tr>
        `;
        tbody.innerHTML += row;
    }
...
function changeBltPerPage(newPerPage) {
    bltPerPage = parseInt(newPerPage);
    currentBltPage = 1;
    loadBltPage(1);
}
