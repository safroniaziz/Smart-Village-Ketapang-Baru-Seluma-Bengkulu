...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const userData = {
        nama: '{{ Auth::user()->name ?? "Belum diisi" }}',
    ...
    function generateModalForm(subJenis) {
        ...
        modalDynamicFormContainer.innerHTML = `
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-r-xl p-6 mb-8 shadow-sm">
                ...
                <!-- User Info Display -->
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 mb-6 border border-blue-200 shadow-sm">
                    <h6 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                        <i class="fas fa-user-circle text-blue-600 mr-3 text-xl"></i>
                        Data Pemohon (Otomatis dari Akun Anda)
                    </h6>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bg-white rounded-lg p-4 shadow-sm">
                            <span class="text-gray-600 text-sm font-medium">Nama Lengkap</span>
                            <div class="font-bold text-gray-900 text-lg">${userData.nama || 'Belum diisi'}</div>
                        </div>
                        <div class="bg-white rounded-lg p-4 shadow-sm">
                            <span class="text-gray-600 text-sm font-medium">NIK</span>
                            <div class="font-bold text-gray-900 text-lg">${userData.nik || 'Belum diisi'}</div>
                        </div>
                        <div class="bg-white rounded-lg p-4 shadow-sm md:col-span-2">
                            <span class="text-gray-600 text-sm font-medium">Alamat</span>
                            <div class="font-bold text-gray-900">${userData.alamat || 'Belum diisi'}</div>
                        </div>
                        <div class="bg-white rounded-lg p-4 shadow-sm">
                            <span class="text-gray-600 text-sm font-medium">No. HP</span>
                            <div class="font-bold text-gray-900 text-lg">${userData.no_hp || 'Belum diisi'}</div>
                        </div>
                    </div>
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mt-4">
                        <p class="text-sm text-yellow-800 flex items-center">
                            <i class="fas fa-exclamation-triangle mr-2"></i>
                            <strong>Penting:</strong> Data di atas akan digunakan untuk surat. Jika ada yang salah, silahkan hubungi admin desa untuk update profil.
                        </p>
                    </div>
                </div>
