    ...
    <div class="w-full lg:w-[80%] max-w-none mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12" data-aos="fade-up" data-aos-duration="800">
            ...
            <p class="text-lg text-gray-600 leading-relaxed max-w-2xl mx-auto mb-8">
                Eksplorasi data penduduk dengan visualization canggih dan filter interaktif real-time
            </p>

            <!-- Statistics Overview Cards -->
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 max-w-4xl mx-auto mb-8">
                <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-6 text-white" data-aos="fade-up" data-aos-delay="100">
                    <div class="flex items-center justify-between mb-2">
                        <div class="text-2xl font-black" id="displayTotalWarga">{{ number_format($totalWarga) }}</div>
                        <i class="fas fa-users text-blue-200 text-xl"></i>
                    </div>
                    <div class="text-sm text-blue-100">Total Penduduk</div>
                    <div class="text-xs text-blue-200 mt-1" id="totalWargaLabel">Seluruh data</div>
                </div>

                <div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-6 text-white" data-aos="fade-up" data-aos-delay="200">
                    <div class="flex items-center justify-between mb-2">
                        <div class="text-2xl font-black">{{ number_format($totalKK) }}</div>
                        <i class="fas fa-home text-emerald-200 text-xl"></i>
                    </div>
                    <div class="text-sm text-emerald-100">Kartu Keluarga</div>
                    <div class="text-xs text-emerald-200 mt-1">{{ round($totalWarga / ($totalKK ?: 1), 1) }} org/KK</div>
                </div>

                <div class="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl p-6 text-white" data-aos="fade-up" data-aos-delay="300">
                    <div class="flex items-center justify-between mb-2">
                        <div class="text-2xl font-black">{{ number_format($activeWarga) }}</div>
                        <i class="fas fa-check-circle text-purple-200 text-xl"></i>
                    </div>
                    <div class="text-sm text-purple-100">Status Aktif</div>
                    <div class="text-xs text-purple-200 mt-1">{{ round(($activeWarga / $totalWarga) * 100, 1) }}% aktif</div>
                </div>

                <div class="bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl p-6 text-white" data-aos="fade-up" data-aos-delay="400">
                    <div class="flex items-center justify-between mb-2">
                        <div class="text-2xl font-black" id="displayFilteredTotal">{{ number_format($filteredTotal ?? $totalWarga) }}</div>
                        <i class="fas fa-filter text-orange-200 text-xl"></i>
                    </div>
                    <div class="text-sm text-orange-100">Hasil Filter</div>
                    <div class="text-xs text-orange-200 mt-1" id="filteredLabel">Menampilkan semua</div>
                </div>
            </div>
        </div>

        <!-- Real-time Charts Section -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12" data-aos="fade-up" data-aos-duration="800" data-aos-delay="500">
            <!-- Gender Distribution Chart -->
            <div class="bg-white rounded-3xl p-8 shadow-2xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-gray-900">Distribusi Gender</h3>
                    <div class="w-3 h-3 bg-blue-500 rounded-full animate-pulse" id="genderChartLoader" style="display: none;"></div>
                </div>
                <div class="h-64">
                    <canvas id="filteredGenderChart"></canvas>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-2xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-gray-900">Kelompok Usia</h3>
                    <div class="w-3 h-3 bg-green-500 rounded-full animate-pulse" id="ageChartLoader" style="display: none;"></div>
                </div>
                <div class="h-64">
                    <canvas id="filteredAgeChart"></canvas>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-2xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-gray-900">Tingkat Pendidikan</h3>
                    <div class="w-3 h-3 bg-purple-500 rounded-full animate-pulse" id="educationChartLoader" style="display: none;"></div>
                </div>
                <div class="h-64">
                    <canvas id="filteredEducationChart"></canvas>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-2xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-gray-900">Top Pekerjaan</h3>
                    <div class="w-3 h-3 bg-orange-500 rounded-full animate-pulse" id="professionChartLoader" style="display: none;"></div>
                </div>
                <div class="h-64">
                    <canvas id="filteredProfessionChart"></canvas>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let genderChart, ageChart, educationChart, professionChart;
    ...
    function initializeCharts() {
        // Gender Chart
        const genderCtx = document.getElementById('filteredGenderChart').getContext('2d');
        genderChart = new Chart(genderCtx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [
                        {{ $filteredGenderStats['L'] ?? 0 }},
                        {{ $filteredGenderStats['P'] ?? 0 }}
                    ],
        ...
        // Age Chart
        const ageCtx = document.getElementById('filteredAgeChart').getContext('2d');
        ageChart = new Chart(ageCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Jumlah',
                    data: [
                        {{ $filteredAgeStats['Balita'] ?? 0 }},
                        {{ $filteredAgeStats['Anak'] ?? 0 }},
                        {{ $filteredAgeStats['Dewasa'] ?? 0 }},
                        {{ $filteredAgeStats['Lansia'] ?? 0 }}
                    ],
        ...
        // Education Chart
        const educationCtx = document.getElementById('filteredEducationChart').getContext('2d');
        educationChart = new Chart(educationCtx, {
            type: 'pie',
            data: {
                labels: {!! json_encode(array_keys($filteredEducationStats ?? [])) !!},
                datasets: [{
                    data: {!! json_encode(array_values($filteredEducationStats ?? [])) !!},
                    backgroundColor: [
                        '#8B5CF6', '#A78BFA', '#C4B5FD',
                        '#DDD6FE', '#EDE9FE', '#F3E8FF'
                    ],
                    borderWidth: 0
                }]
            },
        ...
        // Profession Chart
        const professionCtx = document.getElementById('filteredProfessionChart').getContext('2d');
        professionChart = new Chart(professionCtx, {
            type: 'bar',
            data: {
                labels: {!! json_encode(array_keys($filteredProfessionStats ?? [])) !!},
                datasets: [{
                    label: 'Jumlah',
                    data: {!! json_encode(array_values($filteredProfessionStats ?? [])) !!},
    ...
    function updateCharts(data) {
        if (genderChart && data.filteredGenderStats) {
            genderChart.data.datasets[0].data = [
                data.filteredGenderStats.L || 0,
                data.filteredGenderStats.P || 0
            ];
            genderChart.update('none');
        }

        if (ageChart && data.filteredAgeStats) {
            ageChart.data.datasets[0].data = [
                data.filteredAgeStats.Balita || 0,
                data.filteredAgeStats.Anak || 0,
                data.filteredAgeStats.Dewasa || 0,
                data.filteredAgeStats.Lansia || 0
            ];
            ageChart.update('none');
        }

        if (educationChart && data.filteredEducationStats) {
            const labels = Object.keys(data.filteredEducationStats);
            const values = Object.values(data.filteredEducationStats);
            educationChart.data.labels = labels;
            educationChart.data.datasets[0].data = values;
            educationChart.update('none');
        }

        if (professionChart && data.filteredProfessionStats) {
            const labels = Object.keys(data.filteredProfessionStats);
            const values = Object.values(data.filteredProfessionStats);
            professionChart.data.labels = labels;
            professionChart.data.datasets[0].data = values;
            professionChart.update('none');
        }
    }
    ...
    function performSearch(page = 1) {
        ...
        .then(data => {
            ...
            if (data.stats) {
                document.getElementById('displayFilteredTotal').textContent = data.stats.total.toLocaleString();
                document.getElementById('filteredLabel').textContent =
                    data.stats.total === {{ $totalWarga }} ? 'Menampilkan semua' :
            ...
            // Don't update URL for pagination - keep it clean
        })
        .catch(error => {
    ...
    // Pagination function without URL changes
    window.loadPage = function(page) {
        performSearch(page);
    };

    // Filter changes (immediate)
    [genderFilter, ageRangeFilter, educationFilter, professionFilter, dusunFilter, religionFilter, statusFilter, statusAktifFilter].forEach(filter => {
        if (filter) {
