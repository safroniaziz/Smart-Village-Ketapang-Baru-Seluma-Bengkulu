            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.pengajuan_id) {
                        setTimeout(() => {
                            showPDFActions(data.pengajuan_id);
                        }, 1000);
                    }
                } else {
...
// Function to show PDF actions
function showPDFActions(pengajuanId) {
    const notification = document.createElement('div');
    notification.className = 'fixed top-4 right-4 bg-white border border-gray-200 rounded-xl shadow-xl p-6 z-50 max-w-sm';
    notification.innerHTML = `
        <div class="flex items-center justify-between mb-4">
            <h4 class="text-lg font-semibold text-gray-900">Surat Berhasil Dibuat!</h4>
            <button onclick="this.parentElement.parentElement.remove()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <p class="text-gray-600 mb-4">Surat keterangan kehilangan Anda telah berhasil dibuat. Silakan download atau preview PDF.</p>
        <div class="flex space-x-3">
            <a href="/surat-kehilangan/preview/${pengajuanId}" target="_blank" class="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-center">
                <i class="fas fa-eye mr-2"></i>Preview
            </a>
            <a href="/surat-kehilangan/pdf/${pengajuanId}" class="flex-1 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors text-center">
