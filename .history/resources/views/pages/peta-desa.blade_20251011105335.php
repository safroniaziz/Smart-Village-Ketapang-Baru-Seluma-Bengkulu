...
function initMap() {
    ...
    map.on('load', () => {
        ...
        console.log(`Found ${validData.length} markers, but letting polygon boundary control the view`);
    ...
    setupSearch();
        ...
        map.once('idle', () => {
            console.log('🗺️ Map is idle, adding boundary...');
            setTimeout(() => {
...
function addWargaMarkers() {
    console.log('Adding warga markers...');
    console.log('Warga data received:', wargaData);
    console.log('Warga data length:', wargaData.length);

    if (wargaData.length === 0) {
        console.warn('No warga data available');
        return;
    }
    ...
    wargaData.forEach((warga, index) => {
        ...
        if (
            ...
            console.warn('Skipping invalid warga coords:', warga);
            return;
        }
            ...
            ">
                ...
                <div style="padding: 16px;">
                    ...
                    ">
                        ...
                        ">
                            ${alamat}
                            ${dusun ? `<br><small>Dusun: ${dusun}</small>` : ''}
                            ${rtRw ? `<br><small>RT/RW: ${rtRw}</small>` : ''}
                        </div>
                    </div>
    ...
    console.log(`👥 Added ${wargaMarkers.length} warga markers`);
}
...
async function addVillageBoundary() {
    ...
    try {
        ...
        if (!data || !data.features || data.features.length === 0) {
            console.warn('Boundary not found on Nominatim for Ketapang Baru, trying Overpass...');
        ...
        console.warn('Failed fetching village boundary:', error);
...
function renderBoundaryFeature(feature) {
    console.log('🎨 Rendering boundary feature:', feature);

    // Check if map is ready
    if (!map.isStyleLoaded()) {
        console.log('⏳ Map style not ready, waiting...');
        map.once('styledata', () => {
            console.log('✅ Map style loaded, retrying...');
            renderBoundaryFeature(feature);
        });
        return;
    }
    ...
    if (map.getLayer(lineLayerId)) {
        console.log('🗑️ Removing existing line layer');
    ...
    try {
        ...
        map.addLayer({
            ...
            source: sourceId,
            paint: {
                'fill-color': '#10B981', // Professional emerald green
                'fill-opacity': 0.15
            }
        });

        // Add line layer
        console.log('➕ Adding line layer:', lineLayerId);
        map.addLayer({
            id: lineLayerId,
            type: 'line',
            source: sourceId,
            paint: {
                'line-color': '#059669', // Darker emerald for contrast
                'line-width': 3
            }
        });

        console.log('✅ Boundary layers added successfully');

        // Check if layers are actually visible
        setTimeout(() => {
            const fillVisible = map.getLayer(fillLayerId);
            const lineVisible = map.getLayer(lineLayerId);
            console.log('🔍 Fill layer exists:', !!fillVisible);
            console.log('🔍 Line layer exists:', !!lineVisible);

            if (fillVisible && lineVisible) {
                console.log('✅ Both layers are visible');
            } else {
                console.error('❌ Layers not visible!');
            }
        }, 1000);

        // Fit bounds to feature immediately
        try {
            const bbox = getFeatureBBox(feature);
            if (bbox) {
                console.log('📍 Fitting bounds to polygon:', bbox);
        ...
        createNotification('<i class="fas fa-check mr-2"></i>Batas Desa Ketapang Baru ditambahkan (MERAH)', 'success');
    } catch (error) {
...
async function fetchBoundaryFromOverpass() {
    ...
    try {
        ...
        if (e.name === 'AbortError') {
            console.warn('Overpass API timeout - using manual drawing instead');
        } else {
...
function addPredefinedVillageBoundary() {
    if (!map) {
        console.warn('❌ Map not available for boundary');
        return;
    }

    console.log('🏗️ Building predefined village boundary...');
    ...
    console.log('📊 Polygon 1 points:', coords1.length);
    console.log('📊 Polygon 2 points:', coords2.length);

    // Render the boundary
    renderBoundaryFeature(multiPolygonFeature);
    console.log('✅ Predefined village boundary added');
}
...
function enableDrawTools() {
    console.log('🔧 Checking MapboxDraw availability:', typeof MapboxDraw);
    console.log('🔧 Map available:', !!map);

    if (typeof MapboxDraw === 'undefined') {
        console.warn('❌ MapboxDraw not loaded - check CDN');
    ...
    createNotification('<i class="fas fa-check mr-2"></i>Alat gambar polygon siap digunakan', 'success');
}
