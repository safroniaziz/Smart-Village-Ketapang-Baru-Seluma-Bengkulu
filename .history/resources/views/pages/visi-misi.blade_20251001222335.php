                    ...
                    <div class="space-y-4">
                        @if($pendekatan->count() > 0)
                            @foreach($pendekatan as $index => $item)
                                <div class="flex items-start space-x-4 pendekatan-item">
                                    <div class="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                                        <i class="pendekatan-icon fas fa-check text-white text-sm"></i>
                                    </div>
                                    <div>
                    ...
                    <div class="space-y-6">
                        @if($tahapan->count() > 0)
                            @foreach($tahapan as $index => $item)
                                <div class="flex items-center space-x-4 tahapan-item">
                                    <div class="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center shadow-lg">
                                        <i class="tahapan-icon fas fa-calendar text-white text-sm"></i>
                                    </div>
                                    <div class="flex-1">
...
@else
<!-- Empty State - Visi & Misi Belum Diatur -->
<section class="relative text-white overflow-hidden min-h-[calc(100vh-4rem)] md:min-h-[calc(100vh-5rem)] flex items-center pt-8 py-8 lg:py-12 pb-16 lg:pb-20 mb-4 md:mb-6" style="background: linear-gradient(135deg, #0086c9 0%, #0074b3 50%, #006ba3 100%);">
    <!-- Background Pattern -->
    <div class="absolute inset-0 bg-white/5"></div>

    <div class="relative w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 z-10 text-center">
        <!-- Icon -->
        ...
        <div class="space-y-6" data-aos="fade-up" data-aos-delay="200">
            <h1 class="text-4xl lg:text-5xl font-bold text-white">
                Visi & Misi Belum Diatur
            </h1>
            <p class="text-xl text-blue-100 max-w-2xl mx-auto leading-relaxed">
                Visi dan Misi desa belum diatur oleh administrator. Silakan kembali lagi nanti untuk melihat arah pembangunan desa.
            </p>
            
            <!-- Default Info Card -->
            <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-8 max-w-lg mx-auto">
                <h3 class="text-lg font-semibold text-white mb-4">Arah Pembangunan Desa</h3>
                <div class="space-y-3 text-blue-100 text-sm">
                    <div class="flex items-center justify-center gap-3">
                        <i class="fas fa-eye text-yellow-400"></i>
                        <span>Visi: Akan segera ditetapkan</span>
                    </div>
                    <div class="flex items-center justify-center gap-3">
                        <i class="fas fa-rocket text-yellow-400"></i>
                        <span>Misi: Akan segera ditetapkan</span>
                    </div>
                    <div class="flex items-center justify-center gap-3">
                        <i class="fas fa-users text-yellow-400"></i>
                        <span>Melibatkan partisipasi warga</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3 justify-center pt-6">
                <a href="{{ route('home') }}" class="inline-flex items-center gap-2 bg-white/15 hover:bg-white/25 backdrop-blur-md border-2 border-white/30 hover:border-white/50 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-300 transform hover:scale-105">
                    <i class="fas fa-home"></i>
                    Kembali ke Beranda
                </a>
                <a href="{{ route('tentang') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-yellow-400/20 to-orange-500/20 hover:from-yellow-400/30 hover:to-orange-500/30 backdrop-blur-md border-2 border-yellow-400/30 hover:border-yellow-400/50 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-300 transform hover:scale-105">
...
$(document).ready(function() {
    ...
    // Static Icons for Pendekatan and Tahapan
    const pendekatanIcons = [
        'fas fa-target', 'fas fa-bullseye', 'fas fa-crosshairs', 'fas fa-flag-checkered',
        'fas fa-rocket', 'fas fa-lightbulb', 'fas fa-cog', 'fas fa-tools',
        'fas fa-handshake', 'fas fa-users', 'fas fa-chart-line', 'fas fa-star'
    ];

    const tahapanIcons = [
        'fas fa-calendar', 'fas fa-clock', 'fas fa-hourglass-half', 'fas fa-calendar-alt',
        'fas fa-calendar-check', 'fas fa-calendar-plus', 'fas fa-calendar-times', 'fas fa-calendar-day',
        'fas fa-calendar-week', 'fas fa-calendar-month', 'fas fa-calendar-year', 'fas fa-calendar-range'
    ];
    ...
    function assignStaticIcons() {
        // Assign random icons to pendekatan items
        $('.pendekatan-item').each(function() {
            if (!$(this).data('icon-assigned')) {
                const randomIcon = getRandomIcon(pendekatanIcons);
                $(this).find('.pendekatan-icon').removeClass().addClass(randomIcon);
                $(this).data('icon-assigned', true);
            }
        });

        // Assign random icons to tahapan items
        $('.tahapan-item').each(function() {
            if (!$(this).data('icon-assigned')) {
                const randomIcon = getRandomIcon(tahapanIcons);
                $(this).find('.tahapan-icon').removeClass().addClass(randomIcon);
                $(this).data('icon-assigned', true);
            }
        });
    }

    // Assign icons when page loads
    assignStaticIcons();
