    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.toggleVideoQuality = function() {
            ...
            if (modal) {
                ...
                // Load modal video with fresh iframe - wait a bit to avoid conflicts
                setTimeout(() => {
                    if (iframe) {
                        // Create completely new src with autoplay for modal
                        const youtubeId = '{{ $youtubeId }}';
                        const newSrc = `https://www.youtube.com/embed/${youtubeId}?rel=0&controls=1&autoplay=1&mute=0&start=0`;
                        iframe.src = newSrc;
