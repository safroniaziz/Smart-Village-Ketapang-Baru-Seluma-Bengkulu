...
<style>
    ...
    /* Hide fallback elements when primary works */
    .pdf-embed:not([data-error]) ~ .pdf-object,
    .pdf-object:not([data-error]) ~ .pdf-iframe {
        display: none;
    }
    ...
    .pdf-embed:loaded ~ .pdf-embed-container::before,
    .pdf-object:loaded ~ .pdf-embed-container::before,
    .pdf-iframe:loaded ~ .pdf-embed-container::before {
        display: none;
    }
    ...
    <div class="modal-container">
        ...
        <div class="modal-body p-0">
            <div class="pdf-viewer-container">
                @if($wisata->file_potensi_desa)
                <!-- PDF Document Viewer -->
                <div id="pdfViewerContainer" class="w-full h-full bg-white">
                    <!-- PDF Header -->
                    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 border-b border-gray-200 flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <h4 class="font-semibold text-gray-800">Berkas Lomba Desa Wisata</h4>
                                <p class="text-sm text-gray-600">{{ $wisata->nama_wisata }}</p>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2">
                            <a href="{{ asset('storage/' . $wisata->file_potensi_desa) }}"
                               download
                    ...
                    <!-- PDF Embedded Viewer -->
                    <div class="pdf-embed-container bg-white overflow-hidden" style="height: calc(80vh - 120px); min-height: 500px;">
                        <!-- Primary: HTML5 embed element -->
                        <embed
                            src="{{ asset('storage/' . $wisata->file_potensi_desa) }}#toolbar=1&navpanes=1&scrollbar=1&view=FitH"
                            type="application/pdf"
                            width="100%"
                            height="100%"
                            class="pdf-embed border-0">

                        <!-- Fallback 1: Object element -->
                        <object
                            data="{{ asset('storage/' . $wisata->file_potensi_desa) }}#toolbar=1&navpanes=1&scrollbar=1&view=FitH"
                            type="application/pdf"
                            width="100%"
                            height="100%"
                            class="pdf-object border-0">
                            ...
                            <iframe
                                ...
                                width="100%"
                                height="100%"
                                class="pdf-iframe border-0"
                                title="PDF Viewer - {{ $wisata->nama_wisata }}">

                                <!-- Final fallback: Download/Open options -->
                                <div class="flex items-center justify-center h-full bg-gray-50">
                                    <div class="text-center p-8 max-w-md">
                                        <i class="fas fa-file-pdf text-6xl text-red-500 mb-4"></i>
                                        <h5 class="text-xl font-bold text-gray-800 mb-3">PDF Tidak Dapat Ditampilkan</h5>
                                        <p class="text-gray-600 mb-6">Browser Anda tidak mendukung tampilan PDF secara langsung. Silakan gunakan salah satu opsi di bawah ini:</p>
                                        ...
                                        <div class="space-y-3">
                                            ...
                                            <a href="{{ asset('storage/' . $wisata->file_potensi_desa) }}"
                                               download
                                               class="inline-flex items-center w-full justify-center px-6 py-3 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-all duration-200 shadow-lg hover:shadow-xl font-medium">
                                                <i class="fas fa-download mr-2"></i>
                                                Download PDF
                                                @if(file_exists(storage_path('app/public/' . $wisata->file_potensi_desa)))
                                                <span class="ml-2 text-sm opacity-75">
                                                    ({{ number_format(filesize(storage_path('app/public/' . $wisata->file_potensi_desa)) / 1024, 0) }} KB)
                                                </span>
                                                @endif
                                            </a>
                                        </div>
                                        ...
                                        <div class="mt-6 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                                            <div class="flex items-start text-sm text-yellow-800">
                                                <i class="fas fa-info-circle mt-0.5 mr-2 text-yellow-600"></i>
                                                <span>Untuk tampilan terbaik, gunakan browser Chrome, Firefox, Safari, atau Edge versi terbaru</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="modal-footer">
            <button type="button" class="btn-outline" onclick="closePdfModal()">
            ...
            <a href="{{ asset('storage/' . $wisata->file_potensi_desa) }}"
               target="_blank"
               class="inline-flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg transition-all duration-300">
                <i class="fas fa-external-link-alt mr-2"></i>
                Buka di Tab Baru
            </a>
            <a href="{{ asset('storage/' . $wisata->file_potensi_desa) }}"
        ...
        <div class="modal-body p-0">
            <div class="video-modal-container">
                ...
                <iframe id="videoModalPlayer"
                        src="https://www.youtube.com/embed/{{ $youtubeId }}?enablejsapi=1&rel=0&modestbranding=1&controls=1"
                        class="w-full h-full"
                        frameborder="0"
                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                        allowfullscreen>
                ...
                @else
                <div class="flex items-center justify-center h-96 bg-gray-900">
                    <div class="text-center text-white">
                        <i class="fas fa-video text-6xl text-gray-400 mb-4"></i>
                        <p class="text-gray-300">Video tidak tersedia</p>
                    </div>
                </div>
                @endif
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Enhanced PDF Modal with Loading Handling
        window.openPdfModal = function() {
            const modal = document.getElementById('pdfModal');
            if (modal) {
                modal.style.display = 'flex';
                ...
                const pdfContainer = modal.querySelector('.pdf-embed-container');
                const embed = modal.querySelector('.pdf-embed');
                const object = modal.querySelector('.pdf-object');
                ...
                // Set up load handlers for fallback chain
                if (embed) {
                ...
                console.log('PDF modal opened with enhanced loading handling');
            }
        };
