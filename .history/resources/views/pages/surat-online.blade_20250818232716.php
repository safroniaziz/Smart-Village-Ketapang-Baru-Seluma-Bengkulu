...
<!-- Modern Modal Pengajuan Surat -->
<div id="suratModal" class="fixed inset-0 z-50 hidden">
    <!-- Beautiful Backdrop -->
    <div class="absolute inset-0 bg-gradient-to-br from-blue-900/20 via-indigo-900/30 to-purple-900/20 backdrop-blur-md transition-all duration-500 opacity-0" id="modalBackdrop"></div>

    <div class="flex items-center justify-center min-h-screen p-4 relative z-10">
        <!-- Glassmorphism Modal -->
        <div class="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden transform scale-90 transition-all duration-500 border border-white/20" id="modalContent">

            <!-- Elegant Header with Gradient -->
            <div class="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white p-8">
                <!-- Decorative Elements -->
                ...
                <div class="relative flex items-center justify-between">
                    <div class="flex items-center">
                        <div class="bg-white/20 backdrop-blur-sm rounded-2xl w-16 h-16 flex items-center justify-center mr-4 shadow-lg">
                            <i class="fas fa-file-signature text-2xl text-white"></i>
                        </div>
                        <div>
                            <h3 class="text-2xl font-bold mb-1">Pengajuan Surat Online</h3>
                            <p class="text-blue-100">Proses mudah dalam 3 langkah</p>
                        </div>
                    </div>
                    <button onclick="closeSuratModal()" class="text-white/80 hover:text-white p-3 hover:bg-white/10 rounded-xl transition-all duration-200 group">
                        <i class="fas fa-times text-xl group-hover:rotate-90 transition-transform duration-200"></i>
                    </button>
                </div>

                <!-- Progress Steps -->
                <div class="flex items-center justify-center mt-6 space-x-4" id="progressSteps">
                    <div class="flex items-center">
                        <div class="w-8 h-8 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-sm font-bold step-indicator active" data-step="1">1</div>
                        <span class="ml-2 text-sm text-blue-100">Pilih Sub-Jenis</span>
                    </div>
                    <div class="w-8 h-0.5 bg-white/30 rounded-full"></div>
                    <div class="flex items-center">
                        <div class="w-8 h-8 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-sm font-bold step-indicator" data-step="2">2</div>
                        <span class="ml-2 text-sm text-blue-100">Isi Form</span>
                    </div>
                </div>
            </div>

            <!-- Modal Body with Better Styling -->
            <div class="p-8 overflow-y-auto max-h-[calc(95vh-200px)] bg-gradient-to-br from-gray-50/50 to-blue-50/30">
                ...
                <div id="modalStepSelectSubJenis">
                    <div class="text-center mb-8">
                        <div class="inline-flex items-center bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-800 px-4 py-2 rounded-full text-sm font-semibold mb-4">
                            <i class="fas fa-list mr-2"></i>
                            Langkah 1 dari 2
                        </div>
                        <h4 class="text-2xl font-bold text-gray-900 mb-2" id="modalCategoryTitle">Pilih Jenis Surat Spesifik</h4>
                ...
                <div id="modalStepFormKhusus" class="hidden">
                    <div class="text-center mb-8">
                        ...
                        <h4 class="text-2xl font-bold text-gray-900 mb-2">Lengkapi Data Pengajuan</h4>
                        <p class="text-gray-600" id="modalFormTitle">Isi semua data yang diperlukan dengan benar</p>
                        <button type="button" id="modalBackToSubJenis" class="inline-flex items-center text-blue-600 hover:text-blue-800 text-sm mt-3 font-medium hover:underline transition-all">
                    ...
                    <form id="modalFormAjukanSurat" class="space-y-6" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <div class="bg-blue-100 rounded-xl w-12 h-12 flex items-center justify-center mr-4">
                                        <i class="fas fa-shield-check text-blue-600"></i>
                                    </div>
                                    <div>
                                        <h5 class="font-semibold text-blue-900">Data Aman & Terlindungi</h5>
                                        <p class="text-sm text-blue-700">Informasi Anda akan diproses dengan aman</p>
                                    </div>
                                </div>
                                <button type="submit" class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-4 px-8 rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
    ...
    function updateProgressSteps(activeStep) {
        ...
        stepIndicators.forEach((indicator, index) => {
            if (index + 1 <= activeStep) {
                ...
                indicator.classList.add('bg-white', 'text-blue-600');
            } else {
                indicator.classList.remove('active', 'bg-white', 'text-blue-600');
        ...
        modalSubJenisList.innerHTML = subJenisItems.map(item => `
            <div class="bg-white/80 backdrop-blur-sm p-4 border-2 border-gray-200 rounded-2xl hover:border-blue-500 hover:bg-blue-50 transition-all duration-300 cursor-pointer transform hover:scale-105 modal-sub-jenis-btn" data-subjenis="${item.id}">
                <div class="flex items-center">
                    <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl w-12 h-12 flex items-center justify-center mr-4 text-white shadow-lg">
                        <i class="${item.icon}"></i>
                    </div>
                    <div>
    ...
    function generateModalForm(subJenis) {
        ...
        const fieldsHtml = template.fields.map(field => {
            ...
            if (field.type === 'select') {
                ...
                inputHtml = `<select name="${field.name}" ${field.required ? 'required' : ''} class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white/80">
                    <option value="">Pilih ${field.label.toLowerCase()}</option>
                    ${options}
                </select>`;
            } else if (field.type === 'textarea') {
                inputHtml = `<textarea name="${field.name}" ${field.required ? 'required' : ''} rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white/80" placeholder="Masukkan ${field.label.toLowerCase()}..."></textarea>`;
            } else if (field.type === 'file') {
                inputHtml = `
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-blue-400 transition-all duration-200 bg-white/50 backdrop-blur-sm">
                ...
                inputHtml = `<input type="${field.type}" name="${field.name}" ${field.required ? 'required' : ''} class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white/80" placeholder="Masukkan ${field.label.toLowerCase()}...">`;
            }

            return `
                <div class="mb-6">
                    <label class="block text-sm font-semibold mb-3 text-gray-800">
                        <i class="${field.icon} mr-2 text-blue-600"></i>${field.label} ${field.required ? '<span class="text-red-500">*</span>' : ''}
                    </label>
                    ${inputHtml}
                </div>
            `;
        ...
        modalDynamicFormContainer.innerHTML = `
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-6 mb-6 backdrop-blur-sm">
                <div class="flex items-center mb-3">
                    <div class="bg-blue-100 rounded-xl w-10 h-10 flex items-center justify-center mr-3">
                        <i class="fas fa-file-signature text-blue-600"></i>
                    </div>
                    <h5 class="font-semibold text-blue-900">${template.title}</h5>
                </div>
                <p class="text-sm text-blue-800">Lengkapi semua field yang bertanda <span class="text-red-500">*</span> (wajib diisi)</p>
            </div>
            <div class="space-y-6">
                ${fieldsHtml}
            </div>
