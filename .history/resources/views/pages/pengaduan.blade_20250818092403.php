    ...
    <div class="w-full lg:w-[80%] max-w-none mx-auto px-4 sm:px-6 lg:px-8">
        <div class="subnav-surface">
            <ul class="flex items-center gap-2 sm:gap-3 min-w-max" id="pengaduanSubnav">
                ...
                <li>
                    <a href="#pengaduan-terbaru" class="stat-tab" aria-current="false">
...
<!-- Complaint Status -->
<section id="cek-status" class="section-nav py-20 bg-white relative overflow-hidden">
    <!-- Background Decorations -->
...
<!-- Recent Complaints -->
<section id="pengaduan-terbaru" class="section-nav py-20 bg-gradient-to-br from-gray-50 to-blue-50 relative overflow-hidden">
    <!-- Background Decorations -->
    <div class="absolute top-20 left-10 w-64 h-64 bg-gradient-to-br from-blue-200/20 to-indigo-300/20 rounded-full blur-3xl"></div>
    <div class="absolute bottom-20 right-10 w-96 h-96 bg-gradient-to-br from-purple-200/20 to-pink-300/20 rounded-full blur-3xl"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($pengaduanTerbaru as $index => $pengaduan)
            <div class="group bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/50 overflow-hidden transform transition-all duration-500 hover:scale-105 hover:shadow-3xl" data-aos="fade-up" data-aos-delay="{{ ($index + 1) * 100 }}">
                @php
                    $statusColors = [
                        'Selesai' => 'from-green-500 via-emerald-500 to-teal-500',
                        'Dalam Proses' => 'from-yellow-500 via-orange-500 to-red-500',
                        'Diterima' => 'from-blue-500 via-indigo-500 to-purple-500',
                        'Ditolak' => 'from-red-500 via-pink-500 to-rose-500'
                    ];
                    $statusIcons = [
                        'Selesai' => 'fas fa-check-circle',
                        'Dalam Proses' => 'fas fa-clock',
                        'Diterima' => 'fas fa-inbox',
                ...
                @endphp
                <div class="bg-gradient-to-r {{ $statusColors[$pengaduan->status] ?? 'from-gray-500 to-gray-600' }} text-white p-6">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center">
                            <div class="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center mr-3">
                                <i class="{{ $statusIcons[$pengaduan->status] ?? 'fas fa-question' }} text-white text-xl"></i>
                            </div>
                            <div>
                                <span class="font-bold text-lg">{{ $pengaduan->status }}</span>
                                <p class="text-white/80 text-sm">{{ $pengaduan->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                        <div class="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
                            ...
                            @endphp
                            <i class="{{ $jenisIcons[$pengaduan->jenis_pengaduan] ?? 'fas fa-exclamation-triangle' }} text-white text-2xl"></i>
                        </div>
                    </div>
                </div>
                <div class="p-6">
                    <h3 class="text-xl font-bold text-gray-900 mb-3">{{ Str::limit($pengaduan->deskripsi, 40) }}</h3>
                    <p class="text-gray-600 mb-4">{{ Str::limit($pengaduan->deskripsi, 80) }}</p>
                    <div class="flex justify-between items-center">
                        <span class="inline-block {{ $statusBadgeColors[$pengaduan->status] ?? 'bg-gray-100 text-gray-800' }} px-4 py-2 rounded-full text-sm font-semibold">{{ $statusBadges[$pengaduan->status] ?? $pengaduan->status }}</span>
                        <span class="text-sm text-gray-500 font-mono">{{ $pengaduan->nomor_tracking }}</span>
                    </div>
                </div>
            </div>
            @endforeach
...
<!-- Statistics -->
<section id="statistik" class="section-nav py-20 bg-white relative overflow-hidden">
    <!-- Background Decorations -->
