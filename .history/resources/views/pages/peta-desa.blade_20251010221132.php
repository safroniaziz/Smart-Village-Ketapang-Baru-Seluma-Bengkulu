...
function initMap() {
    ...
    setupSearch();
        ...
        // Add predefined village boundary from coordinates
...
function renderBoundaryFeature(feature) {
    ...
    // Extract and provide coordinates to the user
    try {
        const coordsOnly = extractBoundaryCoordinates(feature);
        console.log('🧭 Boundary coordinates (lng, lat):', coordsOnly);

        // Auto-download GeoJSON for convenience
        const geojsonText = JSON.stringify(feature, null, 2);
        const blob = new Blob([geojsonText], { type: 'application/geo+json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'batas-ketapang-baru.geojson';
        ...
        // Try to copy coordinates to clipboard
        if (navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
            navigator.clipboard.writeText(JSON.stringify(coordsOnly));
        }
        createNotification('<i class="fas fa-check mr-2"></i>Batas Desa ditambahkan • Koordinat disalin & diunduh', 'success');
    } catch (e) {
        createNotification('<i class="fas fa-check mr-2"></i>Batas Desa ditambahkan', 'success');
    }
}
