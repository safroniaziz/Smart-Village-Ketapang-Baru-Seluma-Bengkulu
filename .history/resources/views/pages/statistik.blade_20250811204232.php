...
<nav class="stat-subnav sticky top-20 z-30" aria-label="Navigasi Statistik">
    <div class="w-full lg:w-[80%] max-w-none mx-auto px-4 sm:px-6 lg:px-8">
        <div class="subnav-surface">
            <span class="subnav-indicator" aria-hidden="true"></span>
            <ul class="flex items-center gap-2 sm:gap-3 min-w-max" id="statSubnav" role="tablist">
                <li>
                    <a href="#overview" class="stat-tab" aria-current="false" role="tab" aria-selected="false" tabindex="0">
                        <i class="fas fa-chart-pie mr-2"></i>
                        <span>Ikhtisar</span>
                    </a>
                </li>
                <li>
                    <a href="#education" class="stat-tab" aria-current="false" role="tab" aria-selected="false" tabindex="-1">
                        <i class="fas fa-graduation-cap mr-2"></i>
                        <span>Pendidikan & Pekerjaan</span>
                    </a>
                </li>
                <li>
                    <a href="#social" class="stat-tab" aria-current="false" role="tab" aria-selected="false" tabindex="-1">
                        <i class="fas fa-heart mr-2"></i>
                        <span>Sosial & Agama</span>
                    </a>
                </li>
                <li>
                    <a href="#aid" class="stat-tab" aria-current="false" role="tab" aria-selected="false" tabindex="-1">
                        <i class="fas fa-hands-helping mr-2"></i>
                        <span>Bantuan</span>
                    </a>
                </li>
                <li>
                    <a href="#data" class="stat-tab" aria-current="false" role="tab" aria-selected="false" tabindex="-1">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-2xl" data-aos="fade-right" data-aos-duration="800">
                <div class="flex items-center space-x-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600">Perbandingan Laki-laki & Perempuan</p>
                    </div>
                </div>
                ...
                <div class="space-y-4 mb-6">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-2xl font-bold text-blue-600">{{ $genderStats['L'] ?? 0 }}</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-3">
                        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-3 rounded-full" style="width: {{ $genderStats['L'] ? ($genderStats['L'] / ($genderStats['L'] + ($genderStats['P'] ?? 0))) * 100 : 0 }}%"></div>
                    </div>

                    <div class="flex items-center justify-between">
                        <span class="text-gray-700 font-medium">Perempuan</span>
                        <span class="text-2xl font-bold text-pink-600">{{ $genderStats['P'] ?? 0 }}</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-3">
                        <div class="bg-gradient-to-r from-pink-500 to-fuchsia-600 h-3 rounded-full" style="width: {{ $genderStats['P'] ? ($genderStats['P'] / ($genderStats['L'] + $genderStats['P'])) * 100 : 0 }}%"></div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Indicator positioning animation
    const subnavSurface = document.querySelector('.subnav-surface');
    const indicator = document.querySelector('.subnav-indicator');
    function updateIndicator() {
        if (!indicator || !subnavSurface) return;
        const active = document.querySelector('.stat-tab.active') || tabs[0];
        if (!active) return;
        const left = active.offsetLeft - subnavSurface.scrollLeft;
        const width = active.offsetWidth;
        indicator.style.transform = `translateX(${left}px)`;
        indicator.style.width = `${width}px`;
    }
    updateIndicator();
    subnavSurface?.addEventListener('scroll', () => {
