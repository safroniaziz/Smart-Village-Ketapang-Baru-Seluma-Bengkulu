                ...
                <div class="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden" data-aos="fade-left">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex items-center gap-3 p-3 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl">
                            ...
                            <div>
                                <div class="font-semibold text-gray-900">Titik Lahan</div>
                                <div class="text-xs text-gray-600">Lokasi lahan warga</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-600">Dokumentasi visual</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 p-3 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl">
                            <div class="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-lg flex-shrink-0">
                                GPS
                            </div>
                            <div>
                                <div class="font-semibold text-gray-900">Koordinat GPS</div>
                                <div class="text-xs text-gray-600">Latitude & Longitude</div>
                            </div>
                        </div>
                    </div>
                </div>
...
function initMap() {
    // Initialize Leaflet map with better tiles
    map = L.map('map', {
        zoomControl: false,
        attributionControl: true
    }).setView([-4.314632, 102.763257], 13);
    ...
    const customIcon = L.divIcon({
        ...
        html: '<i class="fas fa-map-marker-alt custom-marker-icon"></i>',
        iconSize: [40, 40],
        iconAnchor: [20, 40],
        popupAnchor: [0, -40]
    });
    ...
    allData.forEach((item, index) => {
        ...
        // Random color untuk setiap lahan
        const color = colors[index % colors.length];
        
        // Tambahkan circle untuk visualisasi area lahan (radius ~50m)
        ...
        marker.addTo(map);
        marker.itemData = item; // Store data for search
        marker.circleRef = circle; // Reference to circle
        markers.push(marker);
        bounds.push([lat, lng]);
        
        // Bind popup to circle too
    ...
    searchInput.addEventListener('input', function(e) {
        ...
        if (query.length < 2) {
            ...
            markers.forEach((m, idx) => {
                m.setOpacity(1);
                if (m.circleRef) m.circleRef.setStyle({ fillOpacity: 0.3, opacity: 1 });
            });
            map.closePopup();
            return;
        }
        ...
        markers.forEach((m, idx) => {
            ...
            if (name.includes(query) || nik.includes(query)) {
                m.setOpacity(1);
                if (m.circleRef) m.circleRef.setStyle({ fillOpacity: 0.5, opacity: 1 });
                ...
                m.setOpacity(0.2);
                if (m.circleRef) m.circleRef.setStyle({ fillOpacity: 0.1, opacity: 0.3 });
            }
        });
    });
}
