...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const userData = {
        nama: '{{ Auth::user()->name ?? Auth::user()->nama ?? "Belum diisi" }}',
        ...
        alamat: '{{ Auth::user()->alamat ?? Auth::user()->address ?? "Belum diisi" }}',
        rt_rw: '{{ (Auth::user()->rt ?? "") }}/{{ (Auth::user()->rw ?? "") }}',
        no_hp: '{{ Auth::user()->no_hp ?? Auth::user()->phone ?? Auth::user()->telefon ?? "Belum diisi" }}',
        pekerjaan: '{{ Auth::user()->pekerjaan ?? Auth::user()->job ?? "Belum diisi" }}'
    };
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-r-xl p-6 mb-8 shadow-sm">
                ...
                <div class="bg-blue-50 rounded-lg p-4 mb-4 border border-blue-200">
                    <h6 class="text-sm font-semibold text-gray-800 mb-3 flex items-center">
                        ...
                        Data Pemohon (Otomatis dari Akun)
                    </h6>
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        <div>
                            <span class="text-gray-600">Nama:</span>
                            <span class="font-medium text-gray-900 ml-1">${userData.nama || 'Belum diisi'}</span>
                        </div>
                        <div>
                            <span class="text-gray-600">NIK:</span>
                            <span class="font-medium text-gray-900 ml-1">${userData.nik || 'Belum diisi'}</span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-gray-600">Alamat:</span>
                            <span class="font-medium text-gray-900 ml-1">${userData.alamat || 'Belum diisi'}</span>
                        </div>
                        <div>
                            <span class="text-gray-600">No. HP:</span>
                            <span class="font-medium text-gray-900 ml-1">${userData.no_hp || 'Belum diisi'}</span>
                        </div>
                    </div>
                    <p class="text-xs text-blue-700 mt-3 flex items-center">
                        <i class="fas fa-info-circle mr-1"></i>
                        Data ini otomatis digunakan untuk surat. Jika salah, hubungi admin.
                    </p>
                </div>
