...
<section id="aid" class="stat-target py-20 bg-white">
    <div class="w-full lg:w-[80%] max-w-none mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            ...
            <!-- Program Details -->
            <div class="stat-card bg-gradient-to-br from-slate-50 via-gray-50 to-zinc-50 rounded-3xl p-8 shadow-2xl border border-gray-200" data-aos="fade-left" data-aos-duration="800" data-aos-delay="200">
                <div class="text-center mb-8">
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mb-2">Detail Program Bantuan</h3>
                ...
                <div class="space-y-6 max-h-96 overflow-y-auto custom-scrollbar pr-2">
                    @forelse($programDetails as $index => $program)
                    <div class="group relative bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 overflow-hidden">
                        <!-- Background Pattern -->
                        <div class="absolute top-0 right-0 w-32 h-32 opacity-5">
                            <div class="w-full h-full bg-gradient-to-br {{ $program['colors']['gradient'] }} rounded-full blur-2xl"></div>
                        </div>
                        ...
                        <div class="relative flex items-start justify-between mb-4">
                            <div class="flex items-center gap-4">
                                <div class="relative">
                                    <div class="w-14 h-14 bg-gradient-to-br {{ $program['colors']['gradient'] }} rounded-xl flex items-center justify-center shadow-lg">
                                        <i class="{{ $program['icon'] }} text-white text-lg"></i>
                                    </div>
                                ...
                                <div>
                                    <h4 class="text-lg font-bold {{ $program['colors']['text'] }} mb-1">{{ $program['program'] }}</h4>
                                    <p class="text-sm {{ $program['colors']['subtext'] }}">{{ $program['description'] }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="text-2xl font-black {{ $program['colors']['amount'] }} mb-1">{{ number_format($program['unique_recipients']) }}</div>
                                <div class="text-xs {{ $program['colors']['subamount'] }} font-medium">Keluarga</div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-4 mb-4">
                            <div class="text-center">
                                <div class="text-lg font-bold {{ $program['colors']['amount'] }}">{{ $program['percentage'] }}%</div>
                                <div class="text-xs {{ $program['colors']['subtext'] }}">Cakupan</div>
                            </div>
                            <div class="text-center">
                                <div class="text-lg font-bold {{ $program['colors']['amount'] }}">{{ number_format($program['total_recipients']) }}</div>
                                <div class="text-xs {{ $program['colors']['subtext'] }}">Total Penerima</div>
                            </div>
                            <div class="text-center">
                                <div class="text-lg font-bold {{ $program['colors']['amount'] }}">Rp {{ $program['amount'] }}</div>
                                <div class="text-xs {{ $program['colors']['subtext'] }}">Per Bulan</div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-medium {{ $program['colors']['subtext'] }}">Tingkat Cakupan</span>
                                <span class="text-xs font-bold {{ $program['colors']['amount'] }}">{{ $program['percentage'] }}%</span>
                            </div>
                            <div class="relative">
                                <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                    <div class="h-full bg-gradient-to-r {{ $program['colors']['bar'] }} rounded-full transition-all duration-2000 ease-out relative overflow-hidden" 
                                ...
                                <div class="flex justify-between mt-1">
                                    @for($i = 0; $i <= 10; $i++)
                                        <div class="w-1 h-1 rounded-full {{ $i * 10 <= $program['percentage'] ? $program['colors']['amount'] : 'bg-gray-300' }}"></div>
                                    @endfor
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Hover Effect -->
                        <div class="absolute inset-0 bg-gradient-to-r {{ $program['colors']['gradient'] }} opacity-0 group-hover:opacity-5 transition-opacity duration-300 rounded-2xl"></div>
                    </div>
                    @empty
