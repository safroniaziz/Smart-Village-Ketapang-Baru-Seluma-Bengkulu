...
let map, markers = [], allData = [], clusterMarkers = [];
...
// Improved coordinates for Ketapang Baru, Talo, Seluma, Bengkulu
const KETAPANG_BARU_CENTER = [102.7633, -4.3146]; // More accurate coordinates
...
function getLandIcon(item, color) {
    // Determine icon based on land type or default
    const landType = (item.jenis_lahan || '').toLowerCase();
    
    if (landType.includes('sawah') || landType.includes('padi')) {
        return `<path d="M-6,-6 L6,-6 L6,6 L-6,6 Z M-4,-4 L4,-4 M-4,0 L4,0 M-4,4 L4,4" stroke="${color}" stroke-width="1.5" fill="none"/>`;
    } else if (landType.includes('kebun') || landType.includes('perkebunan')) {
        return `<circle cx="0" cy="-2" r="3" fill="${color}"/>
                <path d="M0,1 L0,6 M-2,3 L2,3" stroke="${color}" stroke-width="2"/>`;
    } else if (landType.includes('rumah') || landType.includes('bangunan')) {
        return `<path d="M-5,-2 L0,-6 L5,-2 L5,4 L-5,4 Z M-2,-1 L2,-1 L2,4" stroke="${color}" stroke-width="1.5" fill="none"/>`;
    } else {
        // Default agricultural icon
        return `<path d="M-4,-4 C-4,-6 -2,-6 0,-4 C2,-6 4,-6 4,-4 M-3,0 L3,0 M0,0 L0,6" stroke="${color}" stroke-width="2" fill="none"/>
...
function initMap() {
    ...
    map = new mapboxgl.Map({
        container: 'map',
        style: 'mapbox://styles/mapbox/outdoors-v12', // Changed to outdoors for better visibility
        center: KETAPANG_BARU_CENTER,
        zoom: 15, // Increased zoom level
        pitch: 0, // Reduced pitch for better marker visibility
        bearing: 0,
        ...
        validData.forEach((item, index) => {
            ...
            el.innerHTML = `
                ...
                ">
                    ...
                    <!-- Main marker body -->
                    <svg width="50" height="60" viewBox="0 0 50 60" style="display: block;">
                        <defs>
                        ...
                        <!-- Marker shape -->
                        <path d="M25 5 C15 5 7 13 7 23 C7 35 25 55 25 55 C25 55 43 35 43 23 C43 13 35 5 25 5 Z"
                              fill="url(#grad-${index})"
                              stroke="white"
                              stroke-width="2"/>
                        
                        <!-- Inner circle -->
                        <circle cx="25" cy="23" r="12" fill="white" opacity="0.95"/>
                        
                        <!-- Icon based on land type -->
                        <g transform="translate(25, 23)">
                            ${getLandIcon(item, color)}
                        </g>
                    </svg>
    ...
    if (!bounds.isEmpty()) {
        ...
        let padding = 50;
        if (validData.length > 20) padding = 80;
        if (validData.length > 50) padding = 100;
...
function enhanceMapInteractions() {
    ...
    map.on('mouseenter', 'land-circles-layer', () => {
        map.getCanvas().style.cursor = 'pointer';
    });

    map.on('mouseleave', 'land-circles-layer', () => {
        map.getCanvas().style.cursor = '';
    });

    // Add click handler for circle layer
    map.on('click', 'land-circles-layer', (e) => {
