...
class PotensiWisataPublicController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $youtubeId = null;
        $heroPhoto = null;
        $randomPantaiPhotos = collect();

        if ($wisata) {
            // Get hero photo
            $heroPhoto = GalleryFoto::getHeroPhoto();
            
            // Debug: Log hero photo
            if ($heroPhoto) {
                \Log::info('Hero Photo Found:', ['id' => $heroPhoto->id, 'judul' => $heroPhoto->judul, 'is_hero' => $heroPhoto->is_hero]);
            } else {
                \Log::info('No Hero Photo Found');
            }
            
            // Get random pantai photos for floating cards
            $randomPantaiPhotos = GalleryFoto::getRandomPantaiPhotos(2);

            // Get gallery photos from GalleryFoto model (organized by category)
            $galleryCategories = GalleryFoto::aktif()
                                          ->urutan()
                                          ->get()
                                          ->groupBy('kategori');

            $galleryCount = GalleryFoto::aktif()->count();
        ...
        // Create backward compatible galleryPhotos array (flatten all categories)
        $galleryPhotos = $galleryCategories->flatten()->map(function($photo) {
            return [
                'url' => $photo->url_foto,
                'judul' => $photo->judul,
                'keterangan' => $photo->deskripsi,
                'alt_text' => $photo->alt_text
            ];
        })->toArray();

        return view('pages.potensi-wisata', compact('wisata', 'galleryCategories', 'galleryPhotos', 'galleryCount', 'youtubeId', 'heroPhoto', 'randomPantaiPhotos'));
    }
