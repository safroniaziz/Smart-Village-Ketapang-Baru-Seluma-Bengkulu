...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $totalLakiLaki = User::whereNotNull('nik')->where('jenis_kelamin', 'Laki-laki')->count();
        $totalPerempuan = User::whereNotNull('nik')->where('jenis_kelamin', 'Perempuan')->count();
        
        // Data distribusi usia
        $usia0_17 = User::whereNotNull('nik')
            ->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 0 AND 17')
            ->count();
        $usia18_30 = User::whereNotNull('nik')
            ->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 18 AND 30')
            ->count();
        $usia31_45 = User::whereNotNull('nik')
            ->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 31 AND 45')
            ->count();
        $usia46_60 = User::whereNotNull('nik')
            ->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 46 AND 60')
            ->count();
        $usia60Plus = User::whereNotNull('nik')
            ->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) >= 60')
            ->count();
        
        // Data tambahan untuk statistik
        $totalDusun = User::whereNotNull('nik')->distinct('dusun')->count('dusun');
        $totalKepalaKeluarga = User::whereNotNull('nik')->where('is_kepala_keluarga', true)->count();
