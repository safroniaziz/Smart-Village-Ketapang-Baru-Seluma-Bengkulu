...
class DataWargaController extends BaseController
{
    ...
    {
        $query = User::where('nik', '!=', '-')->whereNotNull('nik');
    ...
    {
        return $this->exportExcel($request); // Same as Excel for now
    }
    ...
    /**
     * Fix family head duplicates - ensure only one kepala keluarga per no_kk
     */
    public function fixFamilyHeads()
    {
        try {
            $duplicateFamilies = User::selectRaw('no_kk, COUNT(*) as count')
                ->whereNotNull('no_kk')
                ->where('is_kepala_keluarga', true)
                ->groupBy('no_kk')
                ->having('count', '>', 1)
            ...
            foreach ($duplicateFamilies as $family) {
                // Get all kepala keluarga for this no_kk
                $kepalaKeluargas = User::where('no_kk', $family->no_kk)
                    ->where('is_kepala_keluarga', true)
                    ->orderBy('created_at', 'asc') // Keep the oldest one
                    ->get();

                // Keep only the first one as kepala keluarga
                $kepalaKeluargas->skip(1)->each(function ($user) use (&$fixedCount) {
                    $user->update(['is_kepala_keluarga' => false]);
                    $fixedCount++;
                });
            }

            return redirect()->route('data-warga.index')
                ->with('success', "Berhasil memperbaiki {$fixedCount} duplikasi kepala keluarga.");

        } catch (\Exception $e) {
            return redirect()->route('data-warga.index')
                ->with('error', 'Terjadi error saat memperbaiki data: ' . $e->getMessage());
        }
    }

    /**
     * Fix gender format - convert short format (L/P) to long format (Laki-laki/Perempuan)
     */
    public function fixGenderFormat()
    {
        try {
            $fixedCount = 0;
            
            // Convert L to Laki-laki
            $lakiCount = User::where('jenis_kelamin', 'L')->update(['jenis_kelamin' => 'Laki-laki']);
            $fixedCount += $lakiCount;
            
            // Convert P to Perempuan
            $perempuanCount = User::where('jenis_kelamin', 'P')->update(['jenis_kelamin' => 'Perempuan']);
            $fixedCount += $perempuanCount;

            return redirect()->route('data-warga.index')
                ->with('success', "Berhasil memperbaiki {$fixedCount} data jenis kelamin dari format singkat ke format lengkap.");

        } catch (\Exception $e) {
            return redirect()->route('data-warga.index')
                ->with('error', 'Terjadi error saat memperbaiki format jenis kelamin: ' . $e->getMessage());
        }
    }
}
