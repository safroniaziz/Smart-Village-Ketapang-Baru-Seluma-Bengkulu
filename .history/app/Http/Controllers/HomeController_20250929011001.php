...
class HomeController extends Controller
{
    ...
    public function dataWarga(Request $request)
    {
        ...
        // Group by no_kk (keluarga)
        $groupedWargas = $allWargas->groupBy('no_kk');
        
        // Flatten grouped data for pagination
        $wargasData = collect();
        foreach ($groupedWargas as $kk => $familyMembers) {
            // Add family members to collection
            foreach ($familyMembers as $member) {
                $wargasData->push($member);
            }
        }
        
        // Manual pagination
        $total = $wargasData->count();
        $offset = ($page - 1) * $perPage;
        $items = $wargasData->slice($offset, $perPage)->values();
