...
class KontakController extends Controller
{
    public function index()
    {
        ...
        if ($streetViewUrl) {
            ...
            // Prefer Street View embed from coordinates if available; fallback to map embed otherwise
            if ($monografi && $monografi->latitude && $monografi->longitude) {
                $lat = $monografi->latitude;
                $lng = $monografi->longitude;
                // Use Street View first
                $embedStreetViewMapUrl = "https://maps.google.com/maps?layer=c&cbll={$lat},{$lng}&hl=id&output=embed";
                // Also prepare normal map as secondary (in case the Blade prefers it elsewhere)
                ...
                // Use Street View first
                $embedStreetViewMapUrl = "https://maps.google.com/maps?layer=c&cbll={$lat},{$lng}&hl=id&output=embed";
                // Also prepare normal map as secondary
                $mapUrl = "https://www.google.com/maps?q={$lat},{$lng}&hl=id&z=17&output=embed";
            } else {
                // Final fallback: use default coords for Balai Desa Ketapang Baru to ensure 360° availability
                $defaultLat = '-4.3221828';
                $defaultLng = '102.7635049';
                $embedStreetViewMapUrl = "https://maps.google.com/maps?layer=c&cbll={$defaultLat},{$defaultLng}&hl=id&output=embed";
                $mapUrl = "https://www.google.com/maps?q={$defaultLat},{$defaultLng}&hl=id&z=17&output=embed";
            }
        }
