...
class DataWargaController extends BaseController
{
    ...
    public function exportExcel(Request $request)
    {
        ...
        $filename = 'data-warga-' . date('Y-m-d-H-i-s') . '.xls';
        ...
        $callback = function() use ($groupedWargas) {
            ...
            foreach ($groupedWargas as $noKk => $keluarga) {
                ...
                echo '=== KELUARGA: ' . htmlspecialchars($noKk) . ' - ' . htmlspecialchars($namaKepala) . ' ===';
                echo '</td>';
                echo '</tr>';
                ...
                foreach ($sortedKeluarga as $warga) {
                    $ageData = $this->extractAgeFromNIK($warga->nik);
                    $bgColor = $warga->is_kepala_keluarga ? '#fff3cd' : '';

                    echo '<tr' . ($bgColor ? ' style="background-color: ' . $bgColor . ';"' : '') . '>';
                    echo '<td>' . $no++ . '</td>';
                    echo '<td>' . htmlspecialchars($warga->nik) . '</td>';
                    echo '<td>' . htmlspecialchars($warga->no_kk) . '</td>';
                    echo '<td>' . htmlspecialchars($warga->nama_lengkap) . '</td>';
                    echo '<td>' . htmlspecialchars($warga->jenis_kelamin) . '</td>';
                    echo '<td>' . htmlspecialchars($ageData['age'] . ' tahun') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->alamat ?? '-') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->rt_rw ?? '-') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->dusun ?? '-') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->agama ?? '-') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->status_perkawinan ?? '-') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->pendidikan ?? '-') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->mata_pencaharian ?? '-') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->status_aktif ? 'Aktif' : 'Tidak Aktif') . '</td>';
                    echo '<td>' . htmlspecialchars($warga->is_kepala_keluarga ? 'Ya' : 'Tidak') . '</td>';
                    echo '</tr>';
                }
