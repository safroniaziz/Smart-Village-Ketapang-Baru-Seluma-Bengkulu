...
class SuratController extends Controller
{
    ...
    /**
     * Tampilkan form public untuk surat kehilangan
     */
    public function showPublicForm()
    {
        return view('surat.public-kehilangan');
    }
    ...
    public function checkStatus($id)
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'submitted_at' => $pengajuan->submitted_at?->format('d/m/Y H:i'),
                'approved_at' => $pengajuan->approved_at?->format('d/m/Y H:i'),
                'rejected_at' => $pengajuan->rejected_at?->format('d/m/Y H:i'),
                'alasan_reject' => $pengajuan->alasan_reject,
                'is_public' => $pengajuan->is_public
            ]
        ]);
    }

    /**
     * Handle form surat-online yang sudah ada (public tanpa login)
     */
    public function handlePublicSuratOnline(Request $request)
    {
        ...
        // Validasi NIK dengan database warga
        $warga = Warga::where('nik', $request->nik)->first();
        if (!$warga) {
            return response()->json([
                'success' => false,
    ...
    public function generatePDFKehilangan($pengajuanId)
    {
        ...
        // Generate nomor surat sesuai format desa
        $nomorSurat = $this->generateNomorSurat('surat_kehilangan', $pengajuanId);
