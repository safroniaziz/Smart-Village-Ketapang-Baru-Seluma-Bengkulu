...
class HomeController extends Controller
{
    ...
    public function storePengaduan(PengaduanRequest $request)
    {
        ...
        try {
            ...
            // Send WhatsApp notification via Fonnte (see docs: https://docs.fonnte.com/send-whatsapp-message-with-php-api/)
            try {
                $statusUrl = route('pengaduan') . '?no=' . $pengaduan->nomor_tracking . '#cek-status';
                $kadesPhone = env('KADES_WA'); // e.g. 62XXXXXXXXXXX
                $fonnteToken = env('FONNTE_TOKEN', '9BtdhGbE3hpApQ3Ahc3V');

                if (!empty($kadesPhone) && !empty($fonnteToken)) {
                    ...
                    if (!empty($pengaduan->lampiran)) {
                        $publicUrl = Storage::disk('public')->url($pengaduan->lampiran);
                        $payload['url'] = $publicUrl;
                    }

                    Http::withHeaders([
                        'Authorization' => $fonnteToken,
                    ])->asForm()->post('https://api.fonnte.com/send', array_filter($payload));
                }
            } catch (\Throwable $e) {
