...
class SuratController extends Controller
{
    ...
    public function generatePDFUndangan($data)
    {
        $nomor = sprintf(
            '%02d/SP/KTB/%s/%s',
            rand(1, 99),
        ...
        // QR Code Service untuk verifikasi
        $qrCodeService = app(\App\Services\QrCodeService::class);
        $qrText = "Surat Undangan - No: {$nomor} - Kepada: " . ($data['kepada'] ?? 'Pihak Terkait');
        $qrCode = $qrCodeService->generate($qrText);
