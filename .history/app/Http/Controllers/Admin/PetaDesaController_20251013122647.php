...
class PetaDesaController extends Controller
{
    ...
    {
        // Load statistics directly in controller
        $lahanCount = LahanPoint::count();
        $batasWilayahCount = BatasWilayah::count();
        $wargaCount = User::whereNotNull('lat')
            ->whereNotNull('long')
            ->where('lat', '!=', '')
            ->where('long', '!=', '')
            ->where('is_kepala_keluarga', true)
            ->count();
        $desaCount = 1; // Should be 1 for monografi desa

        // Check if Google Street View URL exists
        $monografi = \App\Models\MonografiDesa::first();
        $streetViewCount = $monografi && $monografi->google_street_view_url ? 1 : 0;

        return view('admin.peta-desa.index', compact('lahanCount', 'batasWilayahCount', 'wargaCount', 'desaCount', 'streetViewCount'));
    }
    ...
    public function lahanIndex()
    {
        ...
        return view('admin.peta-desa.lahan.index', compact('lahan'));
    }
    ...
    public function batasWilayahIndex()
    {
        ...
        return view('admin.peta-desa.batas-wilayah.index', compact('batasWilayah'));
    }
