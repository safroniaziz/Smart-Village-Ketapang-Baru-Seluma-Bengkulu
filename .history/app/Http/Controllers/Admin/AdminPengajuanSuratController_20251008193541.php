...
class AdminPengajuanSuratController extends Controller
{
    ...
    private function sendWhatsAppPDF($pengajuan, $user)
    {
        try {
            ...
            $tempPath = storage_path('app/temp/surat_' . $pengajuan->id . '.pdf');
            ...
            $result = $fonnteService->sendDocument($user->no_hp, $message, $tempPath);
            ...
            Log::info('WhatsApp PDF sent successfully to ' . $user->no_hp);
            return $result;
            
        } catch (\Exception $e) {
            Log::error('Failed to send WhatsApp PDF: ' . $e->getMessage());
