...
class PetaDesaController extends Controller
{
    ...
    public function lahanData()
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('foto', function($row) {
                if ($row->foto_path) {
                    $photoUrl = asset($row->foto_path);
                    return '<img src="' . $photoUrl . '" alt="Foto Lahan" class="img-thumbnail" style="width: 50px; height: 50px; object-fit: cover;">';
                }
                return '<span class="text-muted">Tidak ada foto</span>';
            })
            ->addColumn('action', function($row) {
                $btn = '<div class="btn-group" role="group">';
                $btn .= '<button type="button" class="btn btn-info btn-sm" onclick="showLahan(' . $row->id . ')" title="Lihat Detail">
                            <i class="fas fa-eye"></i>
                        </button>';
                $btn .= '<button type="button" class="btn btn-warning btn-sm" onclick="editLahan(' . $row->id . ')" title="Edit">
                            <i class="fas fa-edit"></i>
                        </button>';
                $btn .= '<button type="button" class="btn btn-danger btn-sm" onclick="deleteLahan(' . $row->id . ')" title="Hapus">
    ...
    {
        $query = BatasWilayah::query()->ordered();
        ...
        return DataTables::of($query)
            ...
            ->addColumn('jarak', function($row) {
                return $row->jarak_km . ' km';
            })
            ->addColumn('action', function($row) {
                $btn = '<div class="btn-group" role="group">';
                $btn .= '<button type="button" class="btn btn-info btn-sm" onclick="showBatasWilayah(' . $row->id . ')" title="Lihat Detail">
                            <i class="fas fa-eye"></i>
                        </button>';
                $btn .= '<button type="button" class="btn btn-warning btn-sm" onclick="editBatasWilayah(' . $row->id . ')" title="Edit">
                            <i class="fas fa-edit"></i>
                        </button>';
                $btn .= '<button type="button" class="btn btn-danger btn-sm" onclick="deleteBatasWilayah(' . $row->id . ')" title="Hapus">
                            <i class="fas fa-trash"></i>
                        </button>';
                $btn .= '</div>';
                return $btn;
