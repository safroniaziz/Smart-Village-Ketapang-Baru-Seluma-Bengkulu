...
class PotensiWisataController extends Controller
{
    ...
    public function index(Request $request)
    {
        $query = PotensiWisata::query();
            ...
            // (kategori filter removed - database does not have kategori_wisata column)
        ...
        $categories = PotensiWisata::getAllKategori();
        $kategoriCounts = PotensiWisata::getKategoriWithCounts();

        return view('admin.potensi-wisata.index', compact('wisata', 'categories', 'kategoriCounts'));
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'sumber_video' => 'nullable|string|max:255',
            'file_potensi_desa' => 'nullable|file|mimes:pdf,doc,docx|max:10240',
        ...
        // Handle file upload for file_potensi_desa
        if ($request->hasFile('file_potensi_desa')) {
            $file = $request->file('file_potensi_desa');
            $fileName = time() . '_' . $file->getClientOriginalName();
            $file->move(public_path('assets/files'), $fileName);
            $data['file_potensi_desa'] = 'assets/files/' . $fileName;
        }
    ...
    public function update(Request $request, PotensiWisata $potensiWisata)
    {
        $validator = Validator::make($request->all(), [
            ...
            'deskripsi' => 'required|string',
            'lokasi' => 'required|string|max:255',
            'aktivitas_wisata' => 'nullable|string',
            'nomor_telepon' => 'nullable|string|max:20',
            'whatsapp' => 'nullable|string|max:20',
            'info_guide' => 'nullable|string',
            'jam_buka' => 'nullable|string|max:100',
            'harga_tiket' => 'nullable|string|max:100',
            'fasilitas_parkir' => 'nullable|string|max:100',
            'warung_makan' => 'nullable|string|max:100',
            'fitur_unggulan' => 'nullable|array',
            'fitur_unggulan.*.icon' => 'required_with:fitur_unggulan|string|max:50',
            'fitur_unggulan.*.judul' => 'required_with:fitur_unggulan|string|max:100',
            'fitur_unggulan.*.deskripsi' => 'required_with:fitur_unggulan|string|max:500',
            'jam_operasional' => 'required|string|max:100',
            'tiket_masuk' => 'required|string|max:100',
            'kontak' => 'nullable|string|max:50',
            'latitude' => 'nullable|numeric|between:-90,90',
            ...
            'deskripsi.required' => 'Deskripsi wajib diisi',
            'lokasi.required' => 'Lokasi wajib diisi',
            'fitur_unggulan.*.icon.required_with' => 'Ikon fitur unggulan wajib diisi',
            'fitur_unggulan.*.judul.required_with' => 'Judul fitur unggulan wajib diisi',
            'fitur_unggulan.*.deskripsi.required_with' => 'Deskripsi fitur unggulan wajib diisi',
            'jam_operasional.required' => 'Jam operasional wajib diisi',
            'tiket_masuk.required' => 'Informasi tiket masuk wajib diisi',
            'gambar.required' => 'Minimal satu gambar harus diupload',
            'gambar.*.url' => 'Format gambar harus berupa URL yang valid',
        ...
        if ($request->hasFile('file_potensi_desa')) {
            ...
            if ($potensiWisata->file_potensi_desa && file_exists(public_path($potensiWisata->file_potensi_desa))) {
                unlink(public_path($potensiWisata->file_potensi_desa));
            }
