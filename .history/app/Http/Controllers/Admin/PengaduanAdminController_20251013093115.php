...
class PengaduanAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        $stats = [
            ...
            'dalam_proses' => Pengaduan::where('status', 'Dalam Proses')->count(),
    ...
    public function update(int $id, Request $request)
    {
        ...
        // Get the logged-in user as the petugas
        $currentUser = Auth::user();
        $petugasId = $currentUser->id;
        $petugasName = $currentUser->nama_lengkap;
        ...
        try {
            ...
            $fonnteService->sendPengaduanFollowupToUser($p, $data['status'], $data['catatan_petugas'] ?? null);
        } catch (\Exception $e) {
            Log::error('Failed to send WhatsApp followup notification: ' . $e->getMessage());
