...
class HomeController extends Controller
{
    public function index()
    {
        ...
        // Data distribusi usia (menggunakan SQLite syntax)
        $usia0_17 = User::where('nik', '!=', '-')->whereNotNull('nik')
            ->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) BETWEEN 0 AND 17')
            ->count();
        $usia18_30 = User::where('nik', '!=', '-')->whereNotNull('nik')
            ->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) BETWEEN 18 AND 30')
            ->count();
        $usia31_45 = User::where('nik', '!=', '-')->whereNotNull('nik')
            ->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) BETWEEN 31 AND 45')
            ->count();
        $usia46_60 = User::where('nik', '!=', '-')->whereNotNull('nik')
            ->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) BETWEEN 46 AND 60')
            ->count();
        $usia60Plus = User::where('nik', '!=', '-')->whereNotNull('nik')
            ->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) >= 60')
            ->count();
    ...
    public function statistik()
    {
        ...
        // Enhanced age groups with proper categorization (SQLite syntax)
        $ageStats = User::whereNotNull('nik')->selectRaw('
            CASE
                WHEN ((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 5 THEN "Balita"
                WHEN ((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 12 THEN "Anak"
                WHEN ((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 18 THEN "Remaja"
                WHEN ((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 60 THEN "Dewasa"
                ELSE "Lansia"
            END as age_group,
            COUNT(*) as count
        ')
        ...
        // Calculate advanced demographic metrics (SQLite syntax)
        $averageAge = User::whereNotNull('nik')->whereNotNull('tanggal_lahir')
            ->selectRaw('AVG((julianday("now") - julianday(tanggal_lahir)) / 365.25) as avg_age')
            ->value('avg_age');
    ...
    public function dataWarga(Request $request)
    {
        ...
        if ($request->filled('age_range')) {
            switch ($request->age_range) {
                case 'child':
                    $query->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 17');
                    break;
                case 'adult':
                    $query->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) BETWEEN 17 AND 60');
                    break;
                case 'senior':
                    $query->whereRaw('((julianday("now") - julianday(tanggal_lahir)) / 365.25) > 60');
                    break;
            }
        }
        ...
        $filteredAgeStats = $statsQuery->selectRaw('
            CASE
                WHEN ((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 5 THEN "Balita"
                WHEN ((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 17 THEN "Anak"
                WHEN ((julianday("now") - julianday(tanggal_lahir)) / 365.25) < 60 THEN "Dewasa"
                ELSE "Lansia"
            END as age_group,
            COUNT(*) as count
