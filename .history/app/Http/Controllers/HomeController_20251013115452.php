...
class HomeController extends Controller
{
    ...
    {
        // Load struktur organisasi dari database
        $strukturPemerintahan = \App\Models\StrukturOrganisasi::with('children')
        ...
        $strukturBPD = \App\Models\StrukturOrganisasi::with('children')
            ->where('kategori', 'bpd')
            ->where('aktif', true)
            ->orderBy('urutan')
            ->get()
        ...
        // Hitung statistik
        $totalPemerintahan = \App\Models\StrukturOrganisasi::where('kategori', 'pemerintahan')
            ->where('aktif', true)
            ->count();

        $totalBPD = \App\Models\StrukturOrganisasi::where('kategori', 'bpd')
            ->where('aktif', true)
            ->count();

        $totalKepala = \App\Models\StrukturOrganisasi::where('level', 'kepala')
            ->where('aktif', true)
            ->count();

        $totalAparatur = \App\Models\StrukturOrganisasi::whereIn('level', ['sekretaris', 'kasi_kaur'])
            ->where('kategori', 'pemerintahan')
            ->where('aktif', true)
            ->count();
        ...
        $data = [
            ...
            'strukturBPD' => $strukturBPD,
            'stats' => [
                'totalKepala' => $totalKepala,
                'totalAparatur' => $totalAparatur,
                'totalBPD' => $totalBPD,
                'totalPemerintahan' => $totalPemerintahan,
            ]
        ];
    ...
    public function kontak()
    {
        ...
        $monografi = \App\Models\MonografiDesa::first();

        return view('pages.kontak', compact('kontak', 'monografi'));
    }
    ...
    public function statistik()
    {
        ...
        foreach ($users as $user) {
            if (!$user->tanggal_lahir) continue; // Skip users without birth date
            $age = $user->tanggal_lahir->diffInYears($currentDate);
            $totalAge += $age;
            $ageCount++;
        ...
        $totalAidValueFormatted = $totalAidValue > 1000000 ?
            'Rp ' . number_format($totalAidValue / 1000000, 1) . ' juta' :
            'Rp ' . number_format($totalAidValue / 1000, 0) . ' ribu';

        // Calculate average aid per recipient (dynamic from database)
        $uniqueRecipients = PenerimaBantuan::distinct('user_id')->count();
        $averageAidPerRecipient = $uniqueRecipients > 0 ? round($totalAidValue / $uniqueRecipients, 0) : 0;
        $averageAidFormatted = $averageAidPerRecipient >= 1000000 ?
            'Rp ' . number_format($averageAidPerRecipient / 1000000, 1) . ' juta' :
            'Rp ' . number_format($averageAidPerRecipient / 1000, 0) . ' ribu';

        // Calculate total active programs (dynamic from database)
        $totalActivePrograms = PenerimaBantuan::distinct('program')->count();

        // Get detailed recipients list per program for public display
        $recipientsByProgram = PenerimaBantuan::with('user')
            ->select('program', 'user_id', 'nominal', 'status', 'tahun', 'keterangan', 'created_at')
            ->orderBy('program')
            ->orderBy('created_at', 'desc')
            ->get()
            ->groupBy('program')
            ->map(function($recipients, $program) {
                return [
                    'program' => $program,
                    'total_recipients' => $recipients->count(),
                    'unique_recipients' => $recipients->unique('user_id')->count(),
                    'total_nominal' => $recipients->sum('nominal'),
                    'recipients' => $recipients->take(10), // Limit to 10 for display
                    'has_more' => $recipients->count() > 10
                ];
            });
        ...
        return view('pages.statistik', compact(
            ...
            'houseStatusStats', 'houseQualityStats', 'villageName', 'districtName', 'totalAidValueFormatted', 'averageAidFormatted', 'totalActivePrograms', 'recipientsByProgram'
        ));
    }
    ...
    public function dataWarga(Request $request)
    {
        ...
        if ($request->filled('status')) {
            $query->where('kewarganegaraan', $request->status);
        }
        ...
        if ($request->filled('age_range')) {
            ...
            $filteredUsers = $baseUsers->filter(function ($user) use ($request, $currentDate) {
                // Check if tanggal_lahir is not null before calling diffInYears
                if (!$user->tanggal_lahir) {
                    return false; // Skip users without birth date
                }
        ...
        // Order by dusun and group by keluarga (no_kk)
        $query->orderBy('dusun')->orderBy('no_kk')->orderBy('is_kepala_keluarga', 'desc');

        // Smart grouping logic based on search
        $searchTerm = $request->get('q', '');
        $familyGroups = collect();
        $isIndividualSearch = false;

        if ($searchTerm) {
            // Get all wargas first (with all filters except search)
            $allWargas = User::where('deleted_at', null);

            // Apply all filters except search
            if ($request->filled('gender')) {
                $allWargas->where('jenis_kelamin', $request->gender);
            }
            if ($request->filled('age_range')) {
                $ageRange = $request->age_range;
                $allWargas->where(function($query) use ($ageRange) {
                    $query->whereNotNull('tanggal_lahir');
                    switch($ageRange) {
                        case '0-17':
                            $query->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 0 AND 17');
                            break;
                        case '18-30':
                            $query->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 18 AND 30');
                            break;
                        case '31-45':
                            $query->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 31 AND 45');
                            break;
                        case '46-60':
                            $query->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 46 AND 60');
                            break;
                        case '60+':
            ...
            if ($request->filled('education')) {
                $allWargas->where('pendidikan', $request->education);
            }
            if ($request->filled('profession')) {
                $allWargas->where('mata_pencaharian', $request->profession);
            }
            if ($request->filled('dusun')) {
                $allWargas->where('dusun', $request->dusun);
            }
            if ($request->filled('religion')) {
                $allWargas->where('agama', $request->religion);
            }
            if ($request->filled('status')) {
                $allWargas->where('kewarganegaraan', $request->status);
            }
            if ($request->filled('status_aktif')) {
                $allWargas->where('status', $request->status_aktif);
            }

            $allWargas = $allWargas->get();

            // Check if any kepala keluarga matches the search
            $matchingKepalaKeluarga = $allWargas->where('is_kepala_keluarga', true)
                ->filter(function($warga) use ($searchTerm) {
                    return stripos($warga->nama_lengkap, $searchTerm) !== false;
                });

            if ($matchingKepalaKeluarga->isNotEmpty()) {
                // If kepala keluarga matches, show their families
                $matchingNoKK = $matchingKepalaKeluarga->pluck('no_kk');
                $familyGroups = $allWargas->whereIn('no_kk', $matchingNoKK)->groupBy('no_kk');
            } else {
                // If no kepala keluarga matches, show individuals only (with search filter)
                $allWargas = $query->get();
                $familyGroups = $allWargas->map(function($warga) {
                    return collect([$warga->id => collect([$warga])]);
                })->flatten(1);
                $isIndividualSearch = true;
            }
        } else {
            // No search, group normally by no_kk
            $allWargas = $query->get();
            $familyGroups = $allWargas->groupBy('no_kk');
        }

        // For pagination, we need to count families, not individuals
        $totalFamilies = $familyGroups->count();
        $offset = ($page - 1) * $perPage;
        $paginatedFamilies = $familyGroups->slice($offset, $perPage);

        // Create paginator manually for families
        $wargas = new \Illuminate\Pagination\LengthAwarePaginator(
            $paginatedFamilies,
            $totalFamilies,
            $perPage,
            $page,
        ...
        $data = compact(
            'wargas', 'familyGroups', 'totalWarga', 'totalKK', 'activeWarga', 'filteredTotal',
        ...
        if ($request->ajax()) {
            ...
            // Return table HTML with appropriate template
            if ($isIndividualSearch) {
                $tableHtml = view('partials.warga-table-individual', ['familyGroups' => $paginatedFamilies])->render();
            } else {
                $tableHtml = view('partials.warga-table-content', ['familyGroups' => $paginatedFamilies])->render();
            }
    ...
    {
        // Get distinct jenis surat dari pengajuan untuk menampilkan statistik
        $surat = PengajuanSurat::select('jenis_surat')
            ->distinct()
            ->get();
        ...
        return view('pages.surat-online', compact('surat', 'totalPengajuan', 'pengajuanDiterima', 'pengajuanMenunggu', 'pengajuanValid', 'pengajuanDitolak', 'jenisSuratTersedia'));
    }

    public function petaDesa()
    {
        $realData = \App\Models\LahanPoint::with('photos:id,lahan_point_id,path,order_index')
            ->select('id', 'nik', 'nama_lengkap', 'lat', 'long', 'foto_path')
            ->get()
            ->map(function ($p) {
                return [
                    'id' => $p->id,
                    ...
                    'photos' => $p->photos->map(fn($ph) => $ph->path)->values()->toArray(),
                ];
            })
            ->toArray();

        // Ambil data kepala keluarga yang memiliki koordinat dengan informasi lengkap
        $wargaData = \App\Models\User::select(
                'nik', 'nama_lengkap', 'lat', 'long', 'alamat', 'dusun', 'rt_rw',
                'no_kk', 'tempat_lahir', 'tanggal_lahir', 'jenis_kelamin',
                'agama', 'status_perkawinan', 'pendidikan',
                'is_kepala_keluarga', 'status_rumah', 'status_sosial',
                'kelayakan_rumah', 'mata_pencaharian', 'jumlah_jiwa_kk',
                'foto', 'status_aktif', 'status'
            )
            ->whereNotNull('lat')
            ->whereNotNull('long')
            ->where('lat', '!=', '')
            ...
            ->get()
            ->map(function($kepalaKeluarga) {
                // Ambil semua anggota keluarga berdasarkan no_kk
                $anggotaKeluarga = \App\Models\User::select('nik', 'nama_lengkap', 'jenis_kelamin', 'tempat_lahir', 'tanggal_lahir', 'agama', 'status_perkawinan', 'pendidikan', 'mata_pencaharian')
                    ->where('no_kk', $kepalaKeluarga->no_kk)
                    ->where('status_aktif', true)
                    ->get()
                    ->toArray();

                $kepalaKeluarga->anggota_keluarga = $anggotaKeluarga;
                return $kepalaKeluarga;
            })
            ->toArray();

        // Debug logging
        Log::info('Warga Data Count: ' . count($wargaData));
        if (count($wargaData) > 0) {
            Log::info('Sample Warga Data: ', array_slice($wargaData, 0, 3));
        }

        // List available lahan image filenames to avoid requesting non-existent files
        try {
            $files = \Illuminate\Support\Facades\File::files(public_path('assets/images/gambar_lahan'));
            $existingImages = collect($files)->map(fn($f) => $f->getFilename())->values()->toArray();
        } catch (\Throwable $e) {
            $existingImages = [];
        }

        // Ambil data desa untuk koordinat kantor desa dan nama desa
        $desaData = \App\Models\MonografiDesa::select('nama_desa', 'latitude', 'longitude')->first();

        // Ambil data batas wilayah desa
        $batasWilayah = \App\Models\BatasWilayah::all();

        return view('pages.peta-desa', compact('realData', 'wargaData', 'existingImages', 'desaData', 'batasWilayah'));
    }

    /**
     * Get count of warga with coordinates
     */
    public function wargaWithCoordinatesCount()
    {
        $count = \App\Models\User::whereNotNull('lat')
            ->whereNotNull('long')
            ->where('lat', '!=', '')
            ->where('long', '!=', '')
            ->where('is_kepala_keluarga', true)
    ...
    /**
     * Get count of desa data
     */
    public function desaCount()
    {
        $count = \App\Models\MonografiDesa::count();
        return response()->json($count);
    }
    ...
    public function pengaduan()
    {
        ...
        $pengaduanSelesaiCollection = Pengaduan::whereNotNull('tanggal_selesai')->get();
        $totalDays = 0;
        $count = 0;

        foreach ($pengaduanSelesaiCollection as $pengaduan) {
        ...
        $satisfactionRate = $pengaduanSelesaiCollection->count() > 0 ? round(($pengaduanSelesaiCollection->count() / ($pengaduanSelesaiCollection->count() + $pengaduanDitolak)) * 100, 1) : 0;
    ...
    public function storePengaduan(PengaduanRequest $request, FonnteService $fonnte)
    {
        ...
        try {
            ...
            // Determine origin (Warga Desa vs Eksternal)
            $isWarga = false;
            $wargaId = null;
            if (!empty($request->nik)) {
                $warga = \App\Models\User::whereNull('deleted_at')->where('nik', $request->nik)->first();
                if ($warga) { $isWarga = true; $wargaId = $warga->id; }
            }
            ...
            $pengaduan = Pengaduan::create([
                ...
                'asal_pelapor' => $isWarga ? 'Warga Desa' : 'Eksternal',
                'is_warga' => $isWarga,
                'status' => 'Diterima',
                'created_at' => now(),
                'updated_at' => now()
            ...
            // Notify user who made the complaint
            try {
                $fonnte->sendPengaduanConfirmationToUser($pengaduan);
            } catch (\Exception $e) {
                Log::error('Failed to send confirmation WhatsApp to user: ' . $e->getMessage());
                // Don't fail the main process if WhatsApp fails
            }
            ...
            if ($request->expectsJson()) {
                return response()->json([
                    ...
                    'asal_pelapor' => $pengaduan->asal_pelapor,
                    'redirect' => route('pengaduan') . '?no=' . $pengaduan->nomor_tracking . '#cek-status'
                ]);
            }

            return redirect()
                ->route('pengaduan', ['no' => $pengaduan->nomor_tracking])
                ->with('success', 'Pengaduan berhasil dikirim. Nomor tracking: ' . $pengaduan->nomor_tracking);
    ...
    public function statusPengaduan(string $nomor_tracking)
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'asal_pelapor' => $pengaduan->asal_pelapor,
                'is_warga' => (bool) $pengaduan->is_warga,
    ...
    public function cekNikPengaduan(string $nik)
    {
        $nik = trim($nik);
        if ($nik === '') {
            return response()->json([
        ...
        $user = \App\Models\User::whereNull('deleted_at')->where('nik', $nik)->first();
        if (!$user) {
            return response()->json([
                'success' => true,
                'is_warga' => false,
                'message' => 'Anda tidak terdaftar sebagai warga desa'
            ]);
        }
        ...
        return response()->json([
            ...
            'dusun' => $user->dusun,
            'message' => 'Anda terdaftar sebagai warga desa'
        ]);
    }
}
