...
class FonnteService
{
    ...
    /**
     * Send confirmation notification to user when they create a complaint
     */
    public function sendPengaduanConfirmationToUser(Pengaduan $pengaduan): void
    {
        if (empty($pengaduan->no_wa)) {
            Log::warning('No WhatsApp number for pengaduan confirmation ' . $pengaduan->id);
            return;
        }
        ...
        $message = "✅ *Pengaduan Anda Berhasil Dikirim!*\n\n"
            ...
            . "🏷️ *Jenis:* {$pengaduan->jenis_pengaduan}\n"
            . "⚡ *Prioritas:* {$pengaduan->prioritas}\n\n"
            . "🔍 *Cek Status Pengaduan:*\n{$statusUrl}\n\n"
            . "📞 *Kontak:* Hubungi kantor desa jika ada pertanyaan\n\n"
            . "_Terima kasih telah melaporkan pengaduan. Tim kami akan segera memproses laporan Anda._";

        $res = $this->sendWithResponse($pengaduan->no_wa, $message);
        Log::info('Pengaduan confirmation sent to user', [
