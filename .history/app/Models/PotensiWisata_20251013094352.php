...
class PotensiWisata extends Model
{
    ...
    {
        // Get categories from KategoriWisata model if it exists
        if (class_exists('App\Models\KategoriWisata')) {
            return \App\Models\KategoriWisata::withCount('potensiWisata')
                ->get()
                ->pluck('potensi_wisata_count', 'nama')
                ->toArray();
        }

        // Fallback to static categories with zero counts
        $categories = static::getKategoriOptions();
        $counts = [];
        
        foreach ($categories as $key => $label) {
            $counts[$key] = static::where('kategori_wisata', $key)->count();
        }
