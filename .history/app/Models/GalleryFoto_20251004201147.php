...
class GalleryFoto extends Model
{
    ...
    /**
     * Set this photo as hero (only one can be hero at a time)
     */
    public function setAsHero()
    {
        // Set all other photos to not hero
        static::where('is_hero', true)->update(['is_hero' => false]);
     ...
     * Get hero photo
     */
    public static function getHeroPhoto()
    {
        return static::hero()->aktif()->first();
    }

    /**
     * Get random photos from pantai category (for floating cards)
     */
    public static function getRandomPantaiPhotos($limit = 2)
    {
        return static::where('kategori', 'pantai')
                    ->where('is_hero', false) // Exclude hero photo
                    ->aktif()
                    ->inRandomOrder()
                    ->limit($limit)
                    ->get();
