...
class PotensiWisata extends Model
{
    ...
    {
        return $query->where('kategori_wisata_id', $kategoriId);
    }

    // Relationships - PotensiWisata is standalone, no category relationship
    ...
    public function getMainImageAttribute()
    {
        if (is_array($this->gambar) && count($this->gambar) > 0) {
            return $this->gambar[0]['url'] ?? asset('images/wisata/default-wisata.jpg');
        }
    ...
    {
        return static::getKategoriOptions();
    }

    public static function getKategoriWithCounts()
    {
        // Get categories from KategoriWisata model (for gallery foto)
        return \App\Models\KategoriWisata::withCount('galleryFoto')
            ->aktif()
            ->urutan()
            ->get()
            ->pluck('gallery_foto_count', 'nama')
            ->toArray();
    }
