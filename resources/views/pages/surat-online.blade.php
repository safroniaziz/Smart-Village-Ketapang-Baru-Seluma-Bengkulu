...
.progress-step {
    @apply flex items-center text-sm font-medium text-gray-500 relative;
}
...
.progress-step-number {
    @apply flex items-center justify-center w-8 h-8 border-2 border-gray-300 rounded-full mr-2 transition-all duration-300;
}

.progress-step.active .progress-step-number {
    @apply bg-blue-600 border-blue-600 text-white scale-110;
...
.progress-line {
    @apply absolute top-4 left-full w-full h-0.5 bg-gray-300 transition-all duration-500;
    z-index: -1;
}
...
.section-header {
    @apply mb-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200;
}
...
.nav-button.previous {
    @apply bg-gray-200 text-gray-700 hover:bg-gray-300 hover:shadow-md;
}

.nav-button.next {
    @apply bg-blue-600 text-white hover:bg-blue-700 hover:shadow-lg hover:scale-105;
}
...
/* Progress Summary */
.progress-summary {
    @apply bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 mb-6 border border-blue-200;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateModalForm(subJenis) {
        ...
        // Check if this is a multi-step form (Akta Kelahiran)
        if (subJenis === 'pengantar_akta_kelahiran') {
            generateMultiStepForm(template);
            return;
        }
    ...
    function generateMultiStepForm(template) {
        // Define form steps for Akta Kelahiran
        formSteps = [
            {
                title: 'Data Wilayah & KK',
                description: 'Informasi wilayah dan kepala keluarga',
                icon: 'fas fa-map-marked-alt',
                fields: ['kabupaten', 'kecamatan', 'desa', 'nama_kepala_keluarga', 'no_kk', 'surat_ket_kelahiran']
            },
            {
                title: 'Data Bayi/Anak',
                description: 'Informasi lengkap bayi yang dilahirkan',
                icon: 'fas fa-baby',
                fields: ['nama_bayi', 'jenis_kelamin_bayi', 'tempat_lahir_bayi', 'hari_tanggal_lahir', 'pukul_lahir', 'jenis_kelahiran', 'kelahiran_ke', 'penolong_kelahiran', 'berat_bayi', 'panjang_bayi']
            },
            {
                title: 'Data Ibu',
                description: 'Informasi lengkap ibu dari bayi',
                icon: 'fas fa-female',
                fields: ['nik_ibu', 'nama_ibu', 'tanggal_lahir_ibu', 'pekerjaan_ibu', 'alamat_ibu', 'kewarganegaraan_ibu', 'kebangsaan_ibu', 'tanggal_perkawinan']
            },
            {
                title: 'Data Ayah',
                description: 'Informasi lengkap ayah dari bayi',
                icon: 'fas fa-male',
                fields: ['nik_ayah', 'nama_ayah', 'tanggal_lahir_ayah', 'pekerjaan_ayah', 'alamat_ayah', 'kewarganegaraan_ayah']
            },
            {
                title: 'Data Pelapor & Saksi',
                description: 'Informasi pelapor dan saksi-saksi',
                icon: 'fas fa-users',
                fields: ['nik_pelapor', 'nama_pelapor', 'umur_pelapor', 'jenis_kelamin_pelapor', 'nik_saksi1', 'nama_saksi1', 'umur_saksi1', 'jenis_kelamin_saksi1', 'pekerjaan_saksi1', 'alamat_saksi1', 'nik_saksi2', 'nama_saksi2', 'umur_saksi2', 'jenis_kelamin_saksi2', 'pekerjaan_saksi2', 'alamat_saksi2', 'kepala_desa']
            }
        ];
        ...
        // Generate the multi-step form HTML
        generateMultiStepFormHTML(template);

        // Show the first step
        showFormStep(0);

        // Add event listeners
        addMultiStepEventListeners();
    }

    function generateMultiStepFormHTML(template) {
        // Generate progress indicator
        const progressHtml = `
            <div class="progress-summary mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="progress-summary-text">
                            <i class="fas fa-clipboard-list mr-2"></i>
                            Progress Pengisian Form
                        </h4>
                    </div>
                    <div class="text-sm font-medium text-blue-600">
                        Langkah <span id="currentStepNumber">1</span> dari ${formSteps.length}
                    </div>
                </div>
                <div class="mt-3 bg-gray-200 rounded-full h-2">
                    <div id="progressBar" class="bg-blue-600 h-2 rounded-full transition-all duration-500" style="width: ${(1/formSteps.length)*100}%"></div>
                </div>
            </div>
                ...
                ${formSteps.map((step, index) => `
                    <div class="progress-step ${index === 0 ? 'active' : ''}" id="step-${index}">
                        <div class="progress-step-number">
                            ${index < currentFormStep ? '<i class="fas fa-check text-sm"></i>' : index + 1}
                        </div>
                        <div class="hidden sm:block">
                            <div class="font-medium">${step.title}</div>
                            <div class="text-xs text-gray-500">${step.description}</div>
                        </div>
                        ${index < formSteps.length - 1 ? '<div class="progress-line"></div>' : ''}
                    </div>
                `).join('')}
            </div>
        `;
        ...
        const sectionsHtml = formSteps.map((step, index) => {
            const stepFields = template.fields.filter(field => step.fields.includes(field.name));
            const fieldsHtml = stepFields.map(field => generateFieldHTML(field)).join('');

            return `
                <div class="form-section ${index === 0 ? 'active' : ''}" id="section-${index}">
        ...
        const navigationHtml = `
            <div class="form-navigation">
                <button type="button" id="prevBtn" class="nav-button previous" onclick="previousStep()" style="display: none;">
                    <i class="fas fa-arrow-left mr-2"></i>
                    Sebelumnya
                </button>
                <div class="flex space-x-3">
                    <button type="button" id="nextBtn" class="nav-button next" onclick="nextStep()">
                        Selanjutnya
                        <i class="fas fa-arrow-right ml-2"></i>
                    </button>
                    <button type="submit" id="submitBtn" class="nav-button next" style="display: none;">
                        <i class="fas fa-paper-plane mr-2"></i>
                        Kirim Pengajuan
                    </button>
                </div>
        ...
        modalDynamicFormContainer.innerHTML = `
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-r-xl p-6 mb-8 shadow-sm">
                <div class="flex items-center mb-3">
                    ...
                    <div>
                        <h5 class="text-lg font-bold text-gray-900">${template.title}</h5>
                        <p class="text-sm text-gray-600">Formulir pelaporan kelahiran yang komprehensif</p>
                    </div>
                </div>
            </div>
    ...
    function generateFieldHTML(field) {
        ...
        if (field.type === 'select') {
            ...
            inputHtml = `
                <div class="relative">
                    <select name="${field.name}" ${field.required ? 'required' : ''}
                        class="w-full px-4 py-3 pr-10 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 appearance-none bg-white shadow-sm hover:border-gray-300"
                        onchange="validateField(this); handleFieldDependency(this)">
                        <option value="">Pilih ${field.label.toLowerCase()}</option>
                        ${options}
                    </select>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
            ...
            inputHtml = `
                <div class="relative">
                    <textarea name="${field.name}" ${field.required ? 'required' : ''} rows="4"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white shadow-sm hover:border-gray-300 resize-none"
                        placeholder="Masukkan ${field.label.toLowerCase()}..."
            ...
            inputHtml = `
                <div class="relative">
                    <input type="time" name="${field.name}" ${field.required ? 'required' : ''}
                        class="w-full px-4 py-3 pl-10 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white shadow-sm hover:border-gray-300"
            ...
            // Set default max date for birth dates and marriage
            let maxDate = '';
            let minDate = '';
            if (field.name === 'hari_tanggal_lahir') {
                maxDate = 'max="' + new Date().toISOString().split('T')[0] + '"';
            } else if (field.name.includes('tanggal_lahir')) {
                const today = new Date();
                const maxYear = today.getFullYear() - 15;
                const minYear = today.getFullYear() - 80;
                maxDate = 'max="' + maxYear + '-12-31"';
                minDate = 'min="' + minYear + '-01-01"';
            } else if (field.name === 'tanggal_perkawinan') {
                maxDate = 'max="' + new Date().toISOString().split('T')[0] + '"';
                minDate = 'min="1950-01-01"';
            }
            ...
            inputHtml = `
                <div class="relative">
                    <input type="date" name="${field.name}" ${field.required ? 'required' : ''}
                        class="w-full px-4 py-3 pl-10 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white shadow-sm hover:border-gray-300"
                        ${maxDate} ${minDate}
                        onchange="validateField(this); formatDateDisplay(this)">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <i class="${field.icon} text-gray-400"></i>
                    </div>
                </div>
            ...
            // Add input masks and specific validations
            if (field.name.includes('nik')) {
                inputAttributes = 'maxlength="16" pattern="[0-9]{16}"';
                inputMask = 'data-mask="nik"';
            } else if (field.name.includes('no_kk')) {
                inputAttributes = 'maxlength="16" pattern="[0-9]{16}"';
                inputMask = 'data-mask="nik"';
            } else if (field.name.includes('no_hp') || field.name.includes('telepon')) {
                inputAttributes = 'maxlength="15" pattern="[0-9+\\-\\s()]{8,15}"';
                inputMask = 'data-mask="phone"';
            } else if (field.name.includes('berat') || field.name.includes('panjang')) {
                inputAttributes = 'min="1" step="0.1"';
            } else if (field.name.includes('umur')) {
            ...
            inputHtml = `
                <div class="relative">
                    <input type="${field.type}" name="${field.name}" ${field.required ? 'required' : ''}
                        ...
                        ${inputAttributes} ${inputMask}
                        oninput="validateField(this); applyInputMask(this)"
        ...
        // Determine if field should span full width
        const isFullWidth = field.type === 'textarea' || field.name.includes('alamat');
        const colSpan = isFullWidth ? 'md:col-span-2' : '';

        // Generate tooltip content
        let tooltipContent = '';
        if (field.name.includes('nik')) {
            tooltipContent = 'NIK terdiri dari 16 digit dan harus sesuai dengan KTP asli';
        } else if (field.name.includes('no_kk')) {
            tooltipContent = 'Nomor Kartu Keluarga terdiri dari 16 digit sesuai dokumen KK';
        } else if (field.name === 'jenis_kelahiran') {
            tooltipContent = 'Pilih jenis kelahiran: Tunggal untuk satu bayi, Kembar untuk lebih dari satu';
        } else if (field.name === 'penolong_kelahiran') {
            tooltipContent = 'Siapa yang membantu proses persalinan (Dokter, Bidan, dll)';
        } else if (field.name.includes('berat_bayi')) {
            tooltipContent = 'Berat bayi saat lahir dalam satuan gram (contoh: 3200)';
        } else if (field.name.includes('panjang_bayi')) {
            tooltipContent = 'Panjang bayi saat lahir dalam satuan centimeter (contoh: 48)';
        } else if (field.name === 'kewarganegaraan_ibu' || field.name === 'kewarganegaraan_ayah') {
            tooltipContent = 'WNI untuk Warga Negara Indonesia, WNA untuk Warga Negara Asing';
        }

        const tooltip = tooltipContent ? `
            <div class="tooltip-container relative inline-block ml-1">
                <i class="fas fa-question-circle text-gray-400 text-xs cursor-help hover:text-blue-500 transition-colors"
                   onmouseenter="showTooltip(this, '${tooltipContent}')"
                   onmouseleave="hideTooltip()"></i>
            </div>
        ` : '';

        return `
            <div class="form-group ${colSpan} transition-all duration-300 hover:shadow-sm hover:bg-gray-50 rounded-lg p-2 -m-2">
                <label class="block text-sm font-semibold mb-3 text-gray-800">
                    <span class="flex items-center">
                        <i class="${field.icon} mr-2 text-blue-600 w-4"></i>
                        ${field.label}
                ...
                <div class="form-input-wrapper">
                    ${inputHtml}
                    <div class="validation-message" id="validation-${field.name}"></div>
                </div>
            </div>
        `;
    }
    ...
    function showFormStep(stepIndex) {
        ...
        if (progressBar && currentStepNumber) {
        ...
        formSteps.forEach((step, index) => {
            ...
            if (stepElement) {
                ...
                if (index < stepIndex) {
                    ...
                    const numberDiv = stepElement.querySelector('.progress-step-number');
                    numberDiv.innerHTML = '<i class="fas fa-check text-sm"></i>';
    ...
    function addMultiStepEventListeners() {
        ...
        document.querySelectorAll('input, select, textarea').forEach(field => {
            if (field.name) {
                ...
                // Add auto-complete for name fields
                if (field.name.includes('nama') && field.type === 'text') {
                    setupAutoComplete(field);
                }
            }
        });

        // Setup initial field dependencies
        setupFieldDependencies();

        // Setup enhanced UX features
        enhanceFormSections();
        setupFormFocusManagement();
        setupAutoSave();

        // Clear auto-save on form submission
        const form = document.getElementById('modalFormAjukanSurat');
        if (form) {
            form.addEventListener('submit', function() {
                localStorage.removeItem('akta_kelahiran_draft');
                sessionStorage.clear(); // Clear progress indicators
            });
        }
    }
    ...
    function setupAutoComplete(field) {
        ...
        // Common Indonesian names for different types
        if (field.name === 'nama_bayi') {
            suggestions = [
                'Ahmad', 'Muhammad', 'Siti', 'Nur', 'Budi', 'Andi', 'Dewi', 'Putra', 'Putri', 'Rizki',
                'Fahmi', 'Fajar', 'Indah', 'Lestari', 'Maharani', 'Pratama', 'Syahrul', 'Widya', 'Yoga', 'Zahra'
            ];
        } else if (field.name.includes('nama_ibu') || field.name.includes('nama_ayah')) {
            suggestions = [
                'Siti', 'Sri', 'Nur', 'Dewi', 'Rina', 'Maya', 'Lina', 'Dian', 'Yuni', 'Wati',
                'Ahmad', 'Budi', 'Andi', 'Dedi', 'Hadi', 'Joko', 'Rudi', 'Wawan', 'Yanto', 'Zaki'
            ];
        } else if (field.name.includes('pekerjaan')) {
            suggestions = [
                'Petani', 'Pedagang', 'Wiraswasta', 'Pegawai Negeri', 'Guru', 'Bidan', 'Perawat',
                'Mengurus Rumah Tangga', 'Buruh', 'Karyawan Swasta', 'Dokter', 'Polisi', 'TNI'
            ];
        }

        if (suggestions.length > 0) {
            setupFieldAutoComplete(field, suggestions);
        }
    }

    function setupFieldAutoComplete(field, suggestions) {
        let activeIndex = -1;
        let dropdown = null;
        ...
        field.addEventListener('input', function() {
            ...
            const matches = suggestions.filter(suggestion =>
                suggestion.toLowerCase().includes(value) && value.length > 0
            );
            ...
            if (matches.length > 0 && value.length > 0) {
                dropdown = createDropdown(matches, field);
                activeIndex = -1;
            }
        });
        ...
        field.addEventListener('keydown', function(e) {
            ...
            switch(e.key) {
                ...
                case 'Enter':
                    e.preventDefault();
                    if (activeIndex >= 0) {
                        field.value = items[activeIndex].textContent;
                        dropdown.remove();
                        dropdown = null;
                        validateField(field);
                    }
    ...
    function createDropdown(matches, field) {
        const dropdown = document.createElement('div');
        dropdown.className = 'autocomplete-dropdown absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-40 overflow-y-auto';
        dropdown.style.top = '100%';
        dropdown.style.left = '0';

        matches.slice(0, 8).forEach(match => {
            const item = document.createElement('div');
            item.className = 'autocomplete-item px-4 py-2 hover:bg-blue-50 cursor-pointer text-sm border-b border-gray-100 last:border-b-0';
    ...
    function updateActiveItem(items, activeIndex) {
        items.forEach((item, index) => {
            item.classList.toggle('bg-blue-100', index === activeIndex);
        });
    }

    // Field dependency handler
    window.handleFieldDependency = function(field) {
        const fieldName = field.name;
        const value = field.value;

        // Handle Jenis Kelahiran dependencies
        if (fieldName === 'jenis_kelahiran') {
            handleKelahiranDependency(value);
        }

        // Handle Kewarganegaraan dependencies
        if (fieldName.includes('kewarganegaraan')) {
            handleKewarganegaraanDependency(field, value);
        }

        // Handle Penolong Kelahiran dependencies
        if (fieldName === 'penolong_kelahiran') {
            handlePenolongDependency(value);
        }
    };

    function setupFieldDependencies() {
        // Setup all initial dependencies when form loads
        const jenisKelahiranField = document.querySelector('[name="jenis_kelahiran"]');
        if (jenisKelahiranField && jenisKelahiranField.value) {
            handleKelahiranDependency(jenisKelahiranField.value);
        }
    }

    function handleKelahiranDependency(jenisKelahiran) {
        // If kembar (twins), show additional information
        if (jenisKelahiran === 'Kembar Dua' || jenisKelahiran === 'Kembar Tiga') {
            showNotification('info', 'Untuk kelahiran kembar, pastikan data semua bayi dicatat dengan benar.');
        }
    }

    function handleKewarganegaraanDependency(field, kewarganegaraan) {
        const isIbu = field.name.includes('ibu');
        const isAyah = field.name.includes('ayah');

        let kebangsaanField;
        if (isIbu) {
            kebangsaanField = document.querySelector('[name="kebangsaan_ibu"]');
        } else if (isAyah) {
            kebangsaanField = document.querySelector('[name="kebangsaan_ayah"]');
        }

        if (kebangsaanField) {
            if (kewarganegaraan === 'WNI') {
                kebangsaanField.value = 'Indonesia';
                kebangsaanField.classList.add('bg-gray-100');
                kebangsaanField.setAttribute('readonly', 'true');
            } else {
                kebangsaanField.value = '';
                kebangsaanField.classList.remove('bg-gray-100');
                kebangsaanField.removeAttribute('readonly');
                kebangsaanField.focus();
            }
        }
    }

    function handlePenolongDependency(penolongKelahiran) {
        if (penolongKelahiran === 'Dukun Beranak') {
            showNotification('warning', 'Untuk keamanan, disarankan persalinan ditolong oleh tenaga medis profesional.');
        }
    }
    ...
    window.showTooltip = function(element, content) {
        ...
        tooltip.className = 'tooltip-popup absolute z-50 bg-gray-800 text-white text-xs rounded-lg px-3 py-2 shadow-lg max-w-xs';
        ...
        arrow.className = 'absolute top-full left-1/2 transform -translate-x-1/2 border-l-4 border-r-4 border-t-4 border-transparent border-t-gray-800';
    ...
    function enhanceFormSections() {
        // Add section collapse/expand functionality for better mobile experience
        const sectionHeaders = document.querySelectorAll('.section-header');

        sectionHeaders.forEach(header => {
            header.style.cursor = 'pointer';

            // Add click handler for mobile screens
            header.addEventListener('click', function(e) {
                if (window.innerWidth <= 768) { // Only on mobile
                    const section = this.parentElement;
                    const content = section.querySelector('.grid');
    ...
    function trackFormProgress() {
        ...
        // Update progress summary if it exists
        const progressSummary = document.querySelector('.progress-summary-text');
        if (progressSummary && totalFields > 0) {
            progressSummary.innerHTML = `
                <i class="fas fa-clipboard-list mr-2"></i>
                Progress Pengisian Form (${progressPercentage}% selesai)
            `;
        }

        // Show encouragement based on progress
        if (progressPercentage >= 75 && !sessionStorage.getItem('progress_75_shown')) {
            showNotification('success', 'Hebat! Anda hampir selesai mengisi formulir.');
            sessionStorage.setItem('progress_75_shown', 'true');
        } else if (progressPercentage >= 50 && !sessionStorage.getItem('progress_50_shown')) {
            showNotification('info', 'Bagus! Anda sudah mengisi separuh formulir.');
            sessionStorage.setItem('progress_50_shown', 'true');
        }
    }

    // Enhanced field focus management
    function setupFormFocusManagement() {
        ...
        inputs.forEach((input, index) => {
            input.addEventListener('keydown', function(e) {
                if (e.key === 'Enter' && this.tagName !== 'TEXTAREA') {
                    ...
                    if (nextInput && nextInput.closest('.form-section.active')) {
                        nextInput.focus();
                    } else {
                        // If no next field in current section, try to go to next step
                        const nextBtn = document.getElementById('nextBtn');
    ...
    function setupAutoSave() {
        ...
        inputs.forEach(input => {
            input.addEventListener('input', debounce(function() {
                ...
                localStorage.setItem('akta_kelahiran_draft', JSON.stringify(formData));
    ...
    function showSaveIndicator() {
        ...
        if (!indicator) {
            ...
            indicator.className = 'fixed bottom-4 right-4 bg-green-500 text-white px-3 py-1 rounded-full text-xs shadow-lg z-50';
            document.body.appendChild(indicator);
        }

        indicator.innerHTML = '<i class="fas fa-cloud mr-1"></i>Tersimpan otomatis';
    ...
    window.formatDateDisplay = function(dateInput) {
        ...
        // Show formatted date as helper text
        let helperText = dateInput.parentElement.querySelector('.date-helper');
        if (!helperText) {
            helperText = document.createElement('div');
            helperText.className = 'date-helper text-xs text-blue-600 mt-1 font-medium';
            dateInput.parentElement.appendChild(helperText);
        }

        helperText.textContent = `📅 ${formattedDate}`;

        // Auto-calculate age for birth dates
        if (dateInput.name.includes('tanggal_lahir')) {
            const today = new Date();
            const age = Math.floor((today - date) / (365.25 * 24 * 60 * 60 * 1000));

            // Update related age field if exists
            let ageFieldName = '';
            if (dateInput.name === 'tanggal_lahir_ibu') {
                ageFieldName = 'umur_ibu';
            } else if (dateInput.name === 'tanggal_lahir_ayah') {
                ageFieldName = 'umur_ayah';
            }

            if (ageFieldName) {
                const ageField = document.querySelector(`[name="${ageFieldName}"]`);
                if (ageField) {
                    ageField.value = `${age} Tahun`;
                    validateField(ageField);
                }
            }

            helperText.textContent = `📅 ${formattedDate} (${age} tahun)`;
        }
    };

    function loadAutoSavedData() {
        const savedData = localStorage.getItem('akta_kelahiran_draft');
        if (savedData) {
            try {
                ...
                showNotification('info', 'Data formulir yang tersimpan telah dimuat kembali.');
            } catch (error) {
    ...
    // Input mask function
    window.applyInputMask = function(field) {
        const maskType = field.getAttribute('data-mask');
        if (!maskType) return;

        let value = field.value.replace(/\D/g, ''); // Remove non-digits first
        ...
        switch (maskType) {
            case 'nik':
                ...
                if (value.length > 0) {
                    ...
                    if (value.length > 19) value = value.substring(0, 19); // Max length with dashes
                }
                break;
            case 'phone':
    ...
    window.validateField = function(field) {
        ...
        if (isRequired && !value) {
            ...
            if (validationDiv) {
                ...
                validationDiv.textContent = 'Field ini wajib diisi';
            }
        ...
        if (value) {
            ...
            // NIK validation
            if (fieldName.includes('nik')) {
                const nikDigits = value.replace(/\D/g, '');
                if (nikDigits.length !== 16) {
                    isValid = false;
                    message = 'NIK harus 16 digit';
                } else {
                    // Basic NIK format validation
                    const provinsi = nikDigits.substring(0, 2);
                    const kabkota = nikDigits.substring(2, 4);
                    const kecamatan = nikDigits.substring(4, 6);

                    if (parseInt(provinsi) < 1 || parseInt(provinsi) > 99) {
                        isValid = false;
                        message = 'Kode provinsi NIK tidak valid';
                    } else if (parseInt(kabkota) < 1 || parseInt(kabkota) > 99) {
                        isValid = false;
                        message = 'Kode kabupaten/kota NIK tidak valid';
                    }
                }
            }

            // KK Number validation
            if (fieldName.includes('no_kk')) {
                const kkDigits = value.replace(/\D/g, '');
            ...
            // Phone validation
            if (fieldName.includes('no_hp') || fieldName.includes('telepon')) {
                const phoneDigits = value.replace(/\D/g, '');
                if (phoneDigits.length < 8 || phoneDigits.length > 15) {
                    isValid = false;
                    message = 'Nomor telepon harus 8-15 digit';
                } else if (!phoneDigits.startsWith('0') && !phoneDigits.startsWith('62')) {
                    isValid = false;
            ...
            // Name validation
            if (fieldName.includes('nama') && fieldName !== 'nama_file') {
            ...
            // Weight and length validation for baby
            if (fieldName === 'berat_bayi') {
                const weight = parseFloat(value);
                if (weight < 500 || weight > 8000) {
                    isValid = false;
                    message = 'Berat bayi normalnya antara 500-8000 gram';
                }
            }

            if (fieldName === 'panjang_bayi') {
                const length = parseFloat(value);
                if (length < 25 || length > 70) {
                    isValid = false;
                    message = 'Panjang bayi normalnya antara 25-70 cm';
                }
            }

            // Age validation
            if (fieldName.includes('umur')) {
                const age = parseInt(value);
                if (age < 1 || age > 120) {
                    isValid = false;
                    message = 'Umur harus antara 1-120 tahun';
                }
            }

            // Address validation
            if (fieldName.includes('alamat')) {
                if (value.length < 10) {
                    isValid = false;
            ...
            if (field.type === 'date') {
                ...
                if (fieldName === 'hari_tanggal_lahir') {
                    // Birth date should not be in the future
                    if (inputDate > today) {
                        isValid = false;
                        message = 'Tanggal lahir tidak boleh di masa depan';
                    }
                    // Should not be too far in the past (150 years)
                ...
                } else if (fieldName.includes('tanggal_lahir')) {
                    // Parent birth dates should be reasonable
                    const maxAge = 80;
                    const minAge = 15;
                ...
                } else if (fieldName === 'tanggal_perkawinan') {
                    // Marriage date should not be in the future
                    if (inputDate > today) {
                        isValid = false;
                        message = 'Tanggal perkawinan tidak boleh di masa depan';
                    }

                    // Check if parents were old enough to marry
                    const ibuBirthField = document.querySelector('[name="tanggal_lahir_ibu"]');
                    const ayahBirthField = document.querySelector('[name="tanggal_lahir_ayah"]');

                    if (ibuBirthField && ibuBirthField.value) {
                        const ibuBirth = new Date(ibuBirthField.value);
                        const marriageAge = (inputDate - ibuBirth) / (365.25 * 24 * 60 * 60 * 1000);
                        if (marriageAge < 16) {
                            isValid = false;
                            message = 'Umur ibu saat menikah terlalu muda';
                        }
                    }

                    if (ayahBirthField && ayahBirthField.value) {
                        const ayahBirth = new Date(ayahBirthField.value);
                        const marriageAge = (inputDate - ayahBirth) / (365.25 * 24 * 60 * 60 * 1000);
                        if (marriageAge < 18) {
                            isValid = false;
                            message = 'Umur ayah saat menikah terlalu muda';
                        }
                    }
                }
            }
    ...
    function validateCurrentStep() {
        const currentStepFields = formSteps[currentFormStep].fields;
