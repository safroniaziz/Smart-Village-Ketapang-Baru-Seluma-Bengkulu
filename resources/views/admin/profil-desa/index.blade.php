            ...
            <div class="col-12 col-xl-6 col-xxl-6">
                <div class="card card-xxl-stretch mb-5 mb-xxl-8">
                    <div class="card-header border-0 pt-5">
                        ...
                        <div class="card-toolbar">
                            <button class="btn btn-sm btn-primary" onclick="addBatas()">
                    ...
                    <div class="card-body py-3">
                        ...
                        @if($batasWilayah->count() > 0)
                            <div class="table-responsive" title="💡 Klik dan seret untuk scroll horizontal">
                                <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                                    <thead>
                                        <tr class="border-bottom border-gray-200 fs-7 fw-bold text-gray-600 text-uppercase">
                                            <th>Arah</th>
                                    ...
                                    <tbody>
                                        @foreach($batasWilayah as $batas)
                                            <tr>
                                                <td class="fw-bold text-primary text-capitalize">{{ $batas->arah }}</td>
                                                <td class="text-dark fw-semibold">{{ $batas->berbatasan_dengan }}</td>
                                                <td class="text-muted">{{ $batas->jenis_wilayah ?? '-' }}</td>
                                                <td class="text-muted">{{ $batas->jarak_km ? number_format($batas->jarak_km, 2) . ' km' : '-' }}</td>
                                                <td class="text-muted">{{ $batas->landmark ?? '-' }}</td>
                                                <td class="text-muted">
                                                    @if($batas->keterangan)
                                                        {{ Str::limit($batas->keterangan, 50) }}
                                                    @else
                                                        -
                                                    @endif
                                                </td>
                                                <td>
                                                    <div class="d-flex gap-2">
                                                        <button class="btn btn-sm btn-light-warning" onclick="editBatas({{ $batas->id }})">
                                                            <i class="ki-duotone ki-pencil fs-6">
                                                                <span class="path1"></span>
                                                                <span class="path2"></span>
                                                            </i>
                                                        </button>
                                                        <button class="btn btn-sm btn-light-danger" onclick="deleteBatas({{ $batas->id }})">
                        ...
                        @else
                            <div class="text-center py-8">
                                ...
                                <div class="text-muted mb-4">Data batas wilayah belum diisi</div>
                                <button class="btn btn-primary" onclick="addBatas()">
                                    <i class="ki-duotone ki-plus fs-4 me-2">
                                        <span class="path1"></span>
                                        <span class="path2"></span>
                                    </i>
                                    Tambah Batas Pertama
                                </button>
                            </div>
                        @endif
                    </div>
                ...
                <div class="card card-xxl-stretch mb-5 mb-xxl-8">
                    <div class="card-header border-0 pt-5">
                        <h3 class="card-title align-items-start flex-column">
                            ...
                            <span class="text-muted fw-semibold fs-7">Informasi kondisi iklim wilayah desa</span>
                        </h3>
                        <div class="card-toolbar">
                            <button class="btn btn-sm btn-success" onclick="editIklim()">
                                <i class="ki-duotone ki-pencil fs-4 me-1">
                                    <span class="path1"></span>
                                    <span class="path2"></span>
                                </i>
                    ...
                    <div class="card-body py-3">
                        @if($iklim->count() > 0)
                            @php $iklimData = $iklim->first() @endphp
                            <div class="row g-4">
                                <!-- Jenis Iklim -->
                                <div class="col-md-6">
                                    <div class="d-flex flex-column bg-light-primary p-4 rounded-3">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="ki-duotone ki-weather-cloudy fs-2x text-primary me-3">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                            </i>
                                            <div>
                                                <span class="text-muted fs-7 fw-semibold d-block">Jenis Iklim</span>
                                                <span class="fw-bold fs-3 text-gray-800">{{ $iklimData->jenis_iklim ?? 'Belum diisi' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- Suhu -->
                                <div class="col-md-6">
                                    <div class="d-flex flex-column bg-light-warning p-4 rounded-3">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="ki-duotone ki-sun fs-2x text-warning me-3">
                                                <span class="path1"></span>
                                            </i>
                                            <div>
                                                <span class="text-muted fs-7 fw-semibold d-block">Rentang Suhu</span>
                                                <span class="fw-bold fs-3 text-gray-800">
                                                    @if($iklimData->suhu_min && $iklimData->suhu_max)
                                                        {{ $iklimData->suhu_min }}°C - {{ $iklimData->suhu_max }}°C
                                                    @else
                                                        Belum diisi
                                                    @endif
                                                </span>
                                ...
                                <!-- Kelembaban -->
                                <div class="col-md-6">
                                    <div class="d-flex flex-column bg-light-info p-4 rounded-3">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="ki-duotone ki-droplet fs-2x text-info me-3">
                                                <span class="path1"></span>
                                            </i>
                                            <div>
                                                <span class="text-muted fs-7 fw-semibold d-block">Kelembaban Rata-rata</span>
                                                <span class="fw-bold fs-3 text-gray-800">
                                                    {{ $iklimData->kelembaban_rata ? $iklimData->kelembaban_rata . '%' : 'Belum diisi' }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- Curah Hujan -->
                                <div class="col-md-6">
                                    <div class="d-flex flex-column bg-light-success p-4 rounded-3">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="ki-duotone ki-rain fs-2x text-success me-3">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                            </i>
                                            <div>
                                                <span class="text-muted fs-7 fw-semibold d-block">Curah Hujan Tahunan</span>
                                                <span class="fw-bold fs-3 text-gray-800">
                                                    {{ $iklimData->curah_hujan_tahunan ? number_format($iklimData->curah_hujan_tahunan) . ' mm' : 'Belum diisi' }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- Musim Kering -->
                                <div class="col-md-6">
                                    <div class="d-flex flex-column bg-light-danger p-4 rounded-3">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="ki-duotone ki-sun-2 fs-2x text-danger me-3">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                                <span class="path3"></span>
                                            </i>
                                            <div>
                                                <span class="text-muted fs-7 fw-semibold d-block">Musim Kering</span>
                                                <span class="fw-bold fs-6 text-gray-800">{{ $iklimData->musim_kering ?? 'Belum diisi' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- Musim Hujan -->
                                <div class="col-md-6">
                                    <div class="d-flex flex-column bg-light-dark p-4 rounded-3">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="ki-duotone ki-cloud-rain fs-2x text-dark me-3">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                                <span class="path3"></span>
                                            </i>
                                            <div>
                                                <span class="text-muted fs-7 fw-semibold d-block">Musim Hujan</span>
                                                <span class="fw-bold fs-6 text-gray-800">{{ $iklimData->musim_hujan ?? 'Belum diisi' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- Action button untuk edit saja -->
                            <div class="d-flex justify-content-end mt-4 pt-4 border-top">
                                <button class="btn btn-sm btn-primary" onclick="editIklim()">
                        ...
                        @else
                            <div class="text-center py-8">
                                <i class="ki-duotone ki-weather-cloudy fs-3x text-muted mb-3">
                                    <span class="path1"></span>
                                    <span class="path2"></span>
                                </i>
                                <div class="text-muted mb-4">Data iklim desa belum diisi</div>
                                <p class="text-muted fs-7 mb-4">
                                    Klik tombol Edit untuk mengisi data iklim desa
                                </p>
                                <button class="btn btn-primary" onclick="editIklim()">
                                                ...
                                                <td>
                                                    @if($potensiItem->foto)
                                                        <div class="symbol symbol-50px">
                                                            <img src="{{ asset('storage/' . $potensiItem->foto) }}" alt="{{ $potensiItem->nama_potensi }}" class="symbol-label" style="object-fit: cover; border-radius: 0.5rem;">
                                                        </div>
                                                    @else
                                                        <div class="symbol symbol-50px">
                                                            <div class="symbol-label bg-light-secondary d-flex align-items-center justify-content-center" style="border-radius: 0.5rem;">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="text-danger">Terjadi kesalahan saat memproses data.</p>
                </div>
...
function saveFasilitas() {
    ...
    const fileInput = document.getElementById('fasilitas_foto');
    if (fileInput && fileInput.files.length > 0) {
        console.log('File detected:', fileInput.files[0].name, fileInput.files[0].size);
    ...
    // Add method spoofing for Laravel when editing
    if (isEdit) {
    ...
    const method = 'POST'; // Always use POST, Laravel will handle method spoofing
    ...
    $.ajax({
        ...
        error: function(xhr) {
            showValidationError(xhr, 'Error Menyimpan Fasilitas');
        },
        complete: function() {
            submitBtn.setAttribute('data-kt-indicator', 'off');
        }
...
// Draggable Table Scroll Functionality
$(document).ready(function() {
    $('.table-responsive').each(function() {
        ...
        $(tableContainer).on('mousedown', function(e) {
            // Only start dragging if clicking on the table container, not on buttons or inputs
            if ($(e.target).closest('button, input, select, textarea, a').length) {
                return;
            }

            isDown = true;
            $(tableContainer).addClass('dragging no-select');
        ...
        $(document).on('mouseleave mouseup', function() {
            isDown = false;
            $(tableContainer).removeClass('dragging no-select');
        });
        ...
        $(tableContainer).on('touchstart', function(e) {
            ...
            isDown = true;
            $(tableContainer).addClass('dragging no-select');
            startX = e.touches[0].pageX - $(tableContainer).offset().left;
            scrollLeft = tableContainer.scrollLeft;
        });
        ...
        $(tableContainer).on('touchmove', function(e) {
            if (!isDown) return;
            const x = e.touches[0].pageX - $(tableContainer).offset().left;
        ...
        $(tableContainer).on('click', function(e) {
            if ($(tableContainer).hasClass('dragging')) {
                e.preventDefault();
                e.stopPropagation();
                return false;
            }
