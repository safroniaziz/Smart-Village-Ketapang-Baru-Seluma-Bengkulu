...
<div class="modal fade" id="tahapanModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content" style="border: none; border-radius: 2rem; overflow: hidden; box-shadow: 0 30px 60px rgba(0,0,0,0.3); background: linear-gradient(145deg, #ffffff 0%, #f8fafc 100%);">
            <form id="tahapanForm">
                <input type="hidden" name="_method" id="tahapan_method" value="POST">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea name="deskripsi" id="tahapan_deskripsi_input" class="form-control form-control-lg border-0 shadow-sm" rows="4" placeholder="Masukkan deskripsi tahapan..." required style="border-radius: 1rem; background: #ffffff;"></textarea>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
...
(function() {
    function initTahapanModal() {
        if (typeof $ === 'undefined') {
            setTimeout(initTahapanModal, 100);
            return;
        }
    ...
    $('#tahapanModal').on('hidden.bs.modal', function () {
        ...
        $('#tahapan_method').val('POST'); // Reset method to POST for add
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    setTimeout(function() {
                        if (typeof loadTahapanData === 'function') {
                            loadTahapanData();
                        } else {
    ...
    window.editTahapan = function(id) {
        ...
        $('#tahapan_method').val('PUT'); // Set method to PUT for edit

        $.get(`/visi-misi/tahapan/${id}/edit`, function(data) {
            $('#nama_tahapan_input').val(data.nama_tahapan);
            $('#tahapan_deskripsi_input').val(data.deskripsi);
