<!--begin::Modal - Visi-->
<div class="modal fade" id="visiModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content" style="border: none; border-radius: 2rem; overflow: hidden; box-shadow: 0 30px 60px rgba(0,0,0,0.3); background: linear-gradient(145deg, #ffffff 0%, #f8fafc 100%);">
            <form id="visiForm">
                <input type="hidden" name="_method" id="visi_method" value="POST">
                <div class="modal-header position-relative" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%); border: none; padding: 3rem 2rem; min-height: 200px;">
                    <!-- Floating Particles -->
                    <div class="position-absolute top-0 start-0 w-100 h-100" style="background: url('data:image/svg+xml,%3Csvg width=\"40\" height=\"40\" viewBox=\"0 0 40 40\" xmlns=\"http://www.w3.org/2000/svg\"%3E%3Cg fill=\"none\" fill-rule=\"evenodd\"%3E%3Cg fill=\"%23ffffff\" fill-opacity=\"0.1\"%3E%3Ccircle cx=\"20\" cy=\"20\" r=\"2\"/%3E%3C/g%3E%3C/g%3E%3C/svg%3E');"></div>

                    <!-- Gradient Overlays -->
                    <div class="position-absolute top-0 end-0" style="width: 200px; height: 200px; background: radial-gradient(circle, rgba(255,255,255,0.2) 0%, transparent 70%); transform: translate(50%, -50%);"></div>
                    <div class="position-absolute bottom-0 start-0" style="width: 150px; height: 150px; background: radial-gradient(circle, rgba(255,255,255,0.15) 0%, transparent 70%); transform: translate(-50%, 50%);"></div>

                    <!-- Top Accent Line -->
                    <div class="position-absolute top-0 start-0 w-100" style="height: 4px; background: linear-gradient(90deg, #ff6b6b, #4ecdc4, #45b7d1, #96ceb4, #feca57);"></div>

                    <div class="d-flex align-items-center position-relative" style="z-index: 10;">
                        <div class="symbol-label bg-white bg-opacity-25 backdrop-blur-sm" style="border-radius: 1.5rem; border: 2px solid rgba(255,255,255,0.3);">
                            <i class="ki-duotone ki-eye fs-1 text-white" style="filter: drop-shadow(0 4px 8px rgba(0,0,0,0.2));">
                                <span class="path1"></span>
                                <span class="path2"></span>
                                <span class="path3"></span>
                            </i>
                        </div>
                    </div>
                    <div>
                        <h2 class="text-white fw-bold fs-1 mb-2" id="visiModalTitle" style="text-shadow: 0 4px 12px rgba(0,0,0,0.3);">Tambah Visi</h2>
                        <p class="text-white fs-5 mb-0" style="opacity: 0.9; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">Kelola data visi desa dengan lengkap dan detail</p>
                    </div>
                    <div class="btn btn-icon btn-sm btn-active-icon-primary position-absolute top-0 end-0 m-4" data-bs-dismiss="modal" style="background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.3); color: white; border-radius: 50%;" title="Tutup modal">
                        <i class="ki-duotone ki-cross fs-1">
                            <span class="path1"></span>
                            <span class="path2"></span>
                        </i>
                    </div>
                </div>
                <div class="modal-body p-6" style="background: linear-gradient(145deg, #ffffff 0%, #f8fafc 100%);">
                    <div class="card border-0 shadow-sm" style="border-radius: 1.5rem; background: linear-gradient(145deg, #ffffff 0%, #f8fafc 100%);">
                        <div class="card-header bg-transparent border-0 pt-6 pb-3">
                            <div class="d-flex align-items-center">
                                <div class="symbol symbol-50px me-3">
                                    <div class="symbol-label bg-light-primary">
                                        <i class="ki-duotone ki-information-5 fs-2x text-primary">
                                            <span class="path1"></span>
                                            <span class="path2"></span>
                                        </i>
                                    </div>
                                </div>
                                <div>
                                    <h4 class="card-title text-gray-900 fw-bold mb-1">Informasi Visi</h4>
                                    <p class="text-muted fs-6 mb-0">Data utama visi desa</p>
                                </div>
                            </div>
                        </div>
                        <div class="card-body pt-0">
                            <div class="row g-4">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label class="form-label fw-bold text-gray-900 mb-3">
                                            <i class="ki-duotone ki-eye fs-5 text-primary me-2">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                                <span class="path3"></span>
                                            </i>
                                            Visi <span class="text-danger">*</span>
                                        </label>
                                        <input type="text" name="visi" id="visi_visi_input" class="form-control form-control-lg border-0 shadow-sm" placeholder="Masukkan visi desa..." required style="border-radius: 1rem; background: #ffffff;">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label class="form-label fw-bold text-gray-900 mb-3">
                                            <i class="ki-duotone ki-notepad-edit fs-5 text-success me-2">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                            </i>
                                            Deskripsi
                                        </label>
                                        <textarea name="deskripsi" id="visi_deskripsi_input" class="form-control form-control-lg border-0 shadow-sm" rows="4" placeholder="Masukkan deskripsi visi..." style="border-radius: 1rem; background: #ffffff;"></textarea>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label class="form-label fw-bold text-gray-900 mb-3">
                                            <i class="ki-duotone ki-document fs-5 text-warning me-2">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                            </i>
                                            Deskripsi Section
                                        </label>
                                        <textarea name="deskripsi_section" id="deskripsi_section_input" class="form-control form-control-lg border-0 shadow-sm" rows="3" placeholder="Masukkan deskripsi section untuk tampilan frontend..." style="border-radius: 1rem; background: #ffffff;"></textarea>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label class="form-label fw-bold text-gray-900 mb-3">
                                            <i class="ki-duotone ki-abstract-26 fs-5 text-info me-2">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                            </i>
                                            Pendekatan Deskripsi
                                        </label>
                                        <textarea name="pendekatan_deskripsi" id="visi_pendekatan_deskripsi_input" class="form-control form-control-lg border-0 shadow-sm" rows="3" placeholder="Masukkan deskripsi pendekatan..." style="border-radius: 1rem; background: #ffffff;"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer" style="background: linear-gradient(145deg, #ffffff 0%, #f8fafc 100%); border-top: 1px solid #e9ecef; padding: 2rem;">
                    <button type="button" class="btn btn-light btn-lg" data-bs-dismiss="modal" style="border-radius: 1rem; padding: 0.75rem 2rem; font-weight: 600;" title="Batal dan tutup modal">Batal</button>
                    <button type="submit" class="btn btn-primary btn-lg" id="visiSubmitBtn" style="border-radius: 1rem; padding: 0.75rem 2rem; font-weight: 600; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none;" title="Simpan data visi">
                        <span class="indicator-label">Simpan</span>
                        <span class="indicator-progress">Mohon tunggu...
                            <span class="spinner-border spinner-border-sm align-middle ms-2"></span>
                        </span>
                    </button>
                </div>
            </form>
        </div>
    </div>
</div>
<!--end::Modal - Visi-->

<script>
// Global variable for visi ID
let visiId = null;

// Wait for jQuery to be available
(function() {
    function initVisiModal() {
        if (typeof $ === 'undefined') {
            setTimeout(initVisiModal, 100);
            return;
        }

        $(document).ready(function() {
            // Reset form when modal is closed
            $('#visiModal').on('hidden.bs.modal', function () {
                $('#visiForm')[0].reset();
                visiId = null;
                $('#visiModalTitle').text('Tambah Visi');
                $('#visiSubmitBtn .indicator-label').text('Simpan');
                $('#visi_method').val('POST'); // Reset method to POST for add
            });

            // Form submission
            $('#visiForm').on('submit', function(e) {
                e.preventDefault();

                const submitBtn = $('#visiSubmitBtn');
                submitBtn.attr('data-kt-indicator', 'on');
                submitBtn.prop('disabled', true);

                const formData = new FormData(this);
                const url = visiId ? `/visi-misi/visi/${visiId}` : '/visi-misi/visi';
                const method = 'POST'; // Always use POST for method spoofing

                $.ajax({
                    url: url,
                    type: method,
                    data: formData,
                    processData: false,
                    contentType: false,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                title: 'Berhasil!',
                                text: response.message,
                                icon: 'success',
                                timer: 2000
                            });
                            $('#visiModal').modal('hide');
                            if (typeof window.loadVisiData === 'function') {
                                window.loadVisiData();
                            }
                        } else {
                            Swal.fire('Error!', response.message || 'Terjadi kesalahan', 'error');
                        }
                    },
                    error: function(xhr) {
                        const errors = xhr.responseJSON?.errors;
                        if (errors) {
                            let errorMessage = 'Terjadi kesalahan:\n';
                            Object.values(errors).forEach(error => {
                                errorMessage += `- ${error[0]}\n`;
                            });
                            Swal.fire('Error!', errorMessage, 'error');
                        } else {
                            Swal.fire('Error!', 'Terjadi kesalahan saat menyimpan data', 'error');
                        }
                    },
                    complete: function() {
                        submitBtn.removeAttr('data-kt-indicator');
                        submitBtn.prop('disabled', false);
                    }
                });
            });
        });
    }

    // Start initialization
    initVisiModal();
})();

// Edit function - global scope
window.editVisi = function(id) {
    console.log('editVisi called with id:', id);
    visiId = id;
    $('#visiModalTitle').text('Edit Visi');
    $('#visiSubmitBtn .indicator-label').text('Update');
    $('#visi_method').val('PUT'); // Set method to PUT for edit

    $.get(`/visi-misi/visi/${id}/edit`, function(data) {
        console.log('Visi data received in editVisi:', data);
        console.log('Setting visi value:', data.visi);
        console.log('Setting deskripsi value:', data.deskripsi);
        console.log('Setting deskripsi_section value:', data.deskripsi_section);
        console.log('Setting pendekatan_deskripsi value:', data.pendekatan_deskripsi);

        // Check if elements exist
        console.log('visi_visi_input exists:', $('#visi_visi_input').length);
        console.log('visi_deskripsi_input exists:', $('#visi_deskripsi_input').length);
        console.log('deskripsi_section_input exists:', $('#deskripsi_section_input').length);
        console.log('visi_pendekatan_deskripsi_input exists:', $('#visi_pendekatan_deskripsi_input').length);

        // Set values with debugging and delay
        setTimeout(function() {
            $('#visi_visi_input').val(data.visi || '');
            $('#visi_deskripsi_input').val(data.deskripsi || '');
            $('#deskripsi_section_input').val(data.deskripsi_section || '');
            $('#visi_pendekatan_deskripsi_input').val(data.pendekatan_deskripsi || '');

            // Verify values are set
            console.log('After setting - visi value:', $('#visi_visi_input').val());
            console.log('After setting - deskripsi value:', $('#visi_deskripsi_input').val());
            console.log('After setting - deskripsi_section value:', $('#deskripsi_section_input').val());
            console.log('After setting - pendekatan_deskripsi value:', $('#visi_pendekatan_deskripsi_input').val());
        }, 100);
    }).fail(function(xhr, status, error) {
        console.error('Error loading visi data:', xhr.responseText);
        console.error('Status:', status);
        console.error('Error:', error);
    });
};
</script>
