...
@section('title', 'Edit Penerima BLT DD')
@section('page-header', 'Edit P            <label for="nominal_bantuan" class="block text-sm font-medium text-gray-700 mb-2">
                Jumlah Bantuan (Rp) <span class="text-red-500">*</span>
            </label>
            <input type="number" name="nominal_bantuan" id="nominal_bantuan"
                   value="{{ old('nominal_bantuan', $bltDd->nominal_bantuan) }}"ma BLT DD')

@section('content')
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Penerima BLT Dana Desa</h1>
    ...
    <form action="{{ route('admin.blt-dd.update', $bltDd->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="nama_lengkap" id="nama_lengkap"
                       value="{{ old('nama_lengkap', $bltDd->nama_lengkap) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <select name="jenis_kelamin" id="jenis_kelamin"
                    ...
                    <option value="L" {{ old('jenis_kelamin', $bltDd->jenis_kelamin) == 'L' ? 'selected' : '' }}>Laki-laki</option>
                    <option value="P" {{ old('jenis_kelamin', $bltDd->jenis_kelamin) == 'P' ? 'selected' : '' }}>Perempuan</option>
                </select>
                @error('jenis_kelamin')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="tahun" id="tahun"
                    ...
                    @for($year = date('Y'); $year >= 2020; $year--)
                        <option value="{{ $year }}" {{ old('tahun', $bltDd->tahun) == $year ? 'selected' : '' }}>{{ $year }}</option>
                    @endfor
                </select>
                @error('tahun')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div>
            ...
            <textarea name="alamat" id="alamat" rows="3"
                      ...
                      required>{{ old('alamat', $bltDd->alamat) }}</textarea>
            @error('alamat')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="jumlah_bantuan" id="jumlah_bantuan"
                   value="{{ old('jumlah_bantuan', $bltDd->jumlah_bantuan) }}"
                   placeholder="Masukkan jumlah bantuan"
                   min="1"
                   class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nominal_bantuan') border-red-500 @else border-gray-300 @enderror"
                   required>
        ...
        <div>
            ...
            <textarea name="keterangan" id="keterangan" rows="3"
                      ...
                      class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('keterangan') border-red-500 @else border-gray-300 @enderror">{{ old('keterangan', $bltDd->keterangan) }}</textarea>
            @error('keterangan')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
