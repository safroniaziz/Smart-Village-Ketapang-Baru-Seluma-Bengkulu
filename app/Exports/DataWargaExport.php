...
class DataWargaExport implements FromCollection, WithHeadings, WithStyles, WithEvents
{
    ...
    {
        // Use same filtering logic as controller
        $wargas = $this->getFilteredData($this->request);
        ...
        foreach ($this->groupedData as $noKk => $keluarga) {
            ...
            foreach ($sortedKeluarga as $warga) {
                ...
                $this->exportData->push([
                    'no' => $no++,
                    'nik' => $this->sanitizeExcelValue($warga->nik ?? ''),
                    'no_kk' => $this->sanitizeExcelValue($warga->no_kk ?? ''),
                    'nama_lengkap' => $this->sanitizeExcelValue($warga->nama_lengkap ?? ''),
                    'jenis_kelamin' => $this->sanitizeExcelValue($warga->jenis_kelamin ?? ''),
                    'umur' => ($ageData['age'] ?? 0) . ' tahun',
                    'dusun' => $this->sanitizeExcelValue($warga->dusun ?? '-'),
                    'agama' => $this->sanitizeExcelValue($warga->agama ?? '-'),
                    'pendidikan' => $this->sanitizeExcelValue($warga->pendidikan ?? '-'),
                    'pekerjaan' => $this->sanitizeExcelValue($warga->mata_pencaharian ? ($warga->mata_pencaharian == 'DLL' ? 'Lain-Lain' : $warga->mata_pencaharian) : '-'),
    ...
    /**
     * Get filtered data for export (same as controller)
     */
    ...
    protected function sanitizeExcelValue($value)
    {
        ...
        // If value starts with formula indicators, add apostrophe to treat as text
        if (preg_match('/^[=+\-@]/', $value)) {
            return "'" . $value;
        }
